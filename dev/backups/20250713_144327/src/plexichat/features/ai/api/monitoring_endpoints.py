...
async def get_performance_analytics(
    ...
    try:
        ...
        recent_metrics = [
            m
            for m in analytics_engine.performance_buffer
            if m.timestamp >= start_time
            and (not model_id or m.model_id == model_id)
...
async def get_cost_analytics(
    ...
    try:
        ...
        recent_costs = [
            m
            for m in analytics_engine.cost_buffer
            if m.timestamp >= start_time
...
async def get_alert_history(
    ...
    try:
        ...
        return {"status": "success", "alerts": history}
...
async def monitoring_health_check():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Monitoring health check failed: {str(e)}"
        )
