...
class UserTable(SQLModel, table=True):
    ...
    username: str = SQLField(
        index=True, unique=True, max_length=50, description="Unique username"
    )
    email: str = SQLField(
        index=True, unique=True, max_length=255, description="User email address"
    )
    ...
    metadata: Optional[Dict[str, Any]] = SQLField(default=None, sa_column=Column(JSON))
