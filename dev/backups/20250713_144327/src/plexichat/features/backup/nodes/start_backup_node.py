...
def check_dependencies():
    ...
    if missing_packages:
        ...
        logger.info("(All dependencies are now consolidated in the root requirements.txt)")
        return False
...
def start_backup_node(args):
    ...
    try:
        ...
        logger.info(f" Failed to start backup node: {e}")
        sys.exit(1)
...
def show_status():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            logger.info(" Backup Node Status: RUNNING")
            logger.info(f" Node ID: {data.get('node_status', {}).get('node_id', 'Unknown')}")
            ...
            if storage:
                ...
                logger.info(f" Storage: {used_gb:.2f} GB / {max_gb:.2f} GB ({usage_pct:.1f}%)")

            shards = data.get('node_status', {}).get('shards', {})
            if shards:
                logger.info(f" Shards: {shards.get('total_count', 0)}")

            network = data.get('node_status', {}).get('network', {})
            if network:
                logger.info(f" Connected Nodes: {network.get('connected_nodes', 0)}")
        else:
            logger.info(" Backup Node Status: ERROR")
            logger.info(f"HTTP {response.status_code}: {response.text}")

    except httpx.ConnectError:
        logger.info(" Backup Node Status: OFFLINE")
        logger.info("The backup node is not running or not accessible")
    except Exception as e:
...
psutil = psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                cmdline = proc.info['cmdline']
                if cmdline and 'backup_node_main.py' in ' '.join(cmdline):
                    proc.terminate()
                    logger.info(f" Terminated backup node process (PID: {proc.info['pid']})")
                    return
...
psutil = psutil.AccessDenied):
        ...
        logger.info(" No backup node process found")

    except ImportError:
        logger.info(" psutil not available, cannot stop process automatically")
        logger.info("Please stop the backup node manually (Ctrl+C)")
...
Examples:
  ...
  python start_backup_node.py stop                     # Stop the node
    ...
    logger.info(" PlexiChat Backup Node Manager")
    logger.info("=" * 40)
