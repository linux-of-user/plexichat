...
class UpdateConfigRequest(BaseModel):
    ...
    auto_install: bool = Field(
        default=False, description="Enable automatic installation"
    )
...
async def download_update(
    background_tasks: BackgroundTasks, current_user=Depends(get_current_admin_user)
):
    ...
    try:
        ...
        if not update_info:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="No updates available"
            )
...
async def install_update(
    ...
    try:
        ...
        package_files = list(
            temp_path.glob(f"plexichat_update_{update_info.latest_version}.*")
        )
        ...
        async def install_task():
            try:
                ...
                if success:
                    logger.info(
                        f"Update installation completed: {update_info.latest_version}"
                    )
                else:
...
async def get_update_history(current_user=Depends(get_current_admin_user)):
    ...
    try:
        ...
        return UpdateHistoryResponse(history=history, total_updates=len(history))
...
async def configure_updates(
    config: UpdateConfigRequest, current_user=Depends(get_current_admin_user)
):
...
async def get_releases(
    include_prerelease: bool = False, current_user=Depends(get_current_admin_user)
):
