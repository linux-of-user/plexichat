...
class IndexDefinition:
    ...
    def _to_postgresql_sql(self) -> str:
        ...
        index_type_clause = (
            f" USING {self.index_type.value.upper()}"
...
class IndexAnalyzer:
    ...
    def _record_usage_patterns(self, analysis, execution_time_ms: float):
        ...
        for table in analysis.tables_accessed:
            ...
            patterns["avg_execution_time"] = (
                patterns["avg_execution_time"] * (patterns["access_count"] - 1)
                + execution_time_ms
    ...
    def _recommend_where_clause_indexes(
        ...
        where_pattern = re.compile(
            r"\bWHERE\s+(.+?)(?:\s+ORDER\s+BY|\s+GROUP\s+BY|\s+LIMIT|$)",
    ...
    def _recommend_join_indexes(
        ...
        join_pattern = re.compile(
            r"\bJOIN\s+(\w+)\s+.*?\bON\s+(\w+)\.(\w+)\s*=\s*(\w+)\.(\w+)", re.IGNORECASE
        )
    ...
    def _recommend_composite_indexes(
        ...
        if analysis.complexity_score > 3.0 and len(analysis.columns_accessed) > 2:
            for table in analysis.tables_accessed:
                ...
                frequent_columns = [
                    col
                    for col in analysis.columns_accessed
...
class IndexManager:
    ...
    def __init__(self):
        self.indexes: Dict[str, Dict[str, IndexDefinition]] = defaultdict(
            dict
        )  # db_name -> index_name -> definition
    ...
    async def analyze_and_recommend(
        self, database_name: str, client: AbstractDatabaseClient
    ) -> List[IndexRecommendation]:
        ...
        try:
            ...
            for query_hash, stats in query_stats.items():
                ...
                if avg_time and avg_time > 100:  # Focus on slower queries
                    ...
                    query_recommendations = self.analyzer.analyze_query_for_indexes(
                        query, float(avg_time)
                    )
                    recommendations.extend(query_recommendations)

            # Filter out recommendations for existing indexes
            recommendations = self._filter_existing_indexes(
    ...
    async def create_recommended_indexes(
        self, database_name: str, client: AbstractDatabaseClient, max_indexes: int = 5
    ) -> List[str]:
        ...
        top_recommendations = sorted(recommendations, key=lambda x: x.priority)[
            :max_indexes
        ]
        ...
        for recommendation in top_recommendations:
            try:
                ...
                logger.error(
                    f" Failed to create index {recommendation.index_definition.name}: {e}"
                )
    ...
    async def cleanup_unused_indexes(
        ...
        for index_name, index_def in self.indexes[database_name].items():
            ...
            if (
                index_def.scan_count == 0
                and index_def.last_used
                and index_def.last_used < cutoff_date
            ):
    ...
    async def _get_existing_indexes(
        self, client: AbstractDatabaseClient
    ...
    def _filter_existing_indexes(
        ...
        existing_indexes: Dict[str, IndexDefinition],
    ) -> List[IndexRecommendation]:
        ...
        for recommendation in recommendations:
            ...
            for existing_name, existing_def in existing_indexes.items():
                # Acknowledge unused variable
                _ = existing_name
                if existing_def.table == index_def.table and set(
                    existing_def.columns
                ) == set(index_def.columns):
                    exists = True
                    break
    ...
    def _prioritize_recommendations(
        ...
        return sorted(
            recommendations, key=lambda x: (x.priority, -x.estimated_improvement)
        )
    ...
    def get_index_report(self, database_name: str) -> Dict[str, Any]:
        ...
        active_indexes = len(
            [idx for idx in indexes.values() if idx.status == IndexStatus.ACTIVE]
        )
        ...
        return {
            ...
            "high_priority_recommendations": len(
                [r for r in recommendations if r.priority == 1]
            ),
