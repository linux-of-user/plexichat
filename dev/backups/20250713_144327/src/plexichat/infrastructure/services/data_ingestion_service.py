...
class DataIngestionService:
    ...
    def _init_default_processors(self):
        ...
        self.processors[DataSourceType.APPLICATION_LOGS] = (
            self._process_application_logs
        )
        self.processors[DataSourceType.USER_EVENTS] = self._process_user_events
        self.processors[DataSourceType.DATABASE_CHANGES] = (
        ...
        self.processors[DataSourceType.WEBSOCKET_EVENTS] = (
            self._process_websocket_events
        )
    ...
    async def ingest_log_entry(
        self, level: str, module: str, message: str, extra_data: Dict[str, Any] = None
    ) -> bool:
    ...
    async def ingest_database_change(
        self, table: str, operation: str, record_id: str, changes: Dict[str, Any]
    ) -> bool:
    ...
    async def _metrics_collector(self):
        ...
        while self.is_running:
            try:
                ...
                await self.ingest_system_metric(
                    "ingestion_events_per_minute", self.metrics["events_ingested"]
                )
                await self.ingest_system_metric(
                    "ingestion_buffer_size", len(self.event_buffer)
                )
