...
class AdvancedAntivirusEngine:
    ...
    async def initialize(self):
        ...
        if self.hash_scanner and hasattr(self.hash_scanner, "initialize"): await self.hash_scanner.initialize()
        if self.behavioral_analyzer and hasattr(self.behavioral_analyzer, "initialize"): await self.behavioral_analyzer.initialize()
        if self.link_scanner and hasattr(self.link_scanner, "initialize"): await self.link_scanner.initialize()
        if self.threat_intelligence and hasattr(self.threat_intelligence, "initialize"): await self.threat_intelligence.initialize()
    ...
    async def scan_file(self, file_path: str, scan_type: ScanType = ScanType.FULL_SCAN) -> ScanResult:
        ...
        if not self.file_path.exists():
            return self._create_error_result(str(self.file_path), "File not found", start_time)

        if self.file_path.stat().st_size > MAX_FILE_SIZE_SCAN:
            return self._create_warning_result(str(self.file_path),
        ...
        if scan_type in [ScanType.FULL_SCAN, ScanType.HASH_SCAN]:
            # Hash-based scanning
            hash_result = await self.hash_scanner.scan_hash(file_hash, str(self.file_path))
            scan_results.append(hash_result)

        if scan_type in [ScanType.FULL_SCAN, ScanType.FILENAME_SCAN]:
            # Filename analysis
            filename_result = await self.filename_analyzer.analyze_filename(str(self.file_path))
        ...
        if scan_type in [ScanType.FULL_SCAN, ScanType.BEHAVIORAL_SCAN]:
            ...
            if self.file_path.suffix.lower() in SUSPICIOUS_EXTENSIONS:
                behavioral_result = await self.behavioral_analyzer.analyze_file(str(self.file_path))
        ...
        logger.info(f"Scan completed: {self.file_path} - {final_result.threat_level.name} in {scan_duration:.2f}s")
        return final_result
