...
class FilenameAnalyzer:
    ...
    def __init__(self):
        ...
        self.advanced_patterns = [
            re.compile(
                r".*\.(exe|bat|cmd|com|pif|scr|vbs|js)\.(txt|doc|pdf|jpg|png)$",
                re.IGNORECASE,
            ),  # Double extensions
            re.compile(r".*update.*\.(exe|bat|cmd)$", re.IGNORECASE),  # Fake updates
            re.compile(
                r".*install.*\.(exe|bat|cmd)$", re.IGNORECASE
            ),  # Fake installers
            ...
            re.compile(
                r".*\d{4}-\d{2}-\d{2}.*\.(exe|zip|rar)$", re.IGNORECASE
            ),  # Date-based naming
        ]
        ...
        self.legitimate_patterns = [
            re.compile(
                r".*\.(txt|doc|docx|pdf|jpg|jpeg|png|gif|mp3|mp4|avi)$", re.IGNORECASE
            ),
            re.compile(
                r"^[a-zA-Z0-9_\-\.]+\.(exe|msi)$"
            ),  # Simple, clean executable names
        ]
    ...
    def _has_double_extension(self, filename: str) -> bool:
        ...
        return second_ext in common_extensions and parts[-1].lower() in [
            "exe",
            "bat",
            "cmd",
            "com",
    ...
    def _check_encoding_tricks(self, filename: str) -> float:
        ...
        if not filename.isascii() and any(
            filename.lower().endswith(ext) for ext in SUSPICIOUS_EXTENSIONS
        ):
            risk += 0.3
