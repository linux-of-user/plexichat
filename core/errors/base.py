...
class PlexiChatException(Exception):
    ...
    def __init__(self, code: str, details: Optional[Dict[str, Any]] = None, context: Optional[Dict[str, Any]] = None):
        self.code = code
        self.details = details or {}
        self.context = context or {}
        self.severity = ERROR_MAPPINGS.get(code, ErrorCodeMapping("", ErrorSeverity.MEDIUM, ErrorCategory.UNKNOWN, "")).severity
        self.category = ERROR_MAPPINGS.get(code, ErrorCodeMapping("", ErrorSeverity.MEDIUM, ErrorCategory.UNKNOWN, "")).category
        super().__init__(f"{code}: {ERROR_MAPPINGS.get(code, ErrorCodeMapping('', ErrorSeverity.MEDIUM, ErrorCategory.UNKNOWN, '')).message}")
...
def create_error_response(
    ...
    mapping = ERROR_MAPPINGS.get(code, ErrorCodeMapping(code, ErrorSeverity.MEDIUM, ErrorCategory.UNKNOWN, "Unknown error"))
    error_data = {
        "code": code,
...
def create_validation_error_response(field: str, value: Any, reason: str) -> ErrorResponse:
    """Specialized creator for validation errors."""
    details = {field: {"value": value, "reason": reason}}
    return create_error_response(PlexiChatErrorCode.VALIDATION_ERROR.value, details=details)
...
def handle_exception(
    ...
    code = PlexiChatErrorCode.SYSTEM_INTERNAL_ERROR.value  # Default; map based on exc type in full impl
