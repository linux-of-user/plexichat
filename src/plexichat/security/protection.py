"""
NetLink Security Protection System

Consolidates security protection functionality from:
- src/netlink/app/security/ (DDoS, rate limiting, input sanitization, etc.)
- src/netlink/core/security/ (penetration testing, vulnerability scanning, etc.)
...
class DDoSProtection:
    ...
    async def check_request(self, ip_address: str, user_agent: str, 
                           endpoint: str) -> Tuple[bool, Optional[SecurityThreat]]:
        """
        ...
        if len(recent_requests) > self.max_requests_per_minute:
            # Block IP for DDoS
            self.blocked_ips[ip_address] = current_time + timedelta(minutes=self.block_duration_minutes)
    ...
    async def _analyze_suspicious_behavior(self, ip_address: str, user_agent: str, endpoint: str) -> bool:
...
class RateLimiter:
    ...
    def __init__(self):
        self.request_counts: Dict[str, Dict[str, deque]] = defaultdict(lambda: defaultdict(lambda: deque(maxlen=1000)))
        self.rate_limits = {
    ...
    async def is_allowed(self, identifier: str, action: str = 'default') -> Tuple[bool, Optional[str]]:
        ...
        if len(requests) >= limit_config['requests']:
            return False, f"Rate limit exceeded for {action}: {len(requests)}/{limit_config['requests']} requests in {limit_config['window_minutes']} minutes"
...
class InputSanitizer:
    ...
    async def sanitize_input(self, input_data: str, input_type: str = 'general') -> Tuple[str, List[SecurityThreat]]:
        """
