...
class StorageNode:
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            ...
            (datetime.now(timezone.utc) - self.last_heartbeat).seconds < 300
        )
...
class DistributedStorageManager:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(f"Distributed storage manager initialized with {len(self.storage_nodes)} nodes")
    ...
    async def _discover_storage_nodes(self):
        ...
        for node_id, cluster_node in self.cluster_manager.cluster_nodes.items():
            ...
            if hasattr(cluster_node, 'capabilities') and 'storage' in cluster_node.capabilities:
                storage_node = StorageNode(
                    ...
                    geographic_region=getattr(cluster_node, 'geographic_region', 'default'),
    ...
    async def store_data(self, 
                        ...
                        replication_factor: int = None) -> StoredData:
        ...
        try:
            ...
            for node in [primary_node] + replica_nodes:
                ...
                node.available_capacity_gb = node.total_capacity_gb - node.used_capacity_gb
            ...
            logger.info(f"Stored data {data_id} on {len([primary_node] + replica_nodes)} nodes")
            return stored_data
    ...
    async def retrieve_data(self, data_id: str) -> Optional[bytes]:
        ...
        if primary_node and primary_node.is_healthy:
            try:
                ...
                logger.warning(f"Failed to retrieve from primary node {primary_node.node_id}: {e}")
        ...
        for replica_node_id in stored_data.replica_node_ids:
            ...
            if replica_node and replica_node.is_healthy:
                try:
                    ...
                    logger.warning(f"Failed to retrieve from replica node {replica_node.node_id}: {e}")
    ...
    async def delete_data(self, data_id: str) -> bool:
        ...
        if primary_node:
            try:
                ...
                primary_node.available_capacity_gb = primary_node.total_capacity_gb - primary_node.used_capacity_gb
                success_count += 1
            except Exception as e:
                logger.warning(f"Failed to delete from primary node {primary_node.node_id}: {e}")
        ...
        for replica_node_id in stored_data.replica_node_ids:
            ...
            if replica_node:
                try:
                    ...
                    replica_node.available_capacity_gb = replica_node.total_capacity_gb - replica_node.used_capacity_gb
                    success_count += 1
                except Exception as e:
                    logger.warning(f"Failed to delete from replica node {replica_node.node_id}: {e}")
        ...
        if success_count > 0:
            ...
            logger.info(f"Deleted data {data_id} from {success_count}/{total_nodes} nodes")
            return True
        
        return False
    
    async def _select_storage_nodes(self, data_size: int, replication_factor: int) -> Tuple[Optional[StorageNode], List[StorageNode]]:
        ...
        if len(available_nodes) < replication_factor + 1:
            logger.warning(f"Insufficient storage nodes: need {replication_factor + 1}, have {len(available_nodes)}")
        ...
        for node in available_nodes[1:]:
            ...
            if node.geographic_region not in used_regions or len(replica_nodes) < replication_factor // 2:
    ...
    async def _retrieve_from_node(self, node: StorageNode, data_id: str) -> Optional[bytes]:
    ...
    async def _cleanup_partial_storage(self, data_id: str, nodes: List[StorageNode]):
        ...
        for node in nodes:
            try:
                ...
                logger.warning(f"Failed to cleanup data {data_id} from node {node.node_id}: {e}")
    ...
    async def _handle_unhealthy_node(self, node: StorageNode):
        ...
        affected_data = [
            data for data in self.stored_data.values()
            if node.node_id == data.primary_node_id or node.node_id in data.replica_node_ids
        ]
    ...
    async def _ensure_replication(self, stored_data: StoredData):
        ...
        healthy_replicas = [
            node_id for node_id in [stored_data.primary_node_id] + stored_data.replica_node_ids
    ...
    async def _rebalance_storage(self):
        ...
        average_usage = total_usage / len(self.storage_nodes) if self.storage_nodes else 0
        ...
        for overloaded_node in overloaded_nodes:
            ...
            data_to_move = [
                data for data in self.stored_data.values()
                if overloaded_node.node_id in [data.primary_node_id] + data.replica_node_ids
            ]
    ...
    async def _move_data_replica(self, stored_data: StoredData, from_node: StorageNode, to_node: StorageNode):
        ...
        try:
            ...
            logger.info(f"Moved data {stored_data.data_id} from {from_node.node_id} to {to_node.node_id}")
    ...
    async def _cleanup_orphaned_data(self):
        ...
        for data in self.stored_data.values():
            healthy_nodes = [
                node_id for node_id in [data.primary_node_id] + data.replica_node_ids
                if node_id in self.storage_nodes and self.storage_nodes[node_id].is_healthy
            ]
    ...
    def get_storage_overview(self) -> Dict[str, Any]:
        ...
        total_capacity = sum(node.total_capacity_gb for node in self.storage_nodes.values())
        total_used = sum(node.used_capacity_gb for node in self.storage_nodes.values())
        
        return {
            "total_nodes": len(self.storage_nodes),
            "healthy_nodes": len([n for n in self.storage_nodes.values() if n.is_healthy]),
    ...
    def get_data_distribution(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_replicas": sum(len(data.replica_node_ids) + 1 for data in self.stored_data.values()),
            "average_replication_factor": sum(len(data.replica_node_ids) + 1 for data in self.stored_data.values()) / len(self.stored_data) if self.stored_data else 0
        }
