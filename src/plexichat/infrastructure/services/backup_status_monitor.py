...
class BackupStatusMonitor:
    ...
    async def get_real_time_status(
        ...
        try:
            ...
            if (
                ...
                and (datetime.now(UTC) - self.last_update).total_seconds()
            ...
            coverage_report = await self._calculate_backup_coverage(
            ...
            logger.info(
                f" Backup status: {coverage_report.overall_availability_percentage:.1f}% available"
            )
    ...
    async def _get_device_availability_statuses(self) -> list[DeviceAvailabilityStatus]:
        ...
        for device in devices:
            ...
            is_online = (
                ...
                and last_seen_minutes is not None
                and last_seen_minutes <= 5  # Consider online if seen within 5 minutes
            )
            ...
            shard_counts = self.session.exec(
                select(
                    ...
                    func.count(DeviceShardAssignment.id)
                    .filter(DeviceShardAssignment.is_verified)
                    .label("verified"),
                ).where(
            ...
            if device.total_storage_bytes > 0:
                storage_utilization = (
                    device.used_storage_bytes / device.total_storage_bytes
                ) * 100
    ...
    async def _get_shard_availability_statuses(self) -> list[ShardAvailabilityStatus]:
        ...
        for shard in shards_query:
            ...
            assignments = self.session.exec(
                ...
                .join(
                    StorageDevice, DeviceShardAssignment.device_id == StorageDevice.id
                )
                .where(
            ...
            for assignment, device in assignments:
                ...
                if device.last_seen_at:
                    ...
                    if (
                        ...
                        and device.status == DeviceStatus.ONLINE
                    ):
            ...
            availability_percentage = (
                (online_assignments / total_assignments * 100)
    ...
    async def _calculate_backup_coverage(
        ...
        fully_available_backups = len(
            [b for b in backup_coverage.values() if b["fully_available"]]
        )
        partially_available_backups = len(
            [
                b
                for b in backup_coverage.values()
                if not b["fully_available"] and b["available_shards"] > 0
            ]
        )
        unavailable_backups = len(
            [b for b in backup_coverage.values() if b["available_shards"] == 0]
        )
        ...
        critical_shards = [
            s for s in shard_statuses if s.redundancy_level == "critical"
        ]
        if critical_shards:
            critical_issues.append(
                f"{len(critical_shards)} shards have critical redundancy (only 1 copy available)"
            )

        # Check for unavailable backups
        if unavailable_backups > 0:
            critical_issues.append(
                f"{unavailable_backups} backups are completely unavailable"
            )
        ...
        if total_devices > 0:
            ...
            if device_availability < 50:
                critical_issues.append(
                    f"Low device availability: {device_availability:.1f}%"
                )
            elif device_availability < 80:
                warnings.append(
                    f"Moderate device availability: {device_availability:.1f}%"
                )

        # Check for low redundancy shards
        low_redundancy_shards = [
            s for s in shard_statuses if s.redundancy_level == "low"
        ]
        if low_redundancy_shards:
            warnings.append(
                f"{len(low_redundancy_shards)} shards have low redundancy (only 2 copies available)"
            )

        # Check for unverified shards
        unverified_shards = [
            s for s in shard_statuses if s.verified_assignments < s.total_assignments
        ]
        ...
        try:
            ...
            for device in device_statuses:
                if device.is_online:
                    ...
                    status_key = (
                        "offline" if device.status == "offline" else "unreachable"
                    )
            ...
            return {
                ...
                "summary": {
                    ...
                    "network_health_percentage": (
                        (len(status_groups["online"]) / len(device_statuses) * 100)
                        if device_statuses
                        else 0
                    ),
                },
    ...
    async def get_shard_distribution_map(self) -> dict[str, Any]:
        ...
        try:
            ...
            for shard in shard_statuses:
                ...
                distribution_data["statistics"]["redundancy_levels"][
