...
class MessageCache:
    ...
    def get_messages_for_user(self, user_id: int) -> List[P2PMessage]:
        ...
        return [msg for msg in self.cache.values() 
    ...
    def _save_to_disk(self):
        ...
        try:
            cache_data = {
                msg_id: msg.to_dict() 
                for msg_id, msg in self.cache.items()
            }
        ...
        try:
            ...
            if other_user_id:
                cached_messages = [
                    ...
                    if (msg.sender_id == other_user_id and msg.recipient_id == user_id) or
                       (msg.sender_id == user_id and msg.recipient_id == other_user_id)
                ]
            ...
            if self.database_available:
                db_messages = await self._get_from_database(user_id, other_user_id, limit)
        ...
        while True:
            try:
                ...
                for user_id, peer in self.peers.items():
                    ...
                    time_since_last_seen = (current_time - peer.last_seen).total_seconds()
