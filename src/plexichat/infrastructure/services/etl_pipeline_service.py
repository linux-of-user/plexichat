...
if # SECURITY: eval() removed - use safe alternativeseval_condition):
            ...
            for agg_field, agg_func in aggregations.items():
                if agg_func == "count":
                    ...
                    values = [
                        ...
                        if isinstance(r.get(agg_field), (int, float))
                    ]
    ...
    async def _join_transform()
        ...
        join_lookup = {
            record[join_key]: record for record in join_data if join_key in record
        }
...
class ETLPipelineService:
    ...
    async def start(self):
        ...
        self.background_tasks.append(asyncio.create_task(self._pipeline_scheduler()))
