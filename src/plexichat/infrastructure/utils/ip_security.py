...
class IPSecurityManager:
    ...
    def _init_geoip(self):
        ...
        try:
            ...
            if not self.geoip_db:
                logger.warning(
                    "GeoIP database not found. Geographic blocking disabled."
                )
        ...
        try:
            ...
            if config_path.exists():
                ...
                for network_str in data.get("whitelist_networks", []):
                    try:
                        self.whitelist_networks.append(
                            ipaddress.ip_network(network_str)
                        )
                    except Exception as e:
                        logger.warning(
                            "Invalid whitelist network %s: %s", network_str, e
                        )

                for network_str in data.get("blacklist_networks", []):
                    try:
                        self.blacklist_networks.append(
                            ipaddress.ip_network(network_str)
                        )
                    except Exception as e:
                        logger.warning(
                            "Invalid blacklist network %s: %s", network_str, e
                        )
                ...
                for ip, timestamp_str in temp_blacklist_data.items():
                    try:
                        ...
                        logger.warning(
                            "Invalid temp blacklist timestamp for %s: %s", ip, e
                        )

                logger.info(
                    "IP security configuration loaded from %s", self.config_file
                )
        ...
        try:
            ...
            data = {
                ...
                "whitelist_networks": [
                    ...
                    if not self._is_default_safe_network(net)
                ],
        ...
        try:
            ...
            if self.config.get("enable_blacklist", True):
                ...
                for network in self.blacklist_networks:
                    if ip_obj in network:
                        logger.warning(
                            "Blocked IP %s (blacklisted network %s)", ip, network
                        )
                        return False
            ...
            if ip in self.temp_blacklist:
                ...
                if datetime.now() < block_until:
                    logger.warning(
                        "Blocked IP %s (temporarily blocked until %s)", ip, block_until
                    )
                    return False
                else:
            ...
            if self.config.get("enable_geo_blocking", False) and self.geoip_db:
                ...
                if country:
                    if self.allowed_countries and country not in self.allowed_countries:
                        logger.warning(
                            "Blocked IP %s (country %s not allowed)", ip, country
                        )
                        return False

                    if country in self.blocked_countries:
                        logger.warning(
                            "Blocked IP %s (country %s blocked)", ip, country
                        )
                        return False
    ...
    def _mark_suspicious(self, ip: str):
        ...
        logger.warning(
            "Marked IP %s as suspicious (%d failed attempts)",
            ip,
            self.suspicious_ips[ip]["failed_attempts"],
        )
