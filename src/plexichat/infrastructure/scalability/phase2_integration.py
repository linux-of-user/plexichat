...
class Phase2ScalabilityCoordinator:
    ...
    def _register_default_task_handlers(self):
        ...
        async def email_notification_handler(payload: Dict[str, Any]):
            ...
            logger.info(f"Processing email notification: {payload.get('subject', 'No subject')}")
        ...
        # Register handlers
        self.task_queue_manager.register_task_handler("email_notification", email_notification_handler)
        self.task_queue_manager.register_task_handler("file_processing", file_processing_handler)
        self.task_queue_manager.register_task_handler("ai_processing", ai_processing_handler)
    ...
    async def _collect_metrics(self):
        ...
        try:
            ...
            self.stats["total_services"] = registry_stats["statistics"]["total_services"]
            self.stats["healthy_services"] = registry_stats["statistics"]["healthy_services"]
            
            # Get cache stats
            cache_stats = self.distributed_cache.get_cache_statistics()
            self.stats["cache_nodes"] = cache_stats["total_nodes"]
            self.metrics.cache_hit_rate_percent = cache_stats.get("hit_rate_percent", 0.0)
            ...
            orchestrator_stats = self.microservices_orchestrator.get_orchestrator_status()
    ...
    async def _evaluate_scaling_decisions(self):
        ...
        try:
            ...
            healthy_ratio = self.stats["healthy_services"] / max(self.stats["total_services"], 1)
            
            if healthy_ratio < 0.8:  # Less than 80% services healthy
                await self._scale_up()
            elif healthy_ratio > 0.95 and self.stats["total_services"] > self.min_replicas:
