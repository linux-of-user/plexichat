...
try:
    ...
    logger.info(f" Failed to import PlexiChat modules: {e}")
    logger.info("Make sure you're running this from the PlexiChat project directory")
    sys.exit(1)
...
def print_banner():
    ...
    logger.info("=" * 60)
    logger.info("  PlexiChat Database Setup Tool")
    logger.info("=" * 60)
    print()
...
def print_status(message: str, status: str = "info"):
    ...
    logger.info(f"{icons.get(status, '')} {message}")

async def interactive_setup():
    """Run interactive database setup."""
    print_banner()
    logger.info("Welcome to the PlexiChat Database Setup Wizard!")
    logger.info("This tool will help you configure your database connection.")
    print()

    # Step 1: Choose database type
    logger.info("Step 1: Choose Database Type")
    logger.info("-" * 30)
    ...
    for i, db_info in enumerate(db_types["database_types"], 1):
        ...
        logger.info(f"{i}. {config['name']}{recommended}")
    ...
    logger.info("Step 4: Advanced Settings (Optional)")
    logger.info("-" * 30)
    ...
    logger.info("Step 5: Testing Connection")
    logger.info("-" * 30)
    ...
    if result["success"]:
        ...
        logger.info(f"   Database version: {test_results.get('version_info', 'Unknown')}")
        logger.info(f"   Response time: {test_results.get('response_time_ms', 0):.2f}ms")
        print()
    else:
    ...
    logger.info("Step 6: Initialize Database Schema")
    logger.info("-" * 30)
    ...
    if result["success"]:
        ...
        logger.info(f"   Tables created: {schema_results.get('tables_created', 0)}")
        if create_sample:
            logger.info(f"   Sample data: {'Created' if schema_results.get('sample_data_created') else 'Failed'}")
        print()
    else:
    ...
    logger.info("Step 7: Save Configuration")
    logger.info("-" * 30)
    ...
    logger.info("-" * 30)
    logger.info("Your PlexiChat database is now configured and ready to use.")
    logger.info("You can start the PlexiChat server with: python -m src.plexichat.app.main")
    print()
...
async def quick_setup(db_type: str, **kwargs):
    ...
    print_banner()
    logger.info(f"Quick setup for {db_type} database...")
    print()
...
def show_status():
    ...
    logger.info("Database Setup Status")
    logger.info("-" * 30)
    logger.info(f"Current step: {status['current_step']}")
    logger.info(f"Progress: {status['progress_percentage']:.1f}%")
    logger.info(f"Completed steps: {len(status['completed_steps'])}/{status['total_steps']}")
