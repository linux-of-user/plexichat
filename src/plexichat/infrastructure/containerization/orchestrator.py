...
class ContainerOrchestrator:
    ...
    async def generate_docker_compose(self) -> str:
        ...
        compose_config = {
            "version": "3.8",
            "services": {},
            "networks": {"plexichat-network": {"driver": "bridge"}},
            "volumes": {},
        }
        ...
        for container_name, config in self.containers.items():
            ...
            if config.ports:
                service_config["ports"] = [
                    f"{host}:{container}" for container, host in config.ports.items()
                ]
            ...
            if config.volumes:
                service_config["volumes"] = [
                    f"{host}:{container}" for host, container in config.volumes.items()
                ]
        ...
        for container_name, config in self.containers.items():
            ...
            deployment = {
                ...
                "metadata": {
                    ...
                    "labels": {"app": config.name, "component": "plexichat"},
                },
                "spec": {
                    "replicas": 1,
