...
class Task:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Task':
        ...
        for field_name in ["created_at", "scheduled_at", "execute_after", "started_at", "completed_at"]:
...
class TaskWorker:
    ...
    async def _handle_task_failure(self, task: Task, error_message: str):
        ...
        if task.retry_count < task.max_retries:
            ...
            task.execute_after = datetime.now(timezone.utc) + timedelta(seconds=task.retry_delay)
...
class AsyncTaskQueueManager:
    ...
    async def _add_task_to_queue(self, task: Task):
        ...
        # Persist to Redis
        if self.redis:
            await self.redis.lpush(f"queue:{task.queue_name}", json.dumps(task.to_dict()))
    ...
    async def get_next_task(self) -> Optional[Task]:
        ...
        sorted_queues = sorted(self.queues.items(), key=lambda x: self.queue_priorities.get(x[0], 999))
    ...
    async def schedule_task(self, task: Task):
        ...
        if self.redis:
            ...
            score = task.execute_after.timestamp() if task.execute_after else time.time()
            await self.redis.zadd("scheduled_tasks", {json.dumps(task.to_dict()): score})
