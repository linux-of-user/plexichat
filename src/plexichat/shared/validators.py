...
def validate_string_length(
    ...
    if not isinstance(value, str):
        raise ValidationError(
            f"{field_name} must be a string", details={"type": type(value).__name__}
        )
...
def validate_email(email: str, field_name: str = "email") -> None:
    ...
    if not isinstance(email, str):
        raise ValidationError(
            f"{field_name} must be a string", details={"value": email}
        )
...
def validate_username(username: str, field_name: str = "username") -> None:
    ...
    validate_string_length(
        username, field_name, min_length=3, max_length=MAX_USERNAME_LENGTH
    )
    ...
    if username.startswith(("_", "-")) or username.endswith(("_", "-")):
        raise ValidationError(
            f"{field_name} cannot start or end with underscore or hyphen",
            details={"value": username},
        )
...
def validate_uuid(value: str, field_name: str) -> None:
    ...
    if not isinstance(value, str):
        raise ValidationError(
            f"{field_name} must be a string", details={"value": value}
        )
...
def validate_url(
    url: str, field_name: str = "url", schemes: list[str] | None = None
) -> None:
    """Validate URL format."""
...
def validate_integer(
    ...
    if not isinstance(value, int):
        raise ValidationError(
            f"{field_name} must be an integer", details={"value": value}
        )

    if min_value is not None and value < min_value:
        raise ValidationError(
            f"{field_name} must be at least {min_value}", details={"value": value}
        )

    if max_value is not None and value > max_value:
        raise ValidationError(
            f"{field_name} must be at most {max_value}", details={"value": value}
        )
...
def validate_float(
    ...
    if not isinstance(value, (int, float)):
        raise ValidationError(
            f"{field_name} must be a number", details={"value": value}
        )
    ...
    if item_validator:
        for i, item in enumerate(value):
            try:
                ...
                raise ValidationError(
                    f"Invalid item at index {i}: {e.message}", details={"value": value}
                )
...
def validate_dict(
    ...
    if not isinstance(value, dict):
        raise ValidationError(
            f"{field_name} must be a dictionary", details={"value": value}
        )
...
def validate_datetime(value: Any, field_name: str) -> None:
    ...
    if not isinstance(value, datetime):
        ...
        raise ValidationError(
            f"{field_name} must be a valid datetime", details={"value": value}
        )


def validate_file_extension(
    filename: str, field_name: str = "filename", allowed_types: list[str] | None = None
) -> None:
    """Validate file extension."""
    if not isinstance(filename, str):
        raise ValidationError(
            f"{field_name} must be a string", details={"value": filename}
        )
    ...
    if not extension:
        raise ValidationError(
            f"{field_name} must have a file extension", details={"value": filename}
        )
    ...
    if extension not in all_extensions:
        raise ValidationError(
            f"File extension '{extension}' is not allowed", details={"value": filename}
        )
...
def validate_message_content(content: str, field_name: str = "content") -> None:
    ...
    validate_string_length(
        content, field_name, min_length=1, max_length=MAX_USERNAME_LENGTH
    )
...
def validate_json(value: str, field_name: str) -> None:
    ...
    try:
        ...
        raise ValidationError(
            f"Invalid JSON in {field_name}: {e!s}", details={"value": value}
        )
