"""
NetLink Base Service

Base class for all NetLink services providing common functionality
...
class BaseService(ABC):
    """Base class for all NetLink services."""
    ...
    def __init__(self, service_name: str):
        ...
        # Service metadata
        self.version = "1.0.0"
        self.description = f"NetLink {service_name} service"
    ...
    async def start(self):
        ...
        if self.state != ServiceState.STOPPED:
            self.logger.warning(f"Service {self.service_name} is already running or starting")
            return
    ...
    async def _health_monitoring_loop(self):
        ...
        while self.state == ServiceState.RUNNING:
            try:
                ...
                self.logger.error(f"Health monitoring error for {self.service_name}: {e}")
                await asyncio.sleep(self.health_check_interval)
    ...
    async def _check_dependencies(self):
        ...
        if self.dependencies:
            self.logger.info(f"Service {self.service_name} depends on: {', '.join(self.dependencies)}")
...
__all__ = [
    "BaseService", "ServiceState", "ServiceHealth", "ServiceRegistry", 
