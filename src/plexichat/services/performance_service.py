"""
NetLink Unified Performance Monitoring Service

Centralized performance monitoring service that consolidates all performance
monitoring capabilities across the NetLink system into a single, comprehensive
...
class PerformanceAggregator:
    ...
    def add_custom_metric(self, name: str, value: Any, timestamp: Optional[datetime] = None):
        """Add custom metric."""
    ...
    def get_latest_metrics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            return {
                ...
                "custom": {name: list(metrics)[-1] if metrics else None 
    ...
    def get_historical_data(self, hours: int = 1) -> Dict[str, List[Dict[str, Any]]]:
        ...
        with self.lock:
            return {
                "system": [m.__dict__ for m in self.system_metrics if m.timestamp >= cutoff_time],
                "application": [m.__dict__ for m in self.app_metrics if m.timestamp >= cutoff_time],
                "database": [m.__dict__ for m in self.db_metrics if m.timestamp >= cutoff_time],
                "cluster": [m.__dict__ for m in self.cluster_metrics if m.timestamp >= cutoff_time],
                "ai": [m.__dict__ for m in self.ai_metrics if m.timestamp >= cutoff_time],
                "custom": {
...
class PerformanceService(BaseService):
    ...
    async def _system_monitoring_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                self.performance_logger.record_metric("system_cpu_usage", system_metrics.cpu_usage, "%")
                self.performance_logger.record_metric("system_memory_usage", system_metrics.memory_usage, "%")
                self.performance_logger.record_metric("system_thread_count", system_metrics.thread_count, "count")
    ...
    async def _application_monitoring_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                # Record individual metrics
                self.performance_logger.record_metric("app_active_connections", app_metrics.active_connections, "count")
                self.performance_logger.record_metric("app_request_rate", app_metrics.request_rate, "req/s")
                self.performance_logger.record_metric("app_response_time_avg", app_metrics.response_time_avg, "ms")
                self.performance_logger.record_metric("app_error_rate", app_metrics.error_rate, "%")
    ...
    async def _database_monitoring_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                # Record individual metrics
                self.performance_logger.record_metric("db_connection_pool_size", db_metrics.connection_pool_size, "count")
                self.performance_logger.record_metric("db_query_rate", db_metrics.query_rate, "queries/s")
                self.performance_logger.record_metric("db_avg_query_time", db_metrics.avg_query_time, "ms")
                self.performance_logger.record_metric("db_cache_hit_rate", db_metrics.cache_hit_rate, "%")
        ...
        while self.monitoring_active:
            try:
                ...
                cluster_metrics = ClusterMetrics(
                    ...
                    cluster_load_balance=cluster_data.get("performance_gain_factor", 1.0),
                    inter_node_latency=cluster_data.get("average_response_time_ms", 0.0),
                ...
                # Record individual metrics
                self.performance_logger.record_metric("cluster_total_nodes", cluster_metrics.total_nodes, "count")
                self.performance_logger.record_metric("cluster_active_nodes", cluster_metrics.active_nodes, "count")
                self.performance_logger.record_metric("cluster_cpu_avg", cluster_metrics.cluster_cpu_avg, "%")
                self.performance_logger.record_metric("cluster_memory_avg", cluster_metrics.cluster_memory_avg, "%")
    ...
    async def _ai_monitoring_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                recent_metrics = list(self.ai_analytics.performance_buffer)[-100:]  # Last 100 requests
                ...
                if recent_metrics:
                    ai_metrics = AIMetrics(
                        ...
                        avg_response_time=statistics.mean([m.latency_ms for m in recent_metrics]),
                        model_accuracy=95.0,  # Would be calculated from actual model performance
                        token_usage=sum([getattr(m, 'tokens_used', 0) for m in recent_metrics]),
                        cost_per_request=statistics.mean([getattr(m, 'cost', 0.0) for m in recent_metrics]),
                    ...
                    # Record individual metrics
                    self.performance_logger.record_metric("ai_requests_per_minute", ai_metrics.requests_per_minute, "req/min")
                    self.performance_logger.record_metric("ai_avg_response_time", ai_metrics.avg_response_time, "ms")
                    self.performance_logger.record_metric("ai_error_rate", ai_metrics.error_rate, "%")
                    self.performance_logger.record_metric("ai_token_usage", ai_metrics.token_usage, "tokens")
    ...
    async def _trigger_alert(self, alert_type: str, value: float):
        ...
        alert_data = {
            ...
            "severity": "high" if value > self.baselines.get(alert_type.replace("high_", "").replace("slow_", ""), 0) * 1.5 else "medium"
        }
    ...
    def add_custom_metric(self, name: str, value: Any):
        ...
        self.performance_logger.record_metric(f"custom_{name}", float(value) if isinstance(value, (int, float)) else 0.0, "custom")
...
__all__ = [
    "PerformanceService", "SystemMetrics", "ApplicationMetrics", "DatabaseMetrics",
    "ClusterMetrics", "AIMetrics", "PerformanceAggregator", "get_performance_service"
]
