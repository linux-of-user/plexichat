...
async def performance_page(request: Request):
    ...
    try:
        ...
        if recent_metrics:
            ...
            for model_stats in by_model.values():
                model_stats["success_rate"] = (
                    model_stats["successful"] / model_stats["requests"]
                )
                model_stats["avg_latency"] = sum(model_stats["latencies"]) / len(
                    model_stats["latencies"]
                )

            for provider_stats in by_provider.values():
                provider_stats["success_rate"] = (
                    provider_stats["successful"] / provider_stats["requests"]
                )
                provider_stats["avg_latency"] = sum(provider_stats["latencies"]) / len(
                    provider_stats["latencies"]
                )
        else:
            total_requests = 0
            success_rate = 0.0
        ...
        return templates.TemplateResponse(
            "performance.html", {"request": request, "performance": performance_data}
        )
...
async def costs_page(request: Request):
    ...
    try:
        ...
        return templates.TemplateResponse(
            "costs.html", {"request": request, "costs": cost_data}
        )
...
async def get_metrics_api():
    ...
    try:
        ...
        if recent_metrics:
            ...
            avg_latency = sum(m.latency_ms for m in recent_metrics) / len(
                recent_metrics
            )
        else:
            success_rate = 0.0
        ...
        return {
            ...
            "costs": {"recent_total": total_cost, "recent_requests": len(recent_costs)},
        }
