...
class MultilingualChatbot:
    ...
    async def process_message(self, 
                             ...
                             metadata: Optional[Dict[str, Any]] = None) -> ChatbotResponse:
        ...
        try:
            ...
            if detected_language != context.language and detected_language != "en":
                working_message = await self._translate_text(message, detected_language, "en")
            ...
            response_content = await self._generate_response(context, working_message, intent, entities)
            
            # Translate response to user's language if needed
            final_response = response_content
            was_translated = False
            if context.language != "en":
                final_response = await self._translate_text(response_content, "en", context.language)
                was_translated = True
    ...
    async def _generate_response(self, 
        ...
        if self.ai_provider:
            try:
                ...
                return ai_response.get("content", "I'm sorry, I couldn't generate a response.")
            except Exception as e:
...
"""
    ...
    async def _generate_welcome_message(self, context: ConversationContext, personality: str) -> str:
        """Generate welcome message."""
        personality_config = self.personalities.get(personality, self.personalities["default"])
    ...
    def _update_statistics(self, context: ConversationContext, response: ChatbotResponse):
        ...
        new_avg = ((current_avg * (total_messages - 1)) + response.response_time_ms) / total_messages
