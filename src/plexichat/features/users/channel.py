...
class Channel(SQLModel, table=True):
    ...
    video_quality_mode: Optional[VideoQualityMode] = Field(default=VideoQualityMode.AUTO)
    ...
    default_sort_order: Optional[SortOrderType] = Field(default=SortOrderType.LATEST_ACTIVITY)
    
    # Permissions
    permission_overwrites: List[Dict[str, Any]] = Field(default=[], sa_column=Column(JSON))
...
class ChannelPermissionOverwrite(SQLModel, table=True):
    ...
    # Indexes
    __table_args__ = (
        Index('idx_permission_channel_target', 'channel_id', 'target_id', 'target_type', unique=True),
    )
...
class ThreadMember(SQLModel, table=True):
    ...
    join_timestamp: datetime = Field(default_factory=datetime.utcnow, sa_column=Column(DateTime))
