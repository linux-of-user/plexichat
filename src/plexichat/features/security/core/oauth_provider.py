...
class OAuthProvider:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.supported_scopes = {"openid", "profile", "email", "read", "write", "admin"}
    ...
    async def delete_client(self, client_id: str) -> bool:
        ...
        if client_id in self.clients:
            ...
            refresh_tokens_to_remove = [
                token
                for token, token_obj in self.refresh_tokens.items()
                if token_obj.client_id == client_id
            ]
    ...
    async def cleanup_expired_tokens(self):
        ...
        expired_codes = [
            code
            for code, code_obj in self.authorization_codes.items()
