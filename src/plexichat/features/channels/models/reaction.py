...
class Reaction(SQLModel, table=True):
    ...
    user_id: str = Field(
        ...
        index=True,
        description="User who added this reaction"
    )
    ...
    emoji_id: Optional[str] = Field(
        default=None,
        description="Custom emoji ID (if custom emoji)"
    )
    
    emoji_name: Optional[str] = Field(
        default=None,
        max_length=100,
        description="Custom emoji name (if custom emoji)"
    )
    
    emoji_animated: bool = Field(
        default=False,
        description="Whether custom emoji is animated"
    )
