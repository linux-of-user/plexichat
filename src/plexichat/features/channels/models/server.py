...
class Server(SQLModel, table=True):
    ...
    name: str = Field()
        max_length=100, index=True, description="Server name (2-100 characters)"
    )

    owner_id: str = Field()
        foreign_key="users.id", index=True, description="User ID of the server owner"
    )

    # Visual customization
    icon_url: Optional[str] = Field()
        default=None, max_length=500, description="Server icon URL"
    )

    banner_url: Optional[str] = Field()
        default=None, max_length=500, description="Server banner URL"
    )

    # Server settings
    region: Optional[str] = Field()
        default="us-east", max_length=50, index=True, description="Server voice region"
    )
    ...
    description: Optional[str] = Field()
        default=None, sa_column=Column(Text), description="Server description"
    )

    splash_url: Optional[str] = Field()
        default=None, max_length=500, description="Server invite splash image URL"
    )

    discovery_splash_url: Optional[str] = Field()
        default=None, max_length=500, description="Server discovery splash image URL"
    )

    # Server features and limits
    max_members: int = Field()
        default=100000, ge=1, description="Maximum number of members"
    )

    max_presences: Optional[int] = Field()
        default=None, ge=1, description="Maximum number of online members"
    )

    max_video_channel_users: Optional[int] = Field()
        default=25, ge=1, description="Maximum users in video channels"
    )
    ...
    widget_enabled: bool = Field()
        default=False, description="Whether server widget is enabled"
    )
    ...
    # Moderation settings
    mfa_level: int = Field()
        default=0, ge=0, le=1, description="MFA level required for moderation actions"
    )
    ...
    premium_subscription_count: int = Field()
        default=0, ge=0, description="Number of premium subscriptions"
    )
    ...
    afk_channel_id: Optional[str] = Field()
        default=None, foreign_key="channels.channel_id", description="AFK voice channel"
    )

    afk_timeout: int = Field()
        default=300, ge=60, le=3600, description="AFK timeout in seconds"
    )
    ...
    features: List[str] = Field()
        default_factory=list, sa_column=Column(JSON), description="Server features list"
    )

    preferred_locale: str = Field()
        default="en-US", max_length=10, description="Preferred locale for the server"
    )
