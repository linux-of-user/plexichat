...
class ServerService:
    ...
    async def create_server(self, owner_id: str, server_data: Dict[str, Any]) -> Server:
        ...
        try:
            ...
            server = await self.server_repository.create_server_with_defaults(server_data)
    ...
    async def join_server(self, server_id: str, user_id: str, invite_code: str = None) -> bool:
        """
        Join a server.
    ...
    async def get_server_members(self, server_id: str, user_id: str) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if not await self._can_user_view_server(server_id, user_id):
                raise PermissionError("User does not have permission to view server members")
    ...
    async def kick_member(self, server_id: str, user_id: str, target_user_id: str) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"User {target_user_id} kicked from server {server_id} by {user_id}")
            return True
    ...
    async def _can_user_view_server(self, server_id: str, user_id: str) -> bool:
        ...
        return (await self.server_repository.is_member(server_id, user_id) or 
                await self.server_repository.is_owner(server_id, user_id))
    ...
    async def _validate_server_creation(self, owner_id: str, server_data: Dict[str, Any]) -> None:
