...
class PermissionService:
    ...
    def calculate_base_permissions(member: ServerMember, roles: List[Role]) -> int:
        ...
        if permissions & Permissions.ADMINISTRATOR:
            return int(Permissions.ADMINISTRATOR) | 0x7FFFFFFFFFFFFFFF  # All permissions
    ...
    def calculate_overwrites(
        ...
        for role in member_roles:
            role_overwrite = next(
                (ow for ow in overwrites 
                 if ow.target_id == role.role_id and ow.target_type == OverwriteType.ROLE),
                None
            )
            if role_overwrite:
        ...
        member_overwrite = next(
            (ow for ow in overwrites 
             if ow.target_id == member.user_id and ow.target_type == OverwriteType.MEMBER),
            None
        )
        if member_overwrite:
    ...
    def has_permission(
        ...
        """
        final_permissions = PermissionService.calculate_permissions(member, roles, overwrites)
