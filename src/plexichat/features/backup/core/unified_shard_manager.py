...
class UnifiedShardManager:
    ...
    async def _initialize_database(self) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            # Create indexes
            await db.execute("CREATE INDEX IF NOT EXISTS idx_shard_backup_id ON shard_metadata(backup_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_shard_state ON shard_metadata(state)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_shard_created_at ON shard_metadata(created_at)")
    ...
    async def create_shards(self, data: bytes, operation) -> List['UnifiedShard']:
        ...
        logger.info(f"Creating shards for backup {operation.backup_id}, data size: {len(data)} bytes")
        
        # Calculate optimal shard configuration
        shard_config = self._calculate_shard_configuration(len(data), operation.redundancy_factor)
        ...
        rs_encoder = self._create_reed_solomon_encoder(
            shard_config["data_shards"], 
            shard_config["parity_shards"]
        )
    ...
    def _calculate_shard_configuration(self, data_size: int, redundancy_factor: int) -> Dict[str, int]:
        """Calculate optimal shard configuration."""
    ...
    async def _encrypt_shard_data(self, data: bytes, operation) -> Tuple[bytes, Dict[str, Any]]:
    ...
    async def _load_shard_metadata(self, shard_id: str) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                "SELECT * FROM shard_metadata WHERE shard_id = ?",
                (shard_id,)
            ) as cursor:
    ...
    async def _reconstruct_shard_data(self, metadata: ShardMetadata) -> Optional[bytes]:
        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                """, (metadata.backup_id, metadata.shard_id)) as cursor:
                    async for row in cursor:
            ...
            if len(backup_shards) < metadata.data_shards:
                logger.warning(f"Not enough healthy shards for reconstruction: {len(backup_shards)}/{metadata.data_shards}")
                return None
