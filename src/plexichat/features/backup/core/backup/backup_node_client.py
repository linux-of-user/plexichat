...
class BackupNodeClient:
    ...
    async def upload_shard(self, shard_id: str, shard_data: bytes) -> bool:
        ...
        try:
            ...
            data.add_field('shard_data', shard_data, content_type='application/octet-stream')
            
            async with self.session.post(f"{self.base_url}/shards", data=data) as response:
                if response.status == 201:
                    logger.info(f"Successfully uploaded shard {shard_id} to node {self.node_info.node_id}")
                    return True
                else:
                    logger.error(f"Failed to upload shard {shard_id}: HTTP {response.status}")
                    return False
        except Exception as e:
            logger.error(f"Error uploading shard {shard_id} to node {self.node_info.node_id}: {e}")
            return False
    
    async def download_shard(self, shard_id: str) -> Optional[bytes]:
        """Download a shard from the backup node."""
        try:
            async with self.session.get(f"{self.base_url}/shards/{shard_id}") as response:
                if response.status == 200:
                    shard_data = await response.read()
                    logger.info(f"Successfully downloaded shard {shard_id} from node {self.node_info.node_id}")
                    return shard_data
                else:
                    logger.error(f"Failed to download shard {shard_id}: HTTP {response.status}")
                    return None
        except Exception as e:
            logger.error(f"Error downloading shard {shard_id} from node {self.node_info.node_id}: {e}")
            return None
    
    async def verify_shard(self, shard_id: str, expected_checksum: str) -> bool:
        """Verify a shard on the backup node."""
        try:
            async with self.session.get(f"{self.base_url}/shards/{shard_id}/verify") as response:
                if response.status == 200:
                    data = await response.json()
            ...
            logger.error(f"Error verifying shard {shard_id} on node {self.node_info.node_id}: {e}")
            return False
                ...
                if response.status == 200:
                    ...
                    for shard_data in data.get("shards", []):
                        shard = ShardInfo(
                            ...
                            last_verified=datetime.fromisoformat(shard_data["last_verified"])
                        )
                        shards.append(shard)
                    return shards
                else:
    ...
    async def delete_shard(self, shard_id: str) -> bool:
        ...
        try:
            async with self.session.delete(f"{self.base_url}/shards/{shard_id}") as response:
                if response.status == 204:
                    logger.info(f"Successfully deleted shard {shard_id} from node {self.node_info.node_id}")
                    return True
                else:
                    logger.error(f"Failed to delete shard {shard_id}: HTTP {response.status}")
                    return False
        except Exception as e:
            logger.error(f"Error deleting shard {shard_id} from node {self.node_info.node_id}: {e}")
            return False
...
class BackupNodeManager:
    ...
    async def register_node(self, node_info: BackupNodeInfo, api_key: str = None):
        ...
        logger.info(f"Registered backup node {node_info.node_id} at {node_info.ip_address}:{node_info.port}")
    ...
    async def select_nodes_for_shard(self, redundancy_factor: int) -> List[str]:
        ...
        if len(healthy_nodes) < redundancy_factor:
            logger.warning(f"Only {len(healthy_nodes)} healthy nodes available, need {redundancy_factor}")
            redundancy_factor = len(healthy_nodes)
        
        # Simple selection based on capacity
        selected_nodes = sorted(
            healthy_nodes,
            key=lambda n: n.used_bytes / n.capacity_bytes
        )[:redundancy_factor]
    ...
    async def upload_shard_to_nodes(
        ...
        logger.info(f"Uploaded shard {shard_id} to {len(successful_uploads)}/{len(node_ids)} nodes")
    ...
    async def download_shard_from_nodes(
        ...
        for node_id in node_ids:
            if node_id in self.clients and self.nodes[node_id].status == NodeStatus.ONLINE:
                client = self.clients[node_id]
                async with client:
                    shard_data = await client.download_shard(shard_id)
                    if shard_data:
                        logger.info(f"Successfully downloaded shard {shard_id} from node {node_id}")
                        return shard_data
