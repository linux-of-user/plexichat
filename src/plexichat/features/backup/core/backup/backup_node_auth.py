...
class BackupNodeAuthManager:
    ...
    async def create_node_credentials(
        ...
        # Log event
        await self._log_auth_event(node_id, "credentials_created", {
            "permission_level": permission_level.value,
            "expires_at": expires_at.isoformat()
        })
        
        logger.info(f"Created credentials for node {node_id} with {permission_level.value} permissions")
    ...
    async def authenticate_node(self, node_id: str, api_key: str) -> bool:
        ...
        if node_id not in self.node_credentials:
            await self._log_auth_event(node_id, "auth_failed", {"reason": "node_not_found"})
            return False
        ...
        if credentials.status != APIKeyStatus.ACTIVE:
            await self._log_auth_event(node_id, "auth_failed", {"reason": "inactive_credentials"})
            return False
        ...
        if credentials.expires_at and credentials.expires_at <= datetime.now(timezone.utc):
            ...
            await self._save_credentials_to_database(credentials)
            await self._log_auth_event(node_id, "auth_failed", {"reason": "expired_credentials"})
            return False
        
        # Verify API key
        api_key_hash = self._hash_api_key(api_key)
        if api_key_hash != credentials.api_key_hash:
            await self._log_auth_event(node_id, "auth_failed", {"reason": "invalid_key"})
            return False
    ...
    async def rotate_credentials(self, node_id: str) -> Tuple[str, str]:
        ...
        new_api_key, new_api_key_hash = await self.create_node_credentials(
            node_id,
            old_credentials.permission_level
        )
    ...
    async def _log_auth_event(self, node_id: str, event_type: str, details: Dict[str, Any]):
        ...
        for credentials in self.node_credentials.values():
            if credentials.expires_at:
                if credentials.expires_at <= now:
                    ...
                    credentials.status = APIKeyStatus.EXPIRED
                    await self._save_credentials_to_database(credentials)
                    logger.warning(f"API key for node {credentials.node_id} has expired")
                elif credentials.expires_at <= warning_threshold:
                    # Key expiring soon
                    logger.info(f"API key for node {credentials.node_id} expires in {(credentials.expires_at - now).days} days")
