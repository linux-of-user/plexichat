...
Command-line interface for managing NetLink's advanced clustering system with:
...
class ClusterCLI:
    ...
    def create_parser(self) -> argparse.ArgumentParser:
        ...
        parser = argparse.ArgumentParser(
            prog='netlink cluster',
            description='NetLink Advanced Cluster Management'
        )
    ...
    async def run(self, args=None):
        ...
        if args is None:
            import sys
            args = parser.parse_args(sys.argv[2:])  # Skip 'netlink cluster'
        else:
