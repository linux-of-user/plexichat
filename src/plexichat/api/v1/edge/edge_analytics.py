...
async def get_edge_overview(
    ...
    try:
        ...
        healthy_nodes = sum(1 for node in manager.edge_nodes.values() if node.is_healthy)
        ...
        avg_cpu_usage = sum(node.cpu_usage_percent for node in manager.edge_nodes.values()) / max(total_nodes, 1)
        avg_memory_usage = sum(node.memory_usage_percent for node in manager.edge_nodes.values()) / max(total_nodes, 1)
        avg_storage_usage = sum(node.storage_usage_percent for node in manager.edge_nodes.values()) / max(total_nodes, 1)
        ...
        # AI/GPU capabilities
        gpu_enabled_nodes = sum(1 for node in manager.edge_nodes.values() 
                               if getattr(node, 'gpu_available', False))
        ai_enabled_nodes = sum(1 for node in manager.edge_nodes.values() 
...
async def get_performance_analytics(
    ...
    node_ids: Optional[List[str]] = Query(None, description="Specific node IDs to analyze"),
    ...
    try:
        ...
        if node_ids:
            target_nodes = {nid: node for nid, node in manager.edge_nodes.items() if nid in node_ids}
        
        # Default metrics if not specified
        if not metrics:
            metrics = ["cpu_usage", "memory_usage", "storage_usage", "network_usage", "response_time"]
        ...
        for node_id, node in target_nodes.items():
            ...
            if "response_time" in metrics:
                node_metrics["avg_response_time_ms"] = getattr(node, 'avg_response_time_ms', 0)
            
            # Connection metrics
            node_metrics["current_connections"] = node.current_connections
            node_metrics["max_connections"] = node.max_connections
            node_metrics["connection_utilization"] = (node.current_connections / max(node.max_connections, 1)) * 100
        ...
        if performance_data:
            cpu_values = [data.get("cpu_usage_percent", 0) for data in performance_data.values()]
            memory_values = [data.get("memory_usage_percent", 0) for data in performance_data.values()]
            ...
            aggregates = {
                ...
                "total_connections": sum(data.get("current_connections", 0) for data in performance_data.values())
            }
        else:
            aggregates = {}
...
async def get_geographic_analytics(
    ...
    try:
        ...
        coverage_metrics = {
            ...
            "geographic_coverage": len(geographic_data) / max(len(manager.edge_nodes), 1) * 100,
        ...
        return {
            ...
            "data": {
                ...
                "map_center": {
                    "latitude": sum(node["latitude"] for node in geographic_data) / max(len(geographic_data), 1),
                    "longitude": sum(node["longitude"] for node in geographic_data) / max(len(geographic_data), 1)
                } if geographic_data else None
            },
            "timestamp": datetime.now(timezone.utc).isoformat()
        }
...
async def get_predictive_analytics(
    ...
    try:
        ...
        for node_id, node in manager.edge_nodes.items():
            ...
            connection_trend = 2 if current_connections > node.max_connections * 0.8 else -1
            
            predictions[node_id] = {
                "predicted_cpu_usage": min(100, max(0, current_cpu + cpu_trend * 24)),
                "predicted_memory_usage": min(100, max(0, current_memory + memory_trend * 24)),
                "predicted_connections": max(0, current_connections + connection_trend * 24),
                "scaling_recommendation": "scale_up" if current_cpu > 80 or current_memory > 85 else "maintain",
                "risk_level": "high" if current_cpu > 85 or current_memory > 90 else "low",
                "predicted_load_level": "high" if current_cpu + cpu_trend * 24 > 80 else "normal"
            }
        
        # System-wide predictions
        total_nodes = len(manager.edge_nodes)
        high_risk_nodes = sum(1 for p in predictions.values() if p["risk_level"] == "high")
        scale_up_recommendations = sum(1 for p in predictions.values() if p["scaling_recommendation"] == "scale_up")
        
        system_predictions = {
            "capacity_utilization_trend": "increasing" if scale_up_recommendations > total_nodes * 0.3 else "stable",
            "recommended_new_nodes": max(0, scale_up_recommendations - 2),
            "system_health_forecast": "degraded" if high_risk_nodes > total_nodes * 0.2 else "healthy",
            "peak_load_prediction": datetime.now(timezone.utc) + timedelta(hours=6)  # Simplified
        }
        ...
        return {
            ...
            "data": {
                ...
                "recommendations": {
                    "immediate_actions": [
                        f"Monitor {high_risk_nodes} high-risk nodes" if high_risk_nodes > 0 else "System operating normally",
                        f"Consider scaling up {scale_up_recommendations} nodes" if scale_up_recommendations > 0 else "No scaling needed"
                    ],
                    "capacity_planning": f"Add {system_predictions['recommended_new_nodes']} nodes in next 30 days" if system_predictions['recommended_new_nodes'] > 0 else "Current capacity sufficient"
                }
            },
            "timestamp": datetime.now(timezone.utc).isoformat()
        }
