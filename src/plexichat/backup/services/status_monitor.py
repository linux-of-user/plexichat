"""
NetLink Backup Status Monitor Service
...
class BackupStatusMonitor:
    ...
    async def get_real_time_status(self, force_refresh: bool = False) -> BackupCoverageReport:
        ...
        try:
            ...
            report = await self._calculate_coverage_report(device_statuses, shard_statuses)
    ...
    async def _get_shard_statuses(self) -> List[ShardStatus]:
        ...
        try:
            ...
            for i in range(20):  # Mock 20 shards
                online_assignments = 3 if i < 15 else 1  # Most shards have good redundancy
                ...
                shard_status = ShardStatus(
                    ...
                    redundancy_level=self._calculate_redundancy_level(online_assignments),
    ...
    async def _calculate_coverage_report(self, 
                                       device_statuses: List[DeviceStatus], 
                                       shard_statuses: List[ShardStatus]) -> BackupCoverageReport:
        ...
        device_utilization_average = sum(d.storage_utilization_percent for d in device_statuses) / max(total_devices, 1)
        
        # Shard metrics
        total_shards = len(shard_statuses)
        available_shards = sum(1 for s in shard_statuses if s.availability_percentage > 0)
        ...
        for backup_id, shards in backup_groups.items():
            available_shard_count = sum(1 for s in shards if s.availability_percentage > 0)
        ...
        critical_issues, warnings, recommendations = self._analyze_issues_and_recommendations(
    ...
    def _assess_health_status(self,
                            ...
                            online_devices: int,
                            total_devices: int,
                            shard_statuses: List[ShardStatus]) -> BackupHealthStatus:
        ...
        if device_online_percentage < (100 - self.health_thresholds["device_offline_critical"]):
            return BackupHealthStatus.CRITICAL

        # Count critical shards
        critical_shards = sum(1 for s in shard_statuses if s.redundancy_level == RedundancyLevel.CRITICAL)
        ...
        if device_online_percentage < (100 - self.health_thresholds["device_offline_warning"]):
            return BackupHealthStatus.WARNING
    ...
    def _analyze_issues_and_recommendations(self,
                                          device_statuses: List[DeviceStatus],
                                          shard_statuses: List[ShardStatus],
                                          availability: float) -> Tuple[List[str], List[str], List[str]]:
        ...
        if len(offline_devices) > len(device_statuses) * 0.5:
            critical_issues.append(f"More than 50% of devices are offline ({len(offline_devices)}/{len(device_statuses)})")
        elif len(offline_devices) > 0:
            warnings.append(f"{len(offline_devices)} devices are currently offline")

        # Shard analysis
        critical_shards = [s for s in shard_statuses if s.redundancy_level == RedundancyLevel.CRITICAL]
        if critical_shards:
            critical_issues.append(f"{len(critical_shards)} shards have critical redundancy (only 1 copy)")

        low_redundancy_shards = [s for s in shard_statuses if s.redundancy_level == RedundancyLevel.LOW]
        if low_redundancy_shards:
            warnings.append(f"{len(low_redundancy_shards)} shards have low redundancy (only 2 copies)")

        # Recommendations
        if offline_devices:
            recommendations.append("Investigate offline devices and restore connectivity")

        if critical_shards:
            recommendations.append("Immediately replicate critical shards to additional devices")

        if availability < 90.0:
            recommendations.append("Consider adding more backup devices to improve redundancy")

        high_utilization_devices = [d for d in device_statuses if d.storage_utilization_percent > 85.0]
        if high_utilization_devices:
            recommendations.append(f"Monitor storage on {len(high_utilization_devices)} devices with high utilization")
    ...
    def _get_error_report(self, error_message: str) -> BackupCoverageReport:
        ...
        return BackupCoverageReport(
            ...
            warnings=[],
            recommendations=["Check backup system connectivity and restart monitoring service"]
        )
    ...
    async def _continuous_monitoring_task(self):
        ...
        while self.monitoring_active:
            try:
                ...
                # Log status summary
                logger.info(f"Backup Status: {status.health_status.value} - "
