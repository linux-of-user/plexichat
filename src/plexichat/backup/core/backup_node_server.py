...
class BackupNodeServer:
    ...
    def __init__(self, config: BackupNodeConfig):
        self.config = config
        self.app = FastAPI(title="NetLink Backup Node", version="3.0.0")
    ...
    async def start(self):
        ...
        logger.info(f"🚀 Starting NetLink Backup Node {self.config.node_id}")
    ...
    async def stop(self):
        ...
        logger.info(f"🛑 Stopping NetLink Backup Node {self.config.node_id}")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="NetLink Backup Node Server")
