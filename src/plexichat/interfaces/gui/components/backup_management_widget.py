...
# Only set GUI_AVAILABLE and MATPLOTLIB_AVAILABLE once at the top, do not reassign below
GUI_AVAILABLE = False
...
# Fallback definition so the name always exists
class EnhancedBackupManagementWidget:
    def __init__(self, *args, **kwargs):
        raise ImportError("tkinter/ttk GUI libraries are not available on this system.")
    ...
    """Enhanced GUI widget for comprehensive backup management.
        ...
        def setup_gui(self):
            ...
            try:
                ...
                # Title
                title_label = ttk.Label(header_frame, text=" Enhanced Backup Management", font=("Arial", 18, "bold"))
                title_label.pack(side=tk.LEFT)

                # Status indicator
                self.status_indicator = ttk.Label(header_frame, text=" Initializing...", foreground="orange", font=("Arial", 10, "bold"))
        ...
        def setup_backups_tab(self):
            ...
            try:
                ...
                ttk.Button(control_frame, text=" Create Backup", command=self.create_backup_dialog).pack(side=tk.LEFT, padx=(0, 10))

                ttk.Button(control_frame, text=" Refresh", command=self.refresh_backups).pack(side=tk.LEFT, padx=(0, 10))

                ttk.Button(control_frame, text=" Delete Backup", command=self.delete_backup_dialog).pack(side=tk.LEFT, padx=(0, 10))

                ttk.Button(control_frame, text=" Export Report", command=self.export_backup_report).pack(side=tk.LEFT)
        ...
        def setup_nodes_tab(self):
            ...
            try:
                ...
                ttk.Button(control_frame, text=" Add Node", command=self.add_backup_node_dialog).pack(side=tk.LEFT, padx=(0, 10))

                ttk.Button(control_frame, text=" Refresh", command=self.refresh_nodes).pack(side=tk.LEFT, padx=(0, 10))

                ttk.Button(control_frame, text=" Node Health", command=self.check_node_health).pack(side=tk.LEFT)
        ...
        def setup_recovery_tab(self):
            ...
            try:
                ...
                ttk.Button(recovery_controls_frame, text=" Browse Backups", command=self.browse_backups_for_recovery).pack(side=tk.LEFT, padx=(0, 10))

                ttk.Button(recovery_controls_frame, text=" Restore Backup", command=self.restore_backup_dialog).pack(side=tk.LEFT, padx=(0, 10))

                ttk.Button(recovery_controls_frame, text=" Verify Integrity", command=self.verify_backup_integrity).pack(side=tk.LEFT)
        ...
        def setup_settings_tab(self):
            ...
            try:
                ...
                if self.auto_backup_var:
                    auto_backup_cb = ttk.Checkbutton(backup_settings_frame, text="Enable Automatic Backups", variable=self.auto_backup_var)
                    auto_backup_cb.pack(anchor=tk.W, pady=2)

                # Encryption checkbox
                if self.encryption_enabled_var:
                    encryption_cb = ttk.Checkbutton(backup_settings_frame, text="Enable Quantum Encryption", variable=self.encryption_enabled_var)
        ...
        def setup_control_panel(self):
            ...
            try:
                ...
                # Export button
                ttk.Button(control_frame, text=" Export Data", command=self.export_backup_data).pack(side=tk.LEFT)
        ...
        def create_backup_dialog(self):
            ...
            try:
                ...
                if source_path:
                    ...
                    backup_type = simpledialog.askstring("Backup Type", "Enter backup type (FULL/INCREMENTAL):", initialvalue="INCREMENTAL")
                    if backup_type:
        ...
        def _create_backup_async(self, source_path, backup_type):
            ...
            try:
                ...
                backup_id = loop.run_until_complete(backup_manager.create_backup(source_path, backup_type, 'GOVERNMENT'))
