...
ai_features_bp = Blueprint(
    "ai_features_admin", __name__, url_prefix="/admin/ai-features"
)
...
def _unauthorized_response(message: str = "Admin privileges required"):
    ...
    if _is_api_request():
        return (
            jsonify({"success": False, "error": "forbidden", "message": message}),
            403,
        )
    ...
    try:
        return (
            render_template(
                "admin/ai_features_management.html", stats={}, health={}, config={}
            ),
            403,
        )
    except Exception:
...
def require_admin(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.exception(
                f"Unhandled exception in admin route '{func.__name__}': {e}"
            )
...
def api_summarize(current_user: dict[str, Any] | None = None):
    ...
    try:
        ...
        user_id = data.get(
            "user_id", current_user.get("id") if current_user else "admin"
        )
        service = get_ai_features_service()
        result = asyncio.run(
...
def api_suggest_content(current_user: dict[str, Any] | None = None):
    ...
    try:
        ...
        suggestions = asyncio.run(
            service.generate_content_suggestions(
                context=context,
    ...
    try:
        ...
        result = asyncio.run(
            service.analyze_sentiment(
                text=text, user_id=user_id, include_emotions=include_emotions
            )
        )
        return jsonify(
            {
...
def api_add_to_index(current_user: dict[str, Any] | None = None):
    ...
    try:
        ...
        service = get_ai_features_service()
        success = asyncio.run(
            service.add_to_semantic_index(
                content_id=content_id, content=content, metadata=metadata
            )
        )
        return jsonify(
            {
