...
def get_performance_service():
    class DummyPerformanceService:
        ...
        def get_historical_metrics(self, *a, **k):
            return []

        def _calculate_trends(self, *a, **k):
            return []
    ...
    try:
        ...
        try:
            logger.log_performance(
                "performance_dashboard.render", elapsed, user_id=current_user.get("id")
            )
        except Exception:
...
async def metrics_page(
    ...
    try:
        ...
        current_metrics = performance_service.get_current_metrics()
        historical_metrics = performance_service.get_historical_metrics(
            24
        )  # Last 24 hours
        ...
        try:
            logger.log_performance(
                "performance_metrics.render", elapsed, user_id=current_user.get("id")
            )
        except Exception:
            try:
                logger.performance(
...
async def alerts_page(request: Request, current_user: dict = Depends(require_admin)):
...
async def health_page(request: Request, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        component_health = {
            ...
            "application": _calculate_component_health(
                current_metrics.get("application")
            ),
        ...
        try:
            logger.log_performance(
                "performance_health.render", elapsed, user_id=current_user.get("id")
            )
        except Exception:
            try:
                logger.performance(
...
async def analytics_page(
    ...
    try:
        ...
        # Get analytics data
        historical_data = performance_service.get_historical_metrics(
            7 * 24
        )  # Last 7 days
        ...
        try:
            logger.log_performance(
                "performance_analytics.render", elapsed, user_id=current_user.get("id")
            )
        except Exception:
            try:
                logger.performance(
                    "performance_analytics.render",
                    elapsed,
                    user_id=current_user.get("id"),
                )
            except Exception:
...
def _calculate_component_health(component_metrics):
    ...
    return {"score": 85, "status": "healthy", "issues": []}
