...
class AdminManagerWrapper:
    ...
    async def authenticate(
        ...
        """
        try:
            result = await self.auth_manager.authenticate_user(
    ...
    def create_admin(
        self, username: str, email: str, password: str, role: str = "admin"
    ) -> bool:
        """
        ...
        try:
            ...
            success, issues = self.auth_manager.register_user(
                username, password, permissions=set(), roles=None
            )
            if not success:
        ...
        try:
            ...
            if by_admin:
                try:
                    ...
                    logger.error(
                        f"Failed to reset password by admin for {username}: {e}"
                    )
                    return False
            ...
            if current_password:
                try:
                    ...
                    logger.debug(
                        "Password manager did not support verification; denying self-reset for safety"
                    )
                    return False
...
def _append_audit_log(
    action: str, admin: dict[str, Any], details: dict[str, Any] = None
):
    entry = {
    ...
    if enhanced_logger:
        try:
            enhanced_logger.info(
                f"AUDIT: {action}",
                extra={"category": LogCategory.AUDIT, "metadata": entry},
            )
        except Exception:
            pass
...
def format_plugin_info(info):
    try:
        ...
        return {
            ...
            "path": str(info.path),
            "status": (
                info.status.name if hasattr(info.status, "name") else str(info.status)
            ),
...
async def admin_dashboard(request: Request):
    ...
    try:
        ...
        try:
            ...
            cluster_status = (
                get_cluster_status()
                if callable(get_cluster_status)
        ...
        logger.error(
            f"Error rendering admin dashboard for {admin['username'] if admin else 'unknown'}: {e}"
        )
        # Fallback simple HTML
        return HTMLResponse(
            f"<h1>Admin Dashboard</h1><p>Welcome, {admin['username']}!</p>"
        )
...
async def admin_login_page(request: Request):
    ...
    try:
        ...
        return HTMLResponse(
            "<h1>Admin Login</h1><form method='post'><input name='username' placeholder='Username'><input name='password' type='password' placeholder='Password'><button>Login</button></form>"
        )
...
async def admin_login(
    ...
    try:
        ...
        token = await admin_manager.authenticate(
...
@rate_limit(
    ...
    key_func=lambda request: f"admin_logout:{get_client_ip(request)}",
)
...
@rate_limit(
    requests_per_minute=30,
    key_func=lambda request: f"admin_users:{get_client_ip(request)}",
)
async def admin_users(request: Request):
    ...
    try:
        ...
        logger.info(
            f"Admin user management page accessed by {admin['username']} from {get_client_ip(request)}"
        )
        context = {"request": request, "admin": admin, "admins": admins}
...
@rate_limit(
    ...
    key_func=lambda request: f"admin_create_user:{get_client_ip(request)}",
)
@protect_from_replay()
async def create_admin_user(
    ...
    if not admin_manager.has_permission(admin["username"], "user_management"):
        logger.warning(
            f"Permission denied: {admin['username']} tried to create admin user {username} from {get_client_ip(request)}"
        )
        raise HTTPException(status_code=403, detail="Insufficient permissions")
    try:
        success = admin_manager.create_admin(
            username, email, admin_create.password, admin_create.role
        )
        if success:
            logger.info(
                f"Admin user {username} created by {admin['username']} from {get_client_ip(request)}"
            )
            _append_audit_log("create_admin_user", admin, {"created": username})
            return JSONResponse({"success": True, "message": "Admin user created"})
        else:
            logger.warning(
                f"Failed to create admin user {username} by {admin['username']} from {get_client_ip(request)}"
            )
            raise HTTPException(status_code=400, detail="Failed to create admin user")
    except Exception as e:
        logger.error(
            f"Error creating admin user {username} by {admin['username']}: {e}"
        )
...
@rate_limit(
    ...
    key_func=lambda request: f"admin_reset_password:{get_client_ip(request)}",
)
@protect_from_replay()
async def admin_reset_password(
    ...
    if not admin_manager.has_permission(admin["username"], "user_management"):
        logger.warning(
            f"Permission denied: {admin['username']} tried to reset password for {reset_request.username} from {get_client_ip(request)}"
        )
        raise HTTPException(status_code=403, detail="Insufficient permissions")
    if reset_request.username == "admin" and admin["username"] != "admin":
        logger.warning(
            f"Permission denied: {admin['username']} tried to reset super_admin password from {get_client_ip(request)}"
        )
        raise HTTPException(
            status_code=403, detail="Only super_admin can reset their own password"
        )
    success = admin_manager.reset_password(
        reset_request.username, reset_request.new_password, by_admin=admin["username"]
    )
    if success:
        logger.info(
            f"Password for {reset_request.username} reset by {admin['username']} from {get_client_ip(request)}"
        )
        _append_audit_log("reset_password", admin, {"target": reset_request.username})
        return JSONResponse(
            {"success": True, "message": f"Password for {reset_request.username} reset"}
        )
    else:
        logger.warning(
            f"Failed password reset for {reset_request.username} by {admin['username']} from {get_client_ip(request)}"
        )
...
@rate_limit(
    ...
    key_func=lambda request: f"self_reset_password:{get_client_ip(request)}",
)
@protect_from_replay()
async def self_reset_password(
    ...
    if reset_request.username != admin["username"]:
        logger.warning(
            f"Permission denied: {admin['username']} tried to self-reset password for {reset_request.username} from {get_client_ip(request)}"
        )
        raise HTTPException(
            status_code=403, detail="Can only reset your own password here"
        )
    if not reset_request.current_password:
        logger.warning(
            f"Password reset denied: {admin['username']} did not provide current password from {get_client_ip(request)}"
        )
    ...
    success = admin_manager.reset_password(
        ...
        current_password=reset_request.current_password,
    )
    if success:
        logger.info(
            f"Admin {admin['username']} reset their own password from {get_client_ip(request)}"
        )
        _append_audit_log("self_reset_password", admin, {})
        return JSONResponse({"success": True, "message": "Password reset successful"})
    else:
        logger.warning(
            f"Failed self password reset for {admin['username']} from {get_client_ip(request)}"
        )
        raise HTTPException(status_code=400, detail="Failed to reset password")
...
async def admin_system(request: Request):
    ...
    try:
        ...
        context = {"request": request, "admin": admin, "system_info": system_info}
...
async def approve_all_plugin_requests(
    ...
    try:
        if plugin_name:
            ...
            _append_audit_log(
                "approve_plugin_requests",
                admin,
                {"plugin": plugin_name, "approved": requested},
            )
        else:
            ...
            _append_audit_log(
                "approve_plugin_requests_all", admin, {"approved_count": len(approved)}
            )
...
async def deny_plugin_request(
    ...
    try:
        ...
        _append_audit_log(
            "deny_plugin_request", admin, {"plugin": plugin_name, "module": module_name}
        )
        return JSONResponse(
            {
                "success": True,
...
async def reload_plugin(
    ...
    try:
        ...
        _append_audit_log(
            "reload_plugin",
            admin,
            {"plugin": plugin_name, "unloaded": success_unload, "loaded": success_load},
        )
...
async def security_center(request: Request):
    ...
    try:
        ...
        try:
            ...
            ddos_info["blocked_ips"] = (
                ddos_protection_manager.list_blocked_ips()
                if hasattr(ddos_protection_manager, "list_blocked_ips")
                else []
            )
        except Exception:
            ddos_info = {"available": False, "blocked_ips": []}
        ...
        try:
            ...
            key_rotation = (
                key_rotation_status()
                if callable(key_rotation_status)
                else {"last_rotated": None}
            )
        except Exception:
            key_rotation = {"last_rotated": None}
...
async def api_security(_admin: dict = Depends(require_admin)):
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to fetch security information"
        )
            ...
            return JSONResponse(
                {
                    "success": True,
                    "ip": ip,
                    "note": "Recorded manual block (ddos manager not available).",
                }
            )
        raise HTTPException(
            status_code=500, detail="DDoS protection system not available"
        )
    except Exception as e:
            ...
            return JSONResponse(
                {
                    "success": True,
                    "ip": ip,
                    "note": "Recorded manual unblock (ddos manager not available).",
                }
            )
        raise HTTPException(
            status_code=500, detail="DDoS protection system not available"
...
async def update_config(update: ConfigUpdate, admin: dict = Depends(require_admin)):
    try:
        ...
        return JSONResponse(
            {"success": True, "key": key, "old": old_value, "new": update.value}
        )
    except Exception as e:
        ...
        if until:
            try:
                ...
                entries = [
                    e
                    for e in entries
                    if datetime.fromisoformat(e["timestamp"]) <= until_dt
                ]
            except Exception:
                pass
        if action:
...
@protect_from_replay()
@admin_endpoint(permissions=[RequiredPermission.SYSTEM])
async def https_setup(
    req: HttpsWizardRequest = Body(...), admin: dict = Depends(require_admin)
):
    ...
    try:
        ...
        if method == "selfsigned":
            ...
            _append_audit_log(
                "https_selfsigned",
                admin,
                {"domain": domain, "crt": str(crt_file), "key": str(key_file)},
            )
            result["crt"] = str(crt_file)
            result["key"] = str(key_file)
            result["note"] = (
                "Self-signed placeholder created. Replace with a real certificate in production."
            )
            return JSONResponse(result)
        elif method == "letsencrypt":
            # Record request and provide instructions instead of performing ACME flow
            _append_audit_log(
                "https_letsencrypt_requested",
                admin,
                {"domain": domain, "email": req.email},
            )
            instructions = (
            ...
            if not req.cert_pem or not req.key_pem:
                raise HTTPException(
                    status_code=400, detail="cert_pem and key_pem required for upload"
                )
            crt_file = ssl_dir / f"{domain}.crt"
            key_file = ssl_dir / f"{domain}.key"
            # Basic validation: ensure PEM headers present
            if (
                "BEGIN CERTIFICATE" not in req.cert_pem
                or "BEGIN PRIVATE KEY" not in req.key_pem
            ):
