...
SHARED_SECRET = os.environ.get(
    "PLEXICHAT_TIME_ENC_SECRET", "default-shared-secret"
).encode()
...
async def secure_admin_endpoint(decrypted_body: bytes = Depends(decrypt_request)):
...
async def setup_admin_page(request: Request):
    try:
        return templates.TemplateResponse(
            "setup/admin.html",
            {"request": request, "title": "Admin Account Setup", "step": "admin"},
        )
    except Exception as e:
...
async def setup_complete(request: Request):
    try:
        ...
        return templates.TemplateResponse(
            "setup/complete.html",
            {"request": request, "title": "Setup Complete", "step": "complete"},
        )
    except Exception as e:
...
async def ssl_check_software():
    ...
    try:
        result = subprocess.run(
            ["certbot", "--version"], check=False, capture_output=True, text=True
        )
...
async def ssl_upload_cert(
    cert_file: bytes = Form(...), key_file: bytes = Form(...), domain: str = Form(...)
):
...
async def add_user_custom_field(
    ...
    if (
        len(user.custom_fields) >= MAX_CUSTOM_FIELDS
        and field_name not in user.custom_fields
    ):
    ...
    logging.getLogger(__name__).info(
        f"Custom field added/updated for user {user_id}: {field_name} ({field_type})"
    )
...
async def get_user_custom_fields(
    user_id: int, _: None = Depends(enforce_global_rate_limit)
):
    user_service = UserModelService()
    user = await user_service.get_user_by_id(user_id)
    if not user:
...
async def get_user(user_id: int, _: None = Depends(enforce_global_rate_limit)):
    ...
    custom_fields = {
        k: cast_custom_field_value(v) for k, v in user.custom_fields.items()
    }
...
async def get_message(message_id: int, _: None = Depends(enforce_global_rate_limit)):
    ...
    if not message:
        raise HTTPException(status_code=404, detail="Message not found.")
    custom_fields = {
        k: cast_custom_field_value(v) for k, v in message.custom_fields.items()
    }
...
async def add_user_custom_field_type(
    field_type: str = Body(...), _: None = Depends(enforce_global_rate_limit)
):
    ...
    logging.getLogger(__name__).info(
        f"Admin added user custom field type: {field_type}"
    )
...
async def remove_user_custom_field_type(
    field_type: str = Query(...), _: None = Depends(enforce_global_rate_limit)
):
    if field_type in {"string", "int", "float", "bool", "list", "dict", "datetime"}:
        raise HTTPException(
            status_code=400, detail="Cannot remove default field types."
        )
    global_user_field_types.discard(field_type)
    import logging

    logging.getLogger(__name__).info(
        f"Admin removed user custom field type: {field_type}"
    )
    return {"success": True, "user_custom_field_types": sorted(global_user_field_types)}
...
async def add_message_custom_field_type(
    ...
    logging.getLogger(__name__).info(
        f"Admin added message custom field type: {field_type}"
    )
    return {
        "success": True,
        "message_custom_field_types": sorted(global_message_field_types),
    }
...
async def remove_message_custom_field_type(
    ...
    logging.getLogger(__name__).info(
        f"Admin removed message custom field type: {field_type}"
    )
    return {
        "success": True,
        "message_custom_field_types": sorted(global_message_field_types),
...
async def remove_user_custom_field(
    ...
    logging.getLogger(__name__).info(
        f"Custom field removed for user {user_id}: {field_name}"
    )
    return {"success": True, "custom_fields": user.custom_fields}
...
async def remove_message_custom_field(
    ...
    logging.getLogger(__name__).info(
        f"Custom field removed for message {message_id}: {field_name}"
    )
    return {"success": True, "custom_fields": message.custom_fields}
