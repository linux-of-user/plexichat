...
class WhitelistRequest(BaseModel):
    ...
    ip_address: str = Field(
        ...,
        regex=r"^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$|^([0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}$|^(?:[0-9]{1,3}\.){3}[0-9]{1,3}/[0-9]{1,2}$",
    )
...
async def require_security_admin(user: dict = Depends(get_current_user)):
    ...
    if "security_admin" not in user.get("permissions", []):
        raise HTTPException(
            status_code=HTTP_403_FORBIDDEN, detail="Security admin permissions required"
        )
    return user
        ...
        if get_quantum_manager:
            try:
                ...
                encryption_status = {
                    ...
                    "hybrid_mode": quantum_manager.config.get(
...
async def update_ddos_config(
    config: DDoSConfigUpdate, user: dict = Depends(require_security_admin)
):
...
async def get_blocked_ips(user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        for ip, info in blocked_ips.items():
            if info["expires_at"]:
                info["expires_at_iso"] = datetime.fromtimestamp(
                    info["expires_at"]
                ).isoformat()
...
async def block_ip(
    request: IPBlockRequest, user: dict = Depends(require_security_admin)
):
    ...
    try:
        ...
        if request.permanent:
            ddos_system.ip_block_manager.permanent_block(
                request.ip_address, request.reason
            )
            action = "permanently blocked"
        else:
            ddos_system.ip_block_manager.block_ip(
                request.ip_address, request.duration_seconds, request.reason
            )
            action = f"blocked for {request.duration_seconds} seconds"

        logger.warning(
            f"IP {request.ip_address} {action} by user {user['user_id']}: {request.reason}"
        )
        ...
        raise HTTPException(
            status_code=HTTP_400_BAD_REQUEST, detail=f"Failed to block IP: {e!s}"
        )


@router.post("/ddos/unblock-ip")
async def unblock_ip(
    request: IPUnblockRequest, user: dict = Depends(require_security_admin)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=HTTP_400_BAD_REQUEST, detail=f"Failed to unblock IP: {e!s}"
        )
    ...
    try:
        ...
        return {
            ...
            "config": {
                ...
                "enable_post_quantum": quantum_manager.config.get(
                    "enable_post_quantum", False
                ),
                "enable_hybrid_mode": quantum_manager.config.get(
                    "enable_hybrid_mode", False
                ),
                "http_traffic_encryption": quantum_manager.config.get(
                    "http_traffic_encryption", False
                ),
                "realtime_key_derivation": quantum_manager.config.get(
                    "realtime_key_derivation", False
                ),
            },
        }
...
async def update_encryption_config(
    config: EncryptionConfigUpdate, user: dict = Depends(require_security_admin)
):
    ...
    try:
        ...
        if config.key_rotation_interval_hours is not None:
            quantum_manager.config["key_rotation_interval"] = (
                config.key_rotation_interval_hours * 3600
            )

        if config.http_traffic_encryption is not None:
            quantum_manager.config["http_traffic_encryption"] = (
                config.http_traffic_encryption
            )

        if config.realtime_key_derivation is not None:
            quantum_manager.config["realtime_key_derivation"] = (
                config.realtime_key_derivation
            )
...
async def rotate_encryption_key(
    request: KeyRotationRequest, user: dict = Depends(require_security_admin)
):
...
async def get_rate_limiting_status(user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        if not rate_limiter:
            return {"enabled": False, "message": "Rate limiting system not available"}
...
async def update_rate_limiting_config(
    config: RateLimitConfigUpdate, user: dict = Depends(require_security_admin)
):
    ...
    try:
        ...
        if config.burst_multiplier is not None:
            rate_limiter.burst_limit = int(
                rate_limiter.requests_per_minute * config.burst_multiplier
            )
    ...
    try:
        ...
        if config_manager:
            ...
            return {
                ...
                "threat_response_level": security_config.get(
                    "threat_response_level", "medium"
                ),
                "alert_thresholds": security_config.get(
                    "alert_thresholds",
                    {
                ...
                "incident_response_enabled": security_config.get(
                    "incident_response_enabled", True
                ),
                "audit_logging_level": security_config.get(
...
async def update_security_policies(
    policies: SecurityPolicyUpdate, user: dict = Depends(require_security_admin)
):
    ...
    try:
        ...
        if policies.incident_response_enabled is not None:
            security_config["incident_response_enabled"] = (
                policies.incident_response_enabled
            )
    ...
    try:
        ...
        if get_ddos_protection:
            try:
                ...
                ddos_metrics = {
                    "requests_per_second": status.get("stats", {}).get(
                        "total_requests", 0
                    )
                    / 60,  # Rough estimate
                    "blocked_requests_per_second": status.get("stats", {}).get(
                        "blocked_requests", 0
                    )
                    / 60,
        ...
        if get_quantum_manager:
            try:
                ...
                encryption_metrics = {
                    ...
                    "hybrid_mode_enabled": quantum_manager.config.get(
                        "enable_hybrid_mode", False
                    ),
                }
            except Exception as e:
...
async def emergency_lockdown(user: dict = Depends(require_security_admin)):
...
async def disable_emergency_lockdown(user: dict = Depends(require_security_admin)):
...
async def security_health_check():
    ...
    try:
        if get_quantum_manager:
            ...
            health_status["components"]["encryption"] = {"status": "unavailable"}
    except Exception as e:
        health_status["components"]["encryption"] = {"status": "error", "error": str(e)}
