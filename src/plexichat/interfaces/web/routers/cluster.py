...
class ClusterService:
    ...
    def _log_audit(self, event: str, details: dict[str, Any] | None = None):
        ...
        logger.debug(
            Fore.MAGENTA + f"[CLUSTER-AUDIT] {event}: {details}" + Style.RESET_ALL
        )
    ...
    def _node_to_info(self, node_obj: Any) -> NodeInfo:
        ...
        try:
            ...
            return NodeInfo(
                ...
                status=str(
                    g("status", "unknown").value
                    if hasattr(g("status"), "value")
                    else g("status", "unknown")
                ),
                cpu_usage=float(
                    getattr(
                        getattr(node_obj, "metrics", {}),
                        "cpu_usage",
                        g("cpu_usage", 0.0),
                    )
                    if hasattr(node_obj, "metrics")
                    else g("cpu_usage", 0.0)
                ),
                memory_usage=float(
                    getattr(
                        getattr(node_obj, "metrics", {}),
                ...
                disk_usage=float(
                    ...
                    if hasattr(node_obj, "metrics")
                    else g("disk_usage", 0.0)
                ),
                last_heartbeat=getattr(
                    node_obj, "last_heartbeat", g("last_heartbeat", datetime.utcnow())
                ),
                uptime_seconds=int(
                    getattr(
                        getattr(node_obj, "metrics", {}),
    ...
    @(
        async_track_performance("cluster_status")
    ...
    async def get_cluster_status(self) -> ClusterStatus:
        ...
        try:
            if self.cluster_manager:
                ...
                cluster_id = status_data.get(
                    "cluster_id", status_data.get("id", "default")
                )
                total_nodes = status_data.get(
                    "total_nodes", len(status_data.get("nodes", []))
                )
                healthy_nodes = status_data.get(
                    "healthy_nodes", status_data.get("active_nodes", 0)
                )
                inactive_nodes = total_nodes - healthy_nodes
                cluster_health = status_data.get(
                    "status", status_data.get("cluster_health", "unknown")
                )
                load_balancer_status = status_data.get(
                    "load_balancer_status", status_data.get("load_balancer", "inactive")
                )
                total_requests = int(
                    status_data.get("metrics", {}).get(
                        "total_request_rate", status_data.get("total_requests", 0)
                    )
                )
                avg_resp = float(
                    status_data.get("metrics", {}).get(
                ...
                # Fallback status
                self._log_audit(
                    "get_cluster_status_single_node",
                    {"requested_at": datetime.utcnow().isoformat()},
                )
                return ClusterStatus(
    ...
    @(
        async_track_performance("cluster_nodes")
        if async_track_performance
        else lambda f: f
    )
    ...
    @(
        async_track_performance("cluster_metrics")
        if async_track_performance
        else lambda f: f
    )
    ...
    async def register_node(self, payload: RegisterNodeRequest) -> dict[str, Any]:
        ...
        try:
            if not self.cluster_manager:
                ...
                self._log_audit(
                    "register_node_not_supported", {"payload": payload.dict()}
                )
            ...
            if _ClusterNodeClass:
                ...
                node_id = (
                    payload.node_id
                    or f"node-{payload.hostname}-{int(datetime.utcnow().timestamp())}"
                )
                ...
                self._log_audit(
                    "register_node", {"node_id": node_obj.node_id, "success": success}
                )
                ...
                success = (
                    await self.cluster_manager.register_node(payload.dict())
                    if hasattr(self.cluster_manager, "register_node")
                    else False
                )
                self._log_audit(
                    "register_node_mapping",
                    {"payload": payload.dict(), "success": success},
                )
    ...
    async def unregister_node(self, node_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            self._log_audit(
                "unregister_node_error", {"node_id": node_id, "error": str(e)}
            )
            return {"success": False, "error": str(e)}
    ...
    async def trigger_failover(self, node_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            self._log_audit(
                "trigger_failover_error", {"node_id": node_id, "error": str(e)}
            )
            return {"success": False, "error": str(e)}
    ...
    async def get_metrics_history(self, limit: int = 100) -> list[dict[str, Any]]:
        ...
        try:
            ...
            self._log_audit(
                "get_metrics_history", {"limit": limit, "returned": len(hist)}
            )
            return hist
        except Exception as e:
    ...
    async def update_node_metrics(
        self, node_id: str, metrics: ManualMetricsUpdate
    ) -> dict[str, Any]:
        """Allow manual / external metrics updates for a node (e.g., from external monitor)."""
        try:
            if not self.cluster_manager:
                self._log_audit(
                    "update_node_metrics_not_supported", {"node_id": node_id}
                )
                return {"success": False, "reason": "Single-node mode"}
            ...
            self._log_audit(
                "update_node_metrics", {"node_id": node_id, "success": success}
            )
            return {"success": bool(success)}
        except Exception as e:
            logger.error(f"Error updating node metrics for {node_id}: {e}")
            self._log_audit(
                "update_node_metrics_error", {"node_id": node_id, "error": str(e)}
            )
            return {"success": False, "error": str(e)}
...
async def get_cluster_status(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Status requested by admin {current_user.get('username')} from {client_ip}"
        + Style.RESET_ALL
    )
    ...
    if performance_logger:
        ...
        logger.debug(
            Fore.GREEN
            + "[CLUSTER] Status performance metric recorded"
            + Style.RESET_ALL
        )
...
async def get_cluster_nodes(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Nodes requested by admin {current_user.get('username')} from {client_ip}"
        + Style.RESET_ALL
    )
    ...
    if performance_logger:
        try:
            ...
            pass
        logger.debug(
            Fore.GREEN + "[CLUSTER] Nodes performance metric recorded" + Style.RESET_ALL
        )
...
async def get_node_details(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Node {node_id} details requested by admin {current_user.get('username')} from {client_ip}"
        + Style.RESET_ALL
    )
...
async def register_node(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Register node requested by admin {current_user.get('username')} from {client_ip}: {payload.dict()}"
        + Style.RESET_ALL
    )
    ...
    if not result.get("success"):
        raise HTTPException(
            ...
            detail=result.get("reason", result.get("error", "Failed to register node")),
        )
    return result
...
async def unregister_node(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Unregister node {node_id} requested by admin {current_user.get('username')} from {client_ip}"
        + Style.RESET_ALL
    )
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=result.get(
                "error", result.get("reason", "Failed to unregister node")
            ),
        )
    return result
...
@router.post(
    "/nodes/{node_id}/metrics", summary="Update node metrics (manual/external)"
)
async def update_node_metrics(
    node_id: str,
    payload: ManualMetricsUpdate,
    request: Request,
    current_user: dict[str, Any] = Depends(require_admin),
):
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Update metrics for node {node_id} by {current_user.get('username')} from {client_ip}: {payload.dict()}"
        + Style.RESET_ALL
    )
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=result.get(
                "error", result.get("reason", "Failed to update node metrics")
            ),
        )
    return result
...
async def failover_node(
    ...
    logger.warning(
        Fore.YELLOW
        + f"[CLUSTER] Failover requested for node {node_id} by admin {current_user.get('username')} from {client_ip}"
        + Style.RESET_ALL
    )
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=result.get(
                "error", result.get("reason", "Failed to trigger failover")
            ),
        )
    return result
...
async def run_health_check(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Health check requested by admin {current_user.get('username')} from {client_ip}"
        + Style.RESET_ALL
    )
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=result.get(
                "error", result.get("reason", "Failed to run health checks")
            ),
        )
    return result
...
async def get_cluster_metrics(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Metrics requested by admin {current_user.get('username')} from {client_ip}"
        + Style.RESET_ALL
    )
    ...
    if performance_logger:
        ...
        logger.debug(
            Fore.GREEN
            + "[CLUSTER] Metrics performance metric recorded"
            + Style.RESET_ALL
        )
...
async def get_metrics_history(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Metrics history requested by admin {current_user.get('username')} from {client_ip} limit={limit}"
        + Style.RESET_ALL
    )
...
async def get_config(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Config requested by admin {current_user.get('username')} from {client_ip}"
        + Style.RESET_ALL
    )
...
async def update_config(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Config update requested by admin {current_user.get('username')} from {client_ip}: {payload.dict(exclude_unset=True)}"
        + Style.RESET_ALL
    )
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=result.get(
                "error", result.get("reason", "Failed to update configuration")
            ),
        )
    return result
...
async def scale_cluster(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Scaling to {target_nodes} nodes requested by admin {current_user.get('username')} from {client_ip}"
        + Style.RESET_ALL
    )
    ...
    if performance_logger:
        try:
            ...
            pass
        logger.debug(
            Fore.GREEN + "[CLUSTER] Scale performance metric recorded" + Style.RESET_ALL
        )
...
async def rebalance_cluster(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Rebalancing requested by admin {current_user.get('username')} from {client_ip}"
        + Style.RESET_ALL
    )
    ...
    if performance_logger:
        ...
        logger.debug(
            Fore.GREEN
            + "[CLUSTER] Rebalance performance metric recorded"
            + Style.RESET_ALL
        )
    ...
    try:
        if cluster_service.cluster_manager:
            ...
            return {"message": "Cluster rebalancing not applicable in single-node mode"}
    except Exception as e:
...
async def update_node_weight(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Update weight for node {node_id} to {payload.weight} requested by admin {current_user.get('username')} from {client_ip}"
        + Style.RESET_ALL
    )
    try:
        if not cluster_service.cluster_manager:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Single-node mode"
            )
        ...
        if callable(rebuild):
            rebuild()
        cluster_service._log_audit(
            "update_node_weight", {"node_id": node_id, "weight": payload.weight}
        )
...
async def get_audit_logs(
    ...
    logger.info(
        Fore.CYAN
        + f"[CLUSTER] Audit logs requested by admin {current_user.get('username')} from {client_ip} limit={limit}"
        + Style.RESET_ALL
    )
