...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        try:
            ...
            logger.security(
                f"Error during middleware component initialization: {e}",
        ...
        try:
            ip_check, rate_check, input_check = await asyncio.gather(
                ip_check_future, rate_check_future, input_check_future
            )
        except Exception as e:
            logger.security(
                f"Error during concurrent security checks: {e}",
        ...
        try:
            auth_check = await self._check_authentication_authorization(
                request, request_info
            )
        except Exception as e:
            logger.security(
                f"Authentication check error: {e}", component="authentication"
            )
            self.stats["auth_failures"] += 1
            return self._create_security_response(
                {"authenticated": False, "reason": "Authentication system error"}, 500
            )
        if not auth_check.get("authenticated") and not self._is_public_endpoint(
            request_info["path"]
        ):
            return self._create_security_response(auth_check, 401)
        # Risk-based authentication for admin/critical endpoints
        endpoint_level = self.endpoint_security_levels.get(
            request_info["path"], SecurityLevel.BASIC
        )
        if endpoint_level >= SecurityLevel.CRITICAL and auth_check.get("authenticated"):
            risk_result = await self._risk_based_authentication(
                request, request_info, auth_check
            )
            if not risk_result["allowed"]:
                return self._create_security_response(risk_result, 403)
        if auth_check.get("authenticated"):
            try:
                session_check = await self._validate_session_security(
                    request, auth_check
                )
            except Exception as e:
                logger.security(
                    f"Session validation error: {e}", component="session_validation"
                )
                return self._create_security_response(
                    {"valid": False, "reason": "Session validation failed"}, 401
                )
    ...
    async def _check_ip_security(self, request_info: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.network_protection:
            try:
                ...
                allowed, threat = await self.network_protection.check_request(
                    rate_request
                )
                if not allowed:
            ...
            if self.input_validator:
                try:
                    validation_result = self.input_validator.validate(
                        body, InputType.TEXT, ValidationLevel.STANDARD
                    )
    ...
    async def _check_authentication_authorization(
        self, request: Request, request_info: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    async def _validate_csrf_token(self, request: Request) -> dict[str, Any]:
        ...
        try:
            ...
            if request.url.path.startswith("/api/") and request.headers.get(
                "Authorization"
            ):
                return {"valid": True}
            ...
            if cookie_token:
                ...
                if not session_id:
                    return {"valid": False, "reason": "No session for CSRF validation"}
    ...
    async def _validate_session_security(
        ...
        """
        session_id = (
            auth_check.get("session_id") or request.cookies.get("session_id") or None
        )
        ...
        if not self.auth_manager:
            logger.security(
                "Authentication backend unavailable during session validation",
                user_id=user_id,
                session_id=session_id,
            )
        ...
        try:
            ...
            if (
                user_id
                and session_info
                and getattr(session_info, "user_id", None) != user_id
            ):
                logger.security(
                    ...
                    session_id=session_id,
                    details={
                        "session_user": getattr(session_info, "user_id", None),
                        "token_user": user_id,
                    },
                )
                await self._log_security_event(
            ...
            logger.security(
                f"Error validating session: {e}", component="session_validation"
            )
            return {"valid": False, "reason": "Session validation error"}

    async def _risk_based_authentication(
        self, request: Request, request_info: dict[str, Any], auth_check: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        user_agent = request_info["user_agent"]
        if (
            "Windows" not in user_agent
            and "Macintosh" not in user_agent
            and "Linux" not in user_agent
        ):
            risk_score += 0.2
        ...
        if mfa_required:
            ...
            mfa_token = request.headers.get("x-mfa-token") or request.query_params.get(
                "mfa_token"
            )
        ...
        if not allowed:
            return {
                "allowed": False,
                "reason": "Risk-based authentication failed: "
                + ", ".join(risk_factors),
                "action": "step_up_auth",
                "risk_score": risk_score,
                "risk_factors": risk_factors,
    ...
    def _create_security_response(
        self, check_result: dict[str, Any], status_code: int
    ) -> JSONResponse:
        return JSONResponse(
            {
                "success": False,
    ...
    async def _log_security_event(
        ...
        try:
            ...
            if self.audit_system:
                ...
                logger.debug(
                    "Audit system not available; security event logged to unified logger only"
                )
        except Exception as e:
