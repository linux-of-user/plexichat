...
class ThreadCreate(BaseModel):
    ...
    parent_message_id: str | None = Field(
        None, description="Parent message ID if replying to a message"
    )
...
async def create_thread(
    thread_data: ThreadCreate, current_user: dict = Depends(get_current_user)
):
    """Create a new thread."""
...
async def update_thread_title(
    ...
    success = await threads_service.update_thread_title(
        thread_id=thread_id, new_title=thread_data.title, user_id=current_user["id"]
    )

    if not success:
        raise HTTPException(
            status_code=403, detail="Permission denied or thread not found"
        )
