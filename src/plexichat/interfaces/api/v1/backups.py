...
async def create_backup(
    ...
    try:
        ...
        logger.info(f"Backup creation requested by user {redact_pii(current_user.get('username', 'unknown'))}",
        ...
        logger.info(f"Backup created successfully: {backup_metadata.backup_id}",
                   extra={"user_id": current_user.get('id'), "backup_id": backup_metadata.backup_id})
        ...
        error_msg = str(e)
        logger.error(f"Backup creation failed: {redact_pii(error_msg)}",
...
async def list_backups(
    ...
    try:
        ...
        logger.info(f"Backups listed for user {redact_pii(current_user.get('username', 'unknown'))}",
        ...
        logger.error(f"Failed to list backups: {redact_pii(str(e))}",
                    extra={"user_id": current_user.get('id')})
...
async def get_backup(
    ...
    try:
        ...
        if backup.get('user_id') != current_user.get('id'):
            logger.warning(f"Unauthorized access attempt to backup {backup_id}",
        ...
        logger.info(f"Backup details retrieved: {backup_id}",
                   extra={"user_id": current_user.get('id')})
        ...
        logger.error(f"Failed to get backup {backup_id}: {redact_pii(str(e))}",
                    extra={"user_id": current_user.get('id')})
...
async def delete_backup(
    ...
    try:
        ...
        if backup.get('user_id') != current_user.get('id'):
            logger.warning(f"Unauthorized delete attempt for backup {backup_id}",
                          extra={"user_id": current_user.get('id')})
            raise HTTPException(status_code=403, detail="Access denied")
        ...
        logger.error(f"Failed to delete backup {backup_id}: {redact_pii(str(e))}",
                    extra={"user_id": current_user.get('id')})
        raise HTTPException(status_code=500, detail="Failed to delete backup")
...
async def rotate_backup_keys(
    ...
    try:
        ...
        if backup.get('user_id') != current_user.get('id'):
            logger.warning(f"Unauthorized key rotation attempt for backup {backup_id}",
                          extra={"user_id": current_user.get('id')})
            raise HTTPException(status_code=403, detail="Access denied")
        ...
        logger.info(f"Keys rotated for backup {backup_id}",
                   extra={"user_id": current_user.get('id')})
        ...
        logger.error(f"Failed to rotate keys for backup {backup_id}: {redact_pii(str(e))}",
                    extra={"user_id": current_user.get('id')})
