...
async def create_group(
    group_data: GroupCreate, current_user: dict = Depends(get_current_user)
):
    """Create a new group."""
    group_id = str(uuid4())
    group = {
...
async def list_groups(current_user: dict = Depends(get_current_user)):
    ...
    return [
        ...
        if g["is_public"]
        or current_user["user_id"] in group_members_db.get(g["id"], [])
    ]
...
async def update_group(
    ...
    if group["owner_id"] != current_user["user_id"]:
        raise HTTPException(
            status_code=403, detail="Only the owner can update the group"
        )
...
async def delete_group(group_id: str, current_user: dict = Depends(get_current_user)):
    ...
    if group["owner_id"] != current_user["user_id"]:
        raise HTTPException(
            status_code=403, detail="Only the owner can delete the group"
        )
...
async def leave_group(group_id: str, current_user: dict = Depends(get_current_user)):
    """Leave a group."""
    if (
        group_id in group_members_db
        and current_user["user_id"] in group_members_db[group_id]
    ):
