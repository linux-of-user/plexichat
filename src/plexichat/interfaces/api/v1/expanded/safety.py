...
class SafetyReport(BaseModel):
    ...
    reported_content_id: Optional[str] = Field(None, description="ID of reported content")
...
class ModerationActionRecord(BaseModel):
    ...
    reversed_at: Optional[datetime] = Field(None, description="When action was reversed")
...
class AutoModRule(BaseModel):
    ...
    action: ModerationAction = Field(..., description="Action to take when rule is triggered")
    ...
    # Scope
    channels: List[str] = Field(default_factory=list, description="Channels to apply rule")
    user_roles: List[str] = Field(default_factory=list, description="User roles to apply rule")
    ...
    updated_at: Optional[datetime] = Field(None, description="Last update timestamp")
    trigger_count: int = Field(default=0, description="Number of times rule was triggered")
...
class ContentFilterResult(BaseModel):
    ...
    suggested_action: ModerationAction = Field(..., description="Suggested moderation action")
...
class TrustScore(BaseModel):
    ...
    history: List[Dict[str, Any]] = Field(default_factory=list, description="Score history")
...
async def setup_safety_endpoints(router: APIRouter):
    ...
    @router.post("/report", response_model=SafetyReport, summary="Report Content or User")
    async def create_safety_report(
        ...
        try:
            ...
            if not reported_user_id and not reported_content_id:
                raise HTTPException(status_code=400, detail="Must specify either user or content to report")
            
            # Create report
            report = await _create_safety_report(
                reporter_id, report_type, description, 
                reported_user_id, reported_content_id, evidence_urls
            )
    ...
    @router.get("/reports", response_model=List[SafetyReport], summary="Get Safety Reports")
    async def get_safety_reports(
        ...
        try:
            ...
            reports = await _get_safety_reports(status, report_type, assigned_to_me, limit, offset)
            return reports
    ...
    @router.get("/reports/{report_id}", response_model=SafetyReport, summary="Get Specific Report")
    ...
    @router.post("/reports/{report_id}/action", response_model=ModerationActionRecord, summary="Take Action on Report")
    async def take_moderation_action(
        ...
        try:
            ...
            action = await _take_moderation_action(report_id, moderator_id, action_type, reason, duration)
    ...
    @router.post("/moderate", response_model=ModerationActionRecord, summary="Moderate Content")
    async def moderate_content(
        ...
        try:
            ...
            action = await _moderate_content(
                moderator_id, target_user_id, target_content_id, 
                action_type, reason, duration, evidence
            )
    ...
    @router.get("/automod/rules", response_model=List[AutoModRule], summary="Get Auto-Mod Rules")
    ...
    @router.post("/automod/rules", response_model=AutoModRule, summary="Create Auto-Mod Rule")
    ...
    @router.put("/automod/rules/{rule_id}", response_model=AutoModRule, summary="Update Auto-Mod Rule")
    ...
    @router.get("/blocked-users", response_model=List[BlockedUser], summary="Get Blocked Users")
    ...
    @router.post("/blocked-users/{user_id}", response_model=BlockedUser, summary="Block User")
    ...
    @router.post("/content-filter", response_model=ContentFilterResult, summary="Filter Content")
    ...
    @router.get("/trust-score/{user_id}", response_model=TrustScore, summary="Get User Trust Score")
...
async def _get_automod_rules(rule_type: Optional[AutoModRuleType], enabled_only: bool) -> List[AutoModRule]:
...
async def _update_automod_rule(rule_id: str, updates: Dict[str, Any]) -> Optional[AutoModRule]:
...
async def _get_blocked_users(user_id: str, limit: int, offset: int) -> List[BlockedUser]:
    """Get blocked users."""
    # Placeholder implementation
    return []

async def _block_user(blocker_id: str, user_id: str, reason: Optional[str]) -> BlockedUser:
    """Block a user."""
    # Placeholder implementation
    return BlockedUser(
...
async def _filter_content(content: str, content_type: str, context: Dict[str, Any]) -> ContentFilterResult:
    """Filter content through safety systems."""
