...
class Phase5APIExpansionCoordinator:
    ...
    async def _initialize_category_routers(self):
        ...
        for category in APICategory:
            router = APIRouter(
                prefix=f"/{category.value.replace('_', '-')}",
                tags=[category.value]
            )
    ...
    async def _register_user_profile_endpoints(self):
        ...
        endpoints = [
            APIEndpointInfo("/user-profiles/me", "GET", APICategory.USER_PROFILES, "Get current user profile"),
            APIEndpointInfo("/user-profiles/me", "PUT", APICategory.USER_PROFILES, "Update current user profile"),
            APIEndpointInfo("/user-profiles/{user_id}", "GET", APICategory.USER_PROFILES, "Get user profile by ID"),
            APIEndpointInfo("/user-profiles/{user_id}/avatar", "POST", APICategory.USER_PROFILES, "Upload user avatar"),
            APIEndpointInfo("/user-profiles/{user_id}/banner", "POST", APICategory.USER_PROFILES, "Upload user banner"),
            APIEndpointInfo("/user-profiles/{user_id}/preferences", "GET", APICategory.USER_PROFILES, "Get user preferences"),
            APIEndpointInfo("/user-profiles/{user_id}/preferences", "PUT", APICategory.USER_PROFILES, "Update user preferences"),
            APIEndpointInfo("/user-profiles/{user_id}/privacy", "GET", APICategory.USER_PROFILES, "Get privacy settings"),
            APIEndpointInfo("/user-profiles/{user_id}/privacy", "PUT", APICategory.USER_PROFILES, "Update privacy settings"),
            APIEndpointInfo("/user-profiles/{user_id}/activity", "GET", APICategory.USER_PROFILES, "Get user activity"),
            APIEndpointInfo("/user-profiles/{user_id}/connections", "GET", APICategory.USER_PROFILES, "Get user connections"),
            APIEndpointInfo("/user-profiles/{user_id}/badges", "GET", APICategory.USER_PROFILES, "Get user badges"),
            APIEndpointInfo("/user-profiles/search", "GET", APICategory.USER_PROFILES, "Search user profiles"),
            APIEndpointInfo("/user-profiles/bulk", "POST", APICategory.USER_PROFILES, "Get multiple user profiles"),
        ]
        
        for endpoint in endpoints:
            self.registered_endpoints[f"{endpoint.category.value}{endpoint.path}"] = endpoint
    ...
    async def _register_search_endpoints(self):
        ...
        endpoints = [
            APIEndpointInfo("/search/global", "GET", APICategory.SEARCH, "Global search across all content"),
            APIEndpointInfo("/search/messages", "GET", APICategory.SEARCH, "Search messages"),
            APIEndpointInfo("/search/users", "GET", APICategory.SEARCH, "Search users"),
            APIEndpointInfo("/search/channels", "GET", APICategory.SEARCH, "Search channels"),
            APIEndpointInfo("/search/files", "GET", APICategory.SEARCH, "Search files"),
            APIEndpointInfo("/search/semantic", "POST", APICategory.SEARCH, "Semantic search with AI"),
            APIEndpointInfo("/search/advanced", "POST", APICategory.SEARCH, "Advanced search with filters"),
            APIEndpointInfo("/search/suggestions", "GET", APICategory.SEARCH, "Get search suggestions"),
            APIEndpointInfo("/search/history", "GET", APICategory.SEARCH, "Get search history"),
            APIEndpointInfo("/search/saved", "GET", APICategory.SEARCH, "Get saved searches"),
            APIEndpointInfo("/search/saved", "POST", APICategory.SEARCH, "Save a search"),
            APIEndpointInfo("/search/saved/{search_id}", "DELETE", APICategory.SEARCH, "Delete saved search"),
        ]
    ...
    async def _register_safety_endpoints(self):
        ...
        endpoints = [
            APIEndpointInfo("/safety/report", "POST", APICategory.SAFETY, "Report content or user"),
            APIEndpointInfo("/safety/reports", "GET", APICategory.SAFETY, "Get safety reports", requires_admin=True),
            APIEndpointInfo("/safety/reports/{report_id}", "GET", APICategory.SAFETY, "Get specific report", requires_admin=True),
            APIEndpointInfo("/safety/reports/{report_id}/action", "POST", APICategory.SAFETY, "Take action on report", requires_admin=True),
            APIEndpointInfo("/safety/moderate", "POST", APICategory.SAFETY, "Moderate content", requires_admin=True),
            APIEndpointInfo("/safety/automod/rules", "GET", APICategory.SAFETY, "Get automod rules", requires_admin=True),
            APIEndpointInfo("/safety/automod/rules", "POST", APICategory.SAFETY, "Create automod rule", requires_admin=True),
            APIEndpointInfo("/safety/automod/rules/{rule_id}", "PUT", APICategory.SAFETY, "Update automod rule", requires_admin=True),
            APIEndpointInfo("/safety/automod/rules/{rule_id}", "DELETE", APICategory.SAFETY, "Delete automod rule", requires_admin=True),
            APIEndpointInfo("/safety/blocked-users", "GET", APICategory.SAFETY, "Get blocked users"),
            APIEndpointInfo("/safety/blocked-users/{user_id}", "POST", APICategory.SAFETY, "Block user"),
            APIEndpointInfo("/safety/blocked-users/{user_id}", "DELETE", APICategory.SAFETY, "Unblock user"),
            APIEndpointInfo("/safety/content-filter", "POST", APICategory.SAFETY, "Filter content"),
            APIEndpointInfo("/safety/trust-score/{user_id}", "GET", APICategory.SAFETY, "Get user trust score"),
        ]
