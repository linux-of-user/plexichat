...
class PerformanceMonitor:
    ...
    def get_average_metrics(
        ...
        return {
            ...
            "avg_network_latency": statistics.mean(
                m["network_latency"] for m in recent_metrics
            ),
            "avg_requests_per_second": statistics.mean(
    ...
    def select_node(
        ...
        healthy_nodes = [
            ...
            if (
                ...
                and self.node_connections[node.node_id]
...
class AutoScaler:
    ...
    def evaluate_scaling_decision(
        ...
        if len(active_nodes) < self.config.min_nodes:
            return (
                ...
                f"Below minimum nodes ({self.config.min_nodes})",
            )

        if len(active_nodes) >= self.config.max_nodes:
            return (
                ScalingAction.SCALE_DOWN,
                f"At maximum nodes ({self.config.max_nodes})",
            )
        ...
        if (
            avg_cpu > self.config.cpu_threshold_scale_up
            or avg_memory > self.config.memory_threshold_scale_up
        ):
        ...
        if (
            avg_cpu < self.config.cpu_threshold_scale_down
            and avg_memory < self.config.memory_threshold_scale_down
            and len(active_nodes) > self.config.min_nodes
        ):
    ...
    def _can_scale_down(self, current_time: datetime) -> bool:
        ...
        time_since_last = (
            current_time - self.last_scale_down["global"]
        ).total_seconds()
...
class EdgeComputingManager:
    ...
    async def _scale_down_nodes(self, reason: str):
        ...
        lowest_util_node = min(
            active_nodes, key=lambda n: n.metrics.cpu_usage + n.metrics.memory_usage
        )
        ...
        logger.info(
            f"Scaled down: Deactivated node {lowest_util_node.node_id}. Reason: {reason}"
        )
...
async def initialize_edge_computing_manager(
    ...
    global _global_edge_manager
    _global_edge_manager = EdgeComputingManager(
        load_balancing_config, auto_scaling_config
    )
