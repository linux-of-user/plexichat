...
class KeyboardShortcutsService:
    ...
    async def get_shortcuts(self, user_id: str) -> List[KeyboardShortcut]:
        ...
        try:
            ...
            user_shortcuts = await execute_query(
                """
    ...
    async def update_shortcut(self, user_id: str, shortcut_id: int, shortcut_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            await execute_query(
                ...
                SET shortcut_key = ?, action = ?, description = ?, updated_at = ?
    ...
    async def validate_shortcut_conflicts(
        ...
        try:
            ...
            conflicts = await execute_query(query, tuple(params))

            if conflicts:
                return f"Shortcut '{shortcut_key}' conflicts with action '{conflicts[0][0]}'"
    ...
    async def get_shortcut_by_action(self, user_id: str, action: str) -> Optional[KeyboardShortcut]:
        ...
        try:
            ...
            user_shortcut = await execute_query(
                """
                SELECT id, user_id, shortcut_key, action, description, is_custom,
                       created_at, updated_at
                FROM keyboard_shortcuts
