...
class ContentValidationSystem:
    ...
    async def validate_content(self, content: Any, context: Any) -> Dict[str, Any]:
        ...
        """
        if not self.enabled:
            return {'valid': True, 'message': 'Validation disabled', 'threat_level': 'none'}
        ...
        try:
            ...
            if threats:
                ...
                highest_threat = max(threats, key=lambda x: self._threat_level_score(x['threat_level']))
            ...
            return {'valid': True, 'message': 'Content validated successfully', 'threat_level': 'none'}

        except Exception as e:
            logger.error(f"Error in content validation: {e}")
            return {'valid': False, 'message': f'Validation error: {str(e)}', 'threat_level': 'unknown'}
    ...
    async def _detect_threats(self, content: str, sql_in_code_block: bool = False) -> List[Dict[str, Any]]:
        ...
        if not sql_in_code_block:
            ...
            for rule in self.sql_injection_rules:
                if rule.enabled and rule.compiled_pattern and rule.compiled_pattern.search(content):
                    threats.append({
                        'type': 'sql_injection',
    ...
    async def check_file_hash(self, file_content: bytes, filename: str) -> Dict[str, Any]:
        """
    ...
    def validate_content_type(self, filename: str, content_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if content_type.startswith('text/') and file_ext in {'.txt', '.md', '.json', '.csv'}:
                return {
                    'valid': True,
                    'message': f'Text content type is reasonable for {file_ext} files'
                }

            if content_type.startswith('image/') and file_ext in {'.jpg', '.jpeg', '.png', '.gif', '.webp'}:
                return {
                    'valid': True,
    ...
    def add_malicious_file_hash(self, file_hash: str, filename: str, threat_level: str,
                               description: str, reported_by: Optional[str] = None):
    ...
    def get_validation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'rules_count': {
                'sql_injection': len([r for r in self.sql_injection_rules if r.enabled]),
                'xss': len([r for r in self.xss_rules if r.enabled]),
                'command_injection': len([r for r in self.command_injection_rules if r.enabled]),
                'path_traversal': len([r for r in self.path_traversal_rules if r.enabled])
            },
