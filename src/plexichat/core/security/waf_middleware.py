...
class AttackPatterns:
    ...
    SQL_INJECTION_PATTERNS = [
        re.compile(r"(?i)(union\s+select|select\s+\*\s+from|drop\s+table|delete\s+from)", re.IGNORECASE),
        re.compile(r"(?i)(insert\s+into|update\s+\w+\s+set|alter\s+table)", re.IGNORECASE),
    ...
    PATH_TRAVERSAL_PATTERNS = [
        ...
        re.compile(r"(?i)(\/etc\/passwd|\/etc\/shadow|\/windows\/system32)", re.IGNORECASE),
    ...
    SSRF_PATTERNS = [
        ...
        re.compile(r"(?i)(169\.254\.|192\.168\.|10\.|172\.(1[6-9]|2[0-9]|3[01])\.)", re.IGNORECASE),
...
class IPReputationChecker:
    ...
    async def _query_threat_intelligence(self, ip: str) -> Tuple[bool, int]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.config.threat_intel_timeout) as client:
...
class WAFMiddleware:
    ...
    async def __call__(self, request: Request, call_next):
        ...
        try:
            ...
            # Check IP reputation
            is_malicious_ip, confidence = await self.ip_checker.check_ip_reputation(client_ip)
    ...
    async def _analyze_request_content(self, request: Request) -> Optional[ThreatDetection]:
    ...
    def _check_attack_patterns(self, content: str, content_type: str) -> Optional[ThreatDetection]:
    ...
    async def _handle_threat(self, request: Request, threat: ThreatDetection) -> Response:
    ...
    async def _log_threat(self, request: Request, client_ip: str, threat: ThreatDetection):
        """Log detected threat"""
        log_data = {
