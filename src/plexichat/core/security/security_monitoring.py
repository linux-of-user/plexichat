"""
NetLink Security Monitoring System
...
class SecurityMonitor:
    ...
    async def _send_email_alert(self, alert: SecurityAlert):
        ...
        try:
            ...
            msg = MIMEMultipart()
            msg['From'] = email_config.get("from_address", "security@netlink.local")
            msg['To'] = email_config.get("to_address", "admin@netlink.local")
            msg['Subject'] = f"NetLink Security Alert: {alert.event.event_type.value}"
...
Please investigate this security event immediately.
    ...
    async def _send_syslog_alert(self, alert: SecurityAlert):
        ...
        try:
            ...
            message = f"NetLink Security Alert [{alert.alert_id}]: {alert.event.event_type.value} - {alert.event.description}"
            
            syslog.openlog("netlink-security", syslog.LOG_PID, syslog.LOG_DAEMON)
