...
class TrafficAnalyzer:
    ...
    def analyze_ip_behavior(self, ip_metrics: IPMetrics) -> Tuple[float, List[AttackType]]:
        ...
        recent_requests = [t for t in ip_metrics.request_intervals 
        ...
        error_requests = sum(count for status, count in ip_metrics.status_codes.items() 
                           if status >= 400)
    ...
    def detect_slowloris(self, active_connections: Dict[str, float]) -> List[str]:
        ...
        for ip, connections in ip_connections.items():
            long_connections = [t for t in connections if current_time - t > 30]  # 30+ seconds
...
class IPBlockManager:
    ...
    def block_ip(self, ip: str, duration_seconds: int, reason: str = ""):
        ...
        with self._lock:
            ...
            logger.warning(f"Blocked IP {ip} for {duration_seconds} seconds. Reason: {reason}")
...
class AlertManager:
    ...
    def send_alert(self, event: AttackEvent):
        ...
        # Log alert
        logger.error(f"DDoS Alert: {event.attack_type.value} attack from {event.source_ip} "
...
class DDoSProtectionSystem:
    ...
    def __init__(self, dynamic_rate_limiter: Optional[DynamicRateLimitingMiddleware] = None):
        # Load configuration
    ...
    async def _analyze_threats(self):
        ...
        with self._lock:
            for ip, metrics in list(self.ip_metrics.items()):
                ...
                # Analyze IP behavior
                threat_score, detected_attacks = self.traffic_analyzer.analyze_ip_behavior(metrics)
                ...
                if threat_level in [ThreatLevel.HIGH, ThreatLevel.CRITICAL]:
                    ...
                    if not self.ip_block_manager.is_blocked(ip):
                        duration = self._calculate_block_duration(threat_level, threat_score)
                        ...
                        for attack_type in detected_attacks:
                            event = AttackEvent(
                                ...
                                metrics={'threat_score': threat_score, 'request_count': metrics.request_count},
        ...
        for ip in slowloris_ips:
            if not self.ip_block_manager.is_blocked(ip) and not self.ip_block_manager.is_whitelisted(ip):
                ...
                event = AttackEvent(
                    ...
                    metrics={'connection_count': len([c for c in self.active_connections if c.startswith(ip)])},
    ...
    def _calculate_block_duration(self, threat_level: ThreatLevel, threat_score: float) -> int:
    ...
    def _cleanup_old_data(self):
        ...
        with self._lock:
            ...
            old_ips = [ip for ip, metrics in self.ip_metrics.items() 
                      if current_time - metrics.last_seen > cleanup_age]
            for ip in old_ips:
                del self.ip_metrics[ip]
            
            # Clean up old connections
            old_connections = [conn_id for conn_id, start_time in self.active_connections.items()
                             if current_time - start_time > cleanup_age]
            for conn_id in old_connections:
                del self.active_connections[conn_id]
            
            if old_ips or old_connections:
                logger.debug(f"Cleaned up {len(old_ips)} old IP metrics and {len(old_connections)} old connections")
    ...
    def update_response_metrics(self, ip: str, status_code: int, response_size: int = 0):
...
class DDoSProtectionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            self.ddos_system.update_response_metrics(client_ip, response.status_code, response_size)
    ...
    def _add_protection_headers(self, response: Response):
        ...
        try:
            ...
            response.headers["X-Threat-Level"] = self.ddos_system.stats.threat_level.value
            if self.ddos_system.stats.active_attacks > 0:
                response.headers["X-Active-Attacks"] = str(self.ddos_system.stats.active_attacks)
        except Exception as e:
...
def add_ddos_protection_middleware(app, dynamic_rate_limiter: Optional[DynamicRateLimitingMiddleware] = None):
