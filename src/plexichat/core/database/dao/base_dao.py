...
class BaseDAO(Generic[T, CreateT, UpdateT], ABC):
    ...
    async def create(self, data: CreateT, **kwargs) -> T:
        ...
        async with await self.get_session() as session:
            try:
                ...
                logger.debug(
                    f"Created {self.model_class.__name__}: {instance.id if hasattr(instance, 'id') else 'unknown'}"
                )
                return instance
    ...
    async def get_by_id(
        self, id: str, include_relations: List[str] = None
    ) -> Optional[T]:
        """Get record by ID."""
