...
try:
    from plexichat.core.rate_limit_config import get_rate_limiting_config, IPBlacklistConfig
...
class IPBlacklistMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app):
        ...
        # Blacklist storage
        self.permanent_blacklist: Set[str] = set(self.blacklist_config.permanent_blacklist)
        self.temporary_blacklist: Dict[str, int] = dict(self.blacklist_config.temporary_blacklist)
    ...
    async def _cleanup_expired_entries(self):
        ...
        expired_ips = [ip for ip, expiry in self.temporary_blacklist.items() if expiry < current_time]
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if blacklist_result["blocked"]:
                return self._create_blocked_response(client_ip, blacklist_result["reason"])
            
            # Check geo-blocking
            if self.blacklist_config.geo_blocking_enabled:
                geo_result = await self._check_geo_blocking(client_ip)
                if geo_result["blocked"]:
                    return self._create_blocked_response(client_ip, geo_result["reason"])
            
            # Check for suspicious patterns
            if await self._is_suspicious_request(request, client_ip):
                return self._create_blocked_response(client_ip, "Suspicious request pattern detected")
            ...
            await self._record_request_pattern(request, response, client_ip, start_time, end_time)
    ...
    async def _is_suspicious_request(self, request: Request, ip: str) -> bool:
        ...
        if len(recent_requests) > self.blacklist_config.auto_blacklist_threshold:
            logger.warning(f"High request rate from {ip}: {len(recent_requests)} requests/minute")
            return True
        ...
        if any(agent in user_agent for agent in suspicious_agents):
            if not any(allowed in user_agent for allowed in ["googlebot", "bingbot", "facebookexternalhit"]):
    ...
    async def _record_request_pattern(self, request: Request, response: Response, 
                                    ip: str, start_time: float, end_time: float):
    ...
    async def _check_auto_blacklist(self, ip: str, status_code: int):
        ...
        if len(recent_requests) >= self.blacklist_config.auto_blacklist_threshold:
            ...
            logger.warning(f"Auto-blacklisted {ip} for {self.blacklist_config.auto_blacklist_duration}s")
        
        # Check for repeated failed attempts
        recent_failures = [t for t in self.failed_attempts[ip] if t > current_time - 300]  # 5 minutes
    ...
    def add_to_temporary_blacklist(self, ip: str, duration: int, reason: str = "Temporary block"):
