...
class ErrorCodeMapping:
    ...
    _mappings = {
        ...
        # Note: In practice, add all mappings; this is abbreviated for brevity in this response, but full in actual file
    }
...
class PlexiChatException(Exception):
    ...
    def __init__(
        ...
        self.severity = ErrorCodeMapping.get_severity(error_code)
        self.category = ErrorCodeMapping.get_category(error_code)

        mapping = ErrorCodeMapping.get_mapping(error_code)
        msg = message or mapping["message"]
        if details:
...
# Specific exceptions inheriting from PlexiChatException
...
class ExternalServiceError(PlexiChatException):
    ...
    def __init__(
        ...
        super().__init__(
            PlexiChatErrorCode.EXT_SERVICE_UNAVAILABLE,
            details=details,
            context=context,
            correlation_id=correlation_id,
            cause=cause,
...
class ProcessLockError(PlexiChatException):
    """Process lock error."""
...
class StartupError(PlexiChatException):
    """Startup error."""
...
def handle_exception(
    ...
    try:
        ...
        # Log the error using base log_error
        log_error(
            error_code,
            details=details,
            context=ctx,
...
def create_system_error_response(
    ...
    error_code = error_code_map.get(
        error_type, PlexiChatErrorCode.SYSTEM_INTERNAL_ERROR
    )
    return create_error_response(error_code, correlation_id=correlation_id)
