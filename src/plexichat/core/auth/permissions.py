...
def format_permission(resource_type: ResourceType, operation: DBOperation, resource_name: str = "any") -> str:
...
def check_permission(required_permission: str, user_permissions: Set[str]):
    ...
    if not user_permissions:
        raise PermissionError(f"Permission denied for '{required_permission}'. No permissions provided.")
    ...
    raise PermissionError(f"Permission denied. Required: '{required_permission}', but not found in user permissions.")
