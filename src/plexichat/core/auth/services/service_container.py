...
class AuthServiceContainer:
    ...
    def register_singleton(self, interface: Type[T], implementation: Type[T]) -> None:
        ...
        logger.debug(f"Registered singleton service: {interface.__name__} -> {implementation.__name__}")

    def register_transient(self, interface: Type[T], implementation: Type[T]) -> None:
        """Register a transient service (new instance each time)."""
        self._services[interface] = implementation
        logger.debug(f"Registered transient service: {interface.__name__} -> {implementation.__name__}")
    ...
    async def _create_instance_with_factory(self, interface: Type[T]) -> T:
        ...
        try:
            ...
            logger.error(f"Error creating service instance with factory for {interface.__name__}: {e}")
            raise
    ...
    async def _create_instance(self, interface: Type[T]) -> T:
        ...
        try:
                ...
                if param.annotation != inspect.Parameter.empty:
                    try:
                        ...
                        if param.default == inspect.Parameter.empty:
                            logger.warning(f"Cannot resolve dependency {param.annotation} for {implementation.__name__}.{param_name}")
                        pass
            ...
            logger.error(f"Error creating service instance for {interface.__name__}: {e}")
            raise
            ...
            for interface, instance in self._singletons.items():
                if hasattr(instance, 'dispose'):
                    try:
                        ...
                        logger.error(f"Error disposing service {interface.__name__}: {e}")
