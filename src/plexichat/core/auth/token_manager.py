...
class TokenManager:
    ...
    async def validate_token(self, token: str) -> TokenValidationResult:
        ...
        try:
            ...
            public_key = self.public_key if self.public_key else b""
            payload = jwt.decode(
                token,
                public_key,
                algorithms=[self.algorithm],
                audience=["plexichat-api", "plexichat-auth"],
    ...
    async def blacklist_token(self, token: str):
        ...
        try:
            ...
            unverified_payload = jwt.decode(token, options={"verify_signature": False}) if jwt else {}
            token_id = unverified_payload.get("jti")
