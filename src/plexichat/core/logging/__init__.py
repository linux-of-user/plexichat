...
class CentralizedDirectoryManager:
    ...
    def _initialize_directories(self):
        ...
        try:
            ...
            logger.info(
                f"Initialized centralized directory structure at {self.base_dir}"
            )
    ...
    def get_directory(self, name: str) -> Path:
        ...
        if name not in self.directories:
            raise ValueError(
                f"Unknown directory: {name}. Available: {list(self.directories.keys())}"
            )
                ...
                for old_dir in self.base_dir.glob(pattern):
                    if old_dir.is_dir() and old_dir not in self.directories.values():
                        try:
                            ...
                            logger.warning(
                                f"Could not clean up directory {old_dir}: {e}"
                            )
...
class UnifiedLoggingManager:
    ...
    def __init__(self):
        ...
        self.log_buffer = LogBuffer(
            max_size=getattr(self.config.logging, "buffer_size", 10000)
        )
    ...
    def _setup_file_handler(self):
        ...
        try:
            ...
            max_file_size = getattr(
                self.config.logging, "max_file_size", 10 * 1024 * 1024
            )
            ...
            if isinstance(max_file_size, str):
                max_file_size = (
                    int(max_file_size) if max_file_size.isdigit() else 10 * 1024 * 1024
                )
        ...
        try:
            ...
            try:
                ...
                max_total_size_mb = getattr(
                    self.config.logging, "max_total_size_mb", 1000
                )
            ...
            for log_file in log_dir.glob("*.log*"):
                if log_file.is_file():
                    stat = log_file.stat()
                    log_files.append(
                        {"path": log_file, "mtime": stat.st_mtime, "size": stat.st_size}
                    )
    ...
    def _setup_structured_handler(self):
        ...
        try:
            ...
            structured_handler = RotatingFileHandler(
                structured_file, maxBytes=max_file_size, backupCount=backup_count
            )
...
def log_audit_event(event_type: str, user_id: Optional[str], details: Dict[str, Any]):
    ...
    logger.audit(
        f"Audit: {event_type}", user_id=user_id, event_type=event_type, details=details
    )
...
try:
    ...
    class EnterprisePerformanceLogger:
        ...
        def __init__(self, config: Optional[Dict[str, Any]] = None):
            ...
            self._collection_interval = self.config.get(
                "collection_interval_seconds", 30
            )
            self._alert_cooldown = self.config.get(
        ...
        def record_metric(
            ...
            logger.debug(
                f"Recorded {metric_type.value} metric '{name}': {value} {unit}"
            )
    ...
    class PerformanceTimer:
        ...
        def __init__(
            self, logger, operation_name: str, tags: Optional[Dict[str, str]] = None
        ):
            self.logger = logger
    ...
    def get_unified_performance_logger() -> Optional[EnterprisePerformanceLogger]:
    ...
    async def initialize_performance_logger(
        config: Optional[Dict[str, Any]] = None,
    ) -> Optional[EnterprisePerformanceLogger]:
        """Initialize and return the performance logger."""
        perf_logger = get_unified_performance_logger()
    ...
    def time_operation(
        operation_name: str, tags: Optional[Dict[str, str]] = None
    ) -> PerformanceTimer:
        """Create a performance timer context manager."""
        return PerformanceTimer(get_unified_performance_logger(), operation_name, tags)

except ImportError:
    # Fallback if performance dependencies are not available
    def get_fallback_performance_logger():
        """Fallback performance logger when dependencies are not available."""
        return None

    def get_performance_logger():
        return get_fallback_performance_logger()
