...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        log_message = self.sanitize_for_logging(log_message)
...
class StructuredFormatter(logging.Formatter):
    ...
    def __init__(self, sanitize_func=None):
        super().__init__(fmt='%(asctime)s %(levelname)s %(message)s %(module)s:%(lineno)d')
        self.sanitize_func = sanitize_func or lambda x: x
    ...
    def format(self, record):
        log_dict = {
            ...
            'message': self.sanitize_for_logging(record.getMessage()),
...
def get_logger(name: str = 'plexichat', level: str = 'INFO') -> logging.Logger:
    ...
    handlers = get_handler_factory(
        ...
        sanitize_func=sanitize_for_logging
    )
...
__all__ = ['get_logger', 'redact_pii', 'sanitize_for_logging', 'ColoredFormatter', 'StructuredFormatter', 'get_handler_factory']