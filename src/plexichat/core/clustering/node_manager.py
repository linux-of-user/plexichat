...
class NodeAuthenticator:
    ...
    async def authenticate_node(
        ...
        try:
            # Verify the token
            is_valid, payload = self.security_system.token_manager.verify_token(
                auth_token
            )
...
class HeartbeatManager:
    ...
    def __init__(
        self, node_config: NodeConfiguration, authenticator: NodeAuthenticator
    ):
        self.node_config = node_config
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        timeout=aiohttp.ClientTimeout(
                            total=self.node_config.health_check_timeout_seconds
                        ),
                    ) as response:
                        if response.status == 200:
                            ...
                            logger.warning(
                                f"Heartbeat failed to {endpoint}: {response.status}"
                            )
            except Exception as e:
...
class NodeManager:
    ...
    async def start(self, cluster_endpoints: Optional[List[str]] = None) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Generated authentication token for node {self.config.node_id}"
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.info(
                            f"Node {self.config.node_id} registered with cluster"
                        )
                        return True
                    else:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.delete(
                    ...
                    if response.status == 200:
                        logger.info(
                            f"Node {self.config.node_id} unregistered from cluster"
                        )
                        return True
                    else:
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_heartbeat": (
                self.node_info.last_heartbeat.isoformat()
                if self.node_info.last_heartbeat
                else None
            ),
    ...
    def add_status_change_callback(
        self, callback: Callable[[NodeStatus, NodeStatus], None]
    ) -> None:
    ...
    async def _update_status(self, new_status: NodeStatus) -> None:
        ...
        logger.info(
            f"Node {self.config.node_id} status changed: {old_status.value} -> {new_status.value}"
        )
    ...
    async def _monitoring_loop(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                if issues:
                    logger.warning(
                        f"Node {self.config.node_id} health issues: {', '.join(issues)}"
                    )
...
def create_networking_node(
    node_id: str, host: str = "localhost", port: int = 8000
) -> NodeManager:
