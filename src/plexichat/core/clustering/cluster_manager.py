...
class NodeMetrics:
    ...
    def health_score(self) -> float:
        ...
        # Weighted average
        return (cpu_score * 0.25 + memory_score * 0.25 + disk_score * 0.15 + 
                error_score * 0.25 + latency_score * 0.1)
...
class ClusterNode:
    ...
    def is_healthy(self) -> bool:
        ...
        if self.last_heartbeat:
            ...
            return heartbeat_age < 60  # Consider unhealthy if no heartbeat for 60 seconds
...
class ClusterManager:
    ...
    def __init__(self, config: Optional[ClusterConfiguration] = None):
        ...
        logger.info(f"Cluster Manager initialized for cluster: {self.config.cluster_id}")
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Cluster manager started successfully for cluster: {self.config.cluster_id}")
            return True
    ...
    async def get_healthy_nodes(self) -> List[ClusterNode]:
        ...
        return [self.nodes[node_id] for node_id in self.healthy_nodes if node_id in self.nodes]

    async def get_nodes_by_type(self, node_type: NodeType) -> List[ClusterNode]:
        """Get nodes by type."""
        return [node for node in self.nodes.values() if node.node_type == node_type]

    async def get_node_for_request(self, request_key: Optional[str] = None) -> Optional[ClusterNode]:
    ...
    async def get_cluster_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.nodes:
                avg_cpu = sum(node.metrics.cpu_usage for node in self.nodes.values()) / len(self.nodes)
                avg_memory = sum(node.metrics.memory_usage for node in self.nodes.values()) / len(self.nodes)
                avg_disk = sum(node.metrics.disk_usage for node in self.nodes.values()) / len(self.nodes)
                avg_latency = sum(node.metrics.network_latency for node in self.nodes.values()) / len(self.nodes)
                total_requests = sum(node.metrics.request_rate for node in self.nodes.values())
    ...
    async def scale_cluster(self, target_nodes: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if target_nodes > current_nodes:
                ...
                nodes_to_remove_list = await self._select_nodes_for_removal(nodes_to_remove)
            ...
            for node_id, node in self.nodes.items():
                try:
                    ...
                    if node.last_heartbeat:
                        heartbeat_age = (datetime.now(timezone.utc) - node.last_heartbeat).total_seconds()
                        ...
                        if heartbeat_age > self.config.heartbeat_timeout:
                            ...
                            # Node is responsive
                            if node_id not in self.healthy_nodes and node.metrics.health_score > 0.5:
    ...
    def _rebuild_hash_ring(self) -> None:
        ...
        try:
            ...
            logger.debug(f"Hash ring rebuilt with {len(self.consistent_hash_ring)} virtual nodes")
    ...
    async def _get_least_loaded_node(self) -> Optional[ClusterNode]:
        ...
        try:
            healthy_nodes = [self.nodes[node_id] for node_id in self.healthy_nodes if node_id in self.nodes]
    ...
    async def _select_nodes_for_removal(self, count: int) -> List[str]:
        ...
        try:
            ...
            unhealthy_nodes = [node_id for node_id in self.nodes if node_id not in self.healthy_nodes]
            healthy_nodes = list(self.healthy_nodes)
...
async def initialize_cluster_manager(config: Optional[ClusterConfiguration] = None) -> ClusterManager:
