...
class ClusterManager:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            try:
                from plexichat.core.performance.cache_cluster_manager import (
    ...
    async def register_node(
        ...
        try:
            ...
            if await self._test_node_connectivity(node):
                ...
                logger.info(
                    f"[CLUSTER] Registered node: {node_id} ({node_type.value}) at {ip_address}:{port}"
                )
                return node_id
            else:
                logger.error(
    ...
    def get_replica_nodes(
        ...
        if node_type:
            available_nodes = {
                node.node_id
                for node in self.get_nodes_by_type(node_type, healthy_only=True)
            }
        else:
        ...
        for i in range(1, replica_count):
            ...
            if replica_node_id in available_nodes and replica_node_id not in {
                node.node_id for node in replicas
            }:
    ...
    async def get_cluster_status(self) -> ClusterStatus:
        ...
        try:
            ...
            status.inactive_nodes = len(
                [n for n in self.nodes.values() if n.status == NodeStatus.INACTIVE]
            )
            ...
            if self.healthy_nodes:
                ...
                status.average_response_time = total_response_time / len(
                    self.healthy_nodes
                )
    ...
    async def scale_cluster(self, target_nodes: int) -> dict[str, Any]:
        ...
        logger.info(
            f"[CLUSTER] Scaling cluster from {current_nodes} to {target_nodes} nodes"
        )
    ...
    async def _check_failover_conditions(self):
        ...
        for node_type in [NodeType.NETWORKING, NodeType.ENDPOINT]:
            ...
            if not healthy_nodes:
                logger.warning(
                    f"[CLUSTER] No healthy {node_type.value} nodes available"
                )
    ...
    def _rebuild_hash_ring(self):
        ...
        logger.debug(
            f"[CLUSTER] Rebuilt hash ring with {len(self.hash_ring)} virtual nodes"
        )
...
class PerformanceMonitor:
    ...
    async def get_cluster_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            total_network = sum(
                node.metrics.network_in_mbps + node.metrics.network_out_mbps
                for node in nodes
            )
