    ...
    def generate_plugins_internal(self) -> bool:
        ...
        try:
            ...
            if self.jinja_env:
                ...
                plugins_internal_content = self._create_plugins_internal_template().replace(
                    "{{ generation_time }}", template_data["generation_time"]
                ).replace(
                    "{{ sdk_version }}", template_data["sdk_version"]
                ).replace(
            ...
            self.logger.info(f"plugins_internal.py generated successfully: {self.output_file}")
            
            return True
            
        except Exception as e:
            self.logger.error(f"Error generating plugins_internal.py: {e}", exc_info=True)
            return False
    ...
    def _update_gitignore(self) -> None:
        ...
        try:
            ...
            if updated:
                ...
                self.logger.info("Updated .gitignore to exclude generated plugins_internal files")
        ...
        try:
            ...
            if self.type_stubs_file.exists():
                ...
                stats["type_stubs"] = {
                    ...
                    "last_modified": datetime.fromtimestamp(stubs_stat.st_mtime).isoformat()
                }
