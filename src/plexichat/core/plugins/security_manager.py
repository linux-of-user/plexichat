...
try:
    from plexichat.shared.exceptions import SecurityError, PluginError, ValidationError
except ImportError:
...
class SafeFileManager:
    ...
    def open(self, filename: str, mode: str = 'r', **kwargs):
        ...
        if 'w' in mode or 'a' in mode or '+' in mode:
            if not self.security_manager.has_permission(self.plugin_name, PermissionType.FILE_WRITE):
                raise PermissionError(f"Plugin {self.plugin_name} does not have file write permission")
        else:
            if not self.security_manager.has_permission(self.plugin_name, PermissionType.FILE_READ):
                raise PermissionError(f"Plugin {self.plugin_name} does not have file read permission")
        
        # Use the existing file manager for controlled access
        file_path = Path(filename).resolve()
        
        # Ensure path is within plugin's allowed directories
        plugin_data_dir = Path(__file__).resolve().parents[4] / "plugins" / self.plugin_name / "data"
        plugin_data_dir.mkdir(parents=True, exist_ok=True)
        
        try:
            file_path.relative_to(plugin_data_dir)
        except ValueError:
            raise PermissionError(f"Access to '{file_path}' is not allowed. Use plugin data directory: {plugin_data_dir}")
    ...
    async def upload_file(self, file_data: bytes, filename: str, **kwargs):
        ...
        if file_manager:
            ...
            return await file_manager.upload_file(file_data, filename, uploaded_by=-1, **kwargs)
        else:
...
class NetworkBroker:
    ...
    def _check_network_permission(self):
        ...
        if not self.security_manager.has_permission(self.plugin_name, PermissionType.NETWORK_ACCESS):
            raise PermissionError(f"Plugin {self.plugin_name} does not have network access permission")
    ...
    async def get(self, url: str, **kwargs):
        ...
        if hasattr(self.security_manager, '_sandboxes') and self.plugin_name in self.security_manager._sandboxes:
            sandbox = self.security_manager._sandboxes[self.plugin_name]
            if hasattr(sandbox, 'network_monitor'):
                from urllib.parse import urlparse
                parsed = urlparse(url)
                sandbox.network_monitor.log_connection(parsed.hostname or 'unknown', parsed.port or 80, 'http')
...
class SecureSandbox:
    ...
    def _restricted_open(self, filename: str, mode: str = 'r', *args, **kwargs):
        ...
        raise SecurityError("Direct file access not allowed. Use the provided safe_file_manager instead.")

    def _restricted_exec(self, *args, **kwargs):
        """Restricted exec function - completely disabled."""
        self._log_security_violation(
            "Attempted to use exec() function",
    ...
    def _restricted_eval(self, *args, **kwargs):
        ...
        self._log_security_violation(
            "Attempted to use eval() function",
            SecurityThreatLevel.CRITICAL
        )
    ...
    def _restricted_compile(self, *args, **kwargs):
        ...
        self._log_security_violation(
            "Attempted to use compile() function",
            SecurityThreatLevel.HIGH
        )
    ...
    def _restricted_input(self, prompt: str = ""):
        ...
        self._log_security_violation(
            f"Attempted to request user input: {prompt}",
            SecurityThreatLevel.MEDIUM
        )
    ...
    def _log_security_violation(self, description: str, threat_level: SecurityThreatLevel):
        """Log a security violation."""
        event = SecurityAuditEvent(
            event_id=hashlib.sha256(f"{self.plugin_name}{description}{time.time()}".encode()).hexdigest()[:16],
    ...
    def get_safe_apis(self) -> Dict[str, Any]:
        ...
        return {
            'safe_file_manager': SafeFileManager(self.plugin_name, self._security_manager),
...
class ResourceMonitor:
    ...
    def _check_violations(self, usage: ResourceUsage):
        ...
        if usage.cpu_percent > self.security_policy.max_cpu_percent:
            violations.append(f"CPU usage ({usage.cpu_percent:.1f}%) exceeds limit ({self.security_policy.max_cpu_percent:.1f}%)")
        
        if usage.memory_bytes > self.security_policy.max_memory_bytes:
            violations.append(f"Memory usage ({usage.memory_bytes / 1024 / 1024:.1f}MB) exceeds limit ({self.security_policy.max_memory_bytes / 1024 / 1024:.1f}MB)")
        
        if usage.file_handles > self.security_policy.max_file_handles:
            violations.append(f"File handles ({usage.file_handles}) exceed limit ({self.security_policy.max_file_handles})")
        
        if usage.thread_count > self.security_policy.max_thread_count:
            violations.append(f"Thread count ({usage.thread_count}) exceeds limit ({self.security_policy.max_thread_count})")
        ...
        if violations:
            ...
            for violation in violations:
                ...
                if hasattr(self, '_security_manager'):
                    event = SecurityAuditEvent(
                        event_id=hashlib.sha256(f"{self.plugin_name}{violation}{time.time()}".encode()).hexdigest()[:16],
                        plugin_name=self.plugin_name,
...
class FileAccessMonitor:
    ...
    def get_access_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'most_accessed_files': dict(sorted(file_counts.items(), key=lambda x: x[1], reverse=True)[:10]),
...
class NetworkMonitor:
    ...
    def check_connection_allowed(self, host: str, port: int) -> bool:
        ...
        if len(self._active_connections) >= self.security_policy.max_network_connections:
            self.logger.warning(f"Network connection limit exceeded: {len(self._active_connections)}")
            return False
    ...
    def get_connection_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'most_contacted_hosts': dict(sorted(host_counts.items(), key=lambda x: x[1], reverse=True)[:10]),
            'most_used_ports': dict(sorted(port_counts.items(), key=lambda x: x[1], reverse=True)[:10]),
            'last_connection': recent_connections[-1]['timestamp'] if recent_connections else None,
        }
...
class PluginSecurityManager:
    ...
    async def _init_database(self):
        ...
        if not database_manager:
            self.logger.warning("Database manager not available, using in-memory storage")
            return
    ...
    async def _create_tables_manually(self):
        ...
        try:
            ...
            await database_manager.ensure_table_exists("plugin_permissions", permissions_schema)
            ...
            await database_manager.ensure_table_exists("plugin_audit_events", audit_schema)
    ...
    def request_permission(self, plugin_name: str, permission_type: PermissionType, 
                          justification: str, additional_data: Optional[Dict[str, Any]] = None) -> str:
        ...
        with self._lock:
            ...
            # Log audit event
            asyncio.create_task(self.log_audit_event(SecurityAuditEvent(
                event_id=hashlib.sha256(f"{plugin_name}{permission_type.value}{time.time()}".encode()).hexdigest()[:16],
                plugin_name=plugin_name,
            ...
            self.logger.info(f"Permission requested: {plugin_name} -> {permission_type.value} (ID: {request_id})")
            return request_id

    def approve_permission(self, plugin_name: str, permission_type: PermissionType, 
                          approved_by: str, expires_in_days: Optional[int] = None) -> bool:
        """Approve a permission request."""
        with self._lock:
            # Find the request
            requests = self._permission_requests.get(plugin_name, [])
            for request in requests:
                if (request.permission_type == permission_type and 
                    ...
                    # Log audit event
                    asyncio.create_task(self.log_audit_event(SecurityAuditEvent(
                        event_id=hashlib.sha256(f"{plugin_name}{permission_type.value}approved{time.time()}".encode()).hexdigest()[:16],
                        plugin_name=plugin_name,
                    ...
                    self.logger.info(f"Permission approved: {plugin_name} -> {permission_type.value} by {approved_by}")
                    return True
            
            return False

    def deny_permission(self, plugin_name: str, permission_type: PermissionType, denied_by: str) -> bool:
        ...
        with self._lock:
            ...
            for request in requests:
                if (request.permission_type == permission_type and 
                    ...
                    # Log audit event
                    asyncio.create_task(self.log_audit_event(SecurityAuditEvent(
                        event_id=hashlib.sha256(f"{plugin_name}{permission_type.value}denied{time.time()}".encode()).hexdigest()[:16],
                        plugin_name=plugin_name,
                    ...
                    self.logger.info(f"Permission denied: {plugin_name} -> {permission_type.value} by {denied_by}")
                    return True
            
            return False

    def revoke_permission(self, plugin_name: str, permission_type: PermissionType, revoked_by: str) -> bool:
        ...
        with self._lock:
            if permission_type in self._approved_permissions[plugin_name]:
                ...
                # Log audit event
                asyncio.create_task(self.log_audit_event(SecurityAuditEvent(
                    event_id=hashlib.sha256(f"{plugin_name}{permission_type.value}revoked{time.time()}".encode()).hexdigest()[:16],
                    plugin_name=plugin_name,
                ...
                self.logger.warning(f"Permission revoked: {plugin_name} -> {permission_type.value} by {revoked_by}")
                return True
                ...
                for request in requests:
                    if (request.permission_type == permission_type and 
                        ...
                        if request.expires_at and datetime.now(timezone.utc) > request.expires_at:
                            ...
                            self._approved_permissions[plugin_name].discard(permission_type)
                            return False
                        return True
    ...
    def quarantine_plugin(self, plugin_name: str, reason: str, quarantined_by: str):
        ...
        with self._lock:
            ...
            # Log audit event
            asyncio.create_task(self.log_audit_event(SecurityAuditEvent(
                event_id=hashlib.sha256(f"{plugin_name}quarantined{time.time()}".encode()).hexdigest()[:16],
                plugin_name=plugin_name,
    ...
    def release_from_quarantine(self, plugin_name: str, released_by: str):
        ...
        with self._lock:
            if plugin_name in self._quarantined_plugins:
                ...
                self.logger.warning(f"Plugin released from quarantine: {plugin_name} by {released_by}")
                return True
    ...
    async def log_audit_event(self, event: SecurityAuditEvent):
        ...
        with self._lock:
            ...
            # Log to file as well
            self.logger.info(f"AUDIT: {event.event_type.value} - {event.plugin_name} - {event.description}")
    ...
    async def approve_module(self, plugin_name: str, module_name: str, approved_by: str, expires_in_days: Optional[int] = None) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Module approved: {plugin_name} -> {module_name} by {approved_by}")
            return True
    ...
    async def revoke_module(self, plugin_name: str, module_name: str, revoked_by: str) -> bool:
        ...
        try:
            ...
            self.logger.warning(f"Module approval revoked: {plugin_name} -> {module_name} by {revoked_by}")
            return True
        ...
        try:
            async with database_manager.get_session() as session:
                query = """
                    ...
                    AND (expires_at IS NULL OR expires_at > :now)
                """
                rows = await session.fetchall(query, {
                    "plugin_name": plugin_name,
    ...
    def get_pending_permission_requests(self) -> List[PermissionRequest]:
        ...
        with self._lock:
            ...
            for plugin_requests in self._permission_requests.values():
                pending.extend([r for r in plugin_requests if r.status == PermissionStatus.PENDING])
            return pending
    ...
    def get_plugin_permissions(self, plugin_name: str) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                'approved_permissions': [p.value for p in self._approved_permissions[plugin_name]],
                'pending_requests': [
                    {
    ...
    def get_security_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            recent_events = [e for e in self._audit_events if e.timestamp > datetime.now(timezone.utc) - timedelta(hours=24)]
    ...
    def cleanup_expired_permissions(self):
        ...
        with self._lock:
            ...
            for plugin_name, requests in self._permission_requests.items():
                for request in requests:
                    if (request.status == PermissionStatus.APPROVED and 
                        request.expires_at and current_time > request.expires_at):
                        
                        request.status = PermissionStatus.EXPIRED
                        self._approved_permissions[plugin_name].discard(request.permission_type)
...
def request_plugin_permission(plugin_name: str, permission_type: PermissionType, 
                             justification: str) -> str:
    """Request a permission for a plugin."""
    return plugin_security_manager.request_permission(plugin_name, permission_type, justification)
