...
class UnifiedAuthManager:
    ...
    def _parse_user_agent(self, user_agent: Optional[str]) -> DeviceInfo:
        ...
        if not user_agent:
            return DeviceInfo(
                device_id=secrets.token_hex(8), device_type=DeviceType.UNKNOWN
            )
        ...
        if any(
            ...
            tablet in user_agent_lower for tablet in ["tablet", "kindle", "playbook"]
        ):
            device_type = DeviceType.TABLET
        elif any(
            ...
            async with self.db_manager.get_session() as session:
                ...
                if expired_sessions_result:
                    ...
                    for row in expired_sessions_result:
                        ...
                        logger.security(
                            "Session expired and marked inactive",
                            session_id=row["id"],
                            user_id=row["user_id"],
                        )
                        logger.info(
                ...
                if expired_challenges_result:
                    ...
                    for row in expired_challenges_result:
                        logger.security(
                            "MFA challenge expired and removed",
    ...
    async def _calculate_risk_score(
        self, ip_address: str, device_info: DeviceInfo, user_id: str
    ) -> float:
    ...
    async def _is_device_trusted(self, device_id: str) -> bool:
        ...
        try:
            async with self.db_manager.get_session() as session:
                result = await session.fetchone(
                    "SELECT is_trusted FROM devices WHERE device_id = ?", (device_id,)
                )
    ...
    def _check_brute_force_protection(
        ...
        if protection.is_blocked and protection.blocked_until:
            if current_time < protection.blocked_until:
                ...
                return (
                    False,
                    f"IP blocked for {remaining.total_seconds():.0f} more seconds",
                )
            else:
                # Unblock IP
        ...
        try:
            ...
            if self.password_policy.require_special_chars:
                ...
                if len(special_chars) < self.password_policy.min_special_chars:
                    issues.append(
                        f"Password must contain at least {self.password_policy.min_special_chars} special characters"
                    )
            ...
            if complexity_score < self.password_policy.complexity_score_threshold:
                issues.append(
                    f"Password complexity score {complexity_score} is below required {self.password_policy.complexity_score_threshold}"
                )

            if issues:
                self._record_metric("password_policy_violations")
                logger.security(
                    "Password policy violation", user_id=user_id, violations=issues
                )
    ...
    async def create_mfa_challenge(
        ...
        try:
            ...
            challenge = MFAChallenge(
                challenge_id=challenge_id, user_id=user_id, method=method
            )
            ...
            if method == MFAMethod.TOTP:
                ...
                logger.info(
                    f"MFA email code generated for user {user_id}: {challenge.code}"
                )
                ...
                logger.info(
                    f"MFA SMS code generated for user {user_id}: {challenge.code}"
                )
        ...
        try:
            async with self.db_manager.get_session() as db_session:
                ...
                if not challenge_result:
                    logger.security(
                        "MFA verification failed - challenge not found",
                        challenge_id=challenge_id,
                    )
                    return False
                ...
                if attempts > challenge_result["max_attempts"]:
                    ...
                    logger.security(
                        "MFA verification failed - too many attempts",
                        challenge_id=challenge_id,
                        user_id=challenge_result["user_id"],
                        attempts=attempts,
                    )
                ...
                if method == MFAMethod.TOTP:
                    ...
                    if self.mfa_store and hasattr(self.mfa_store, "verify_totp"):
                        verified = await self.mfa_store.verify_totp(
                            challenge_result["user_id"], user_code
                        )
                    ...
                    if self.mfa_store and hasattr(self.mfa_store, "verify_backup_code"):
                        verified = await self.mfa_store.verify_backup_code(
                            challenge_result["user_id"], user_code
                        )
            ...
            logger.error(
                f"Error verifying MFA challenge: {e}", challenge_id=challenge_id
            )
            return False

    def configure_oauth2_provider(
        self, provider: AuthProvider, config: OAuth2Config
    ) -> None:
    ...
    async def authenticate_oauth2(
        ...
        try:
            ...
            if user_id not in self.security_system.user_credentials:
                ...
                self.register_user(
                    user_id, secrets.token_urlsafe(32), {"read", "write_own"}
                )
    ...
    async def authenticate_user(
        ...
        if not username or not username.strip():
            logger.security(
                "Authentication failed - empty username", ip_address=ip_address
            )
        ...
        if ip_address:
            device_info.device_id = self._generate_device_id(
            ...
            async with self.db_manager.get_session() as db_session:
                ...
                if stored_device_result:
                    ...
                    await db_session.execute(
                        ...
                        {
                            "1": device_info.last_seen.isoformat(),
                            "2": device_info.device_id,
                        },
                    )
                    await db_session.commit()
        except Exception as e:
        ...
        try:
            ...
            allowed, block_message = self._check_brute_force_protection(
            ...
            if not allowed:
                ...
                logger.security(
                    "Authentication blocked by brute force protection",
                    user_id=username,
                    ip_address=ip_address,
                )
                logger.info(
            ...
            success, security_context = await self.security_system.authenticate_user(
                username, password
            )
            ...
            risk_score = await self._calculate_risk_score(
                ip_address or "", device_info, username
            )
            ...
            requires_mfa = (
                risk_score > 50.0  # High risk
                or not device_info.is_trusted  # Unknown device
                or "admin" in all_permissions  # Admin access
                or not device_trust  # User didn't request device trust
            )
            ...
            if mfa_code:
                ...
                try:
                    async with self.db_manager.get_session() as db_session:
                        ...
                        if not challenge_result:
                            logger.security(
                                "No active MFA challenge found", user_id=username
                            )
                            return AuthResult(
                                success=False,
                        ...
                        if method == MFAMethod.TOTP:
                            # Verify TOTP code
                            if self.mfa_store and hasattr(
                                self.mfa_store, "verify_totp"
                            ):
                                mfa_verified = await self.mfa_store.verify_totp(
                                    username, mfa_code
                                )
                        elif method in [MFAMethod.EMAIL, MFAMethod.SMS]:
                            # Verify generated code
                            mfa_verified = hmac.compare_digest(
                                stored_code or "", mfa_code
                            )
                        elif method == MFAMethod.BACKUP_CODES:
                            # Verify backup code
                            if self.mfa_store and hasattr(
                                self.mfa_store, "verify_backup_code"
                            ):
                                mfa_verified = await self.mfa_store.verify_backup_code(
                                    username, mfa_code
                                )
            ...
            if self.auth_cache:
                try:
                    await self.auth_cache.cache_token_verification(
                        access_token,
                        (
                            True,
                            {
            ...
            logger.error(
                f"Authentication error: {e}", user_id=username, ip_address=ip_address
            )
            self._record_metric("failed_authentications")
            duration = (time.time() - start_time) * 1000
    ...
    async def validate_token(self, token: str) -> Tuple[bool, Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            if self.auth_cache:
                try:
                    cached_result = await self.auth_cache.get_cached_token_verification(
                        token
                    )
            ...
            if valid and payload:
                ...
                logger.performance(
                    "token_validation", duration / 1000, cache_hit=False, success=False
                )
            ...
            duration = (time.time() - start_time) * 1000
            logger.performance(
                "token_validation", duration / 1000, success=False, error=str(e)
            )
            return False, None
    ...
    async def validate_session(
        ...
        try:
            async with self.db_manager.get_session() as db_session:
                ...
                permissions = set(
                    json.loads(session_result["permissions"])
                    if session_result["permissions"]
                    else []
                )
                roles = set(
                    json.loads(session_result["roles"])
                    if session_result["roles"]
                    else []
                )
                device_info = (
                    json.loads(session_result["device_info"])
                    if session_result["device_info"]
                    else None
                )
                ...
                session = SessionInfo(
                    ...
                    elevation_expires_at=(
                        datetime.fromisoformat(session_result["elevation_expires_at"])
                        if session_result["elevation_expires_at"]
                        else None
                    ),
                ...
                if self._is_session_expired(session):
                    ...
                    self._record_metric("session_invalidations")
                    logger.security(
                        "Session expired and invalidated",
                        session_id=session_id,
                        user_id=session.user_id,
                    )
                    logger.info(
    ...
    async def validate_api_key(self, api_key: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if self.auth_cache:
                try:
                    cached_result = await self.auth_cache.get_cached_api_key_validation(
                        api_key
                    )
                    if cached_result and cached_result.is_valid:
                        self._record_metric("cache_hits")
                        duration = (time.time() - start_time) * 1000
                        self._record_metric(
                            "api_key_validation_time_ms", duration, "ms"
                        )
            ...
            for username, credentials in self.security_system.user_credentials.items():
                if api_key in credentials.api_keys:
                    ...
                    if self.auth_cache:
                        try:
                            await self.auth_cache.cache_api_key_validation(
                                api_key,
                                (
                                    True,
                                    {
            ...
            logger.performance(
                "api_key_validation", duration / 1000, cache_hit=False, success=False
            )
            ...
            duration = (time.time() - start_time) * 1000
            logger.performance(
                "api_key_validation", duration / 1000, success=False, error=str(e)
            )
            return None
    ...
    def create_access_token(
        ...
        try:
            ...
            logger.error(
                f"Error creating access token: {e}", user_id=user_id or "unknown"
            )
            return ""
    ...
    def create_refresh_token(self, user_id: str) -> str:
        ...
        try:
            ...
            logger.error(
                f"Error creating refresh token: {e}", user_id=user_id or "unknown"
            )
            return ""
    ...
    async def invalidate_session(self, session_id: str) -> bool:
        ...
        try:
            async with self.db_manager.get_session() as db_session:
                ...
                if not session_result:
                    logger.debug(
                        "Session not found or already inactive", session_id=session_id
                    )
                    return False
    ...
    def register_user(
        ...
        try:
            ...
            if issues:
                logger.warning(
                    "User registration failed - password validation",
                    user_id=username,
    ...
    async def elevate_session(self, session_id: str, password: str) -> bool:
        ...
        try:
            async with self.db_manager.get_session() as db_session:
                ...
                if not session_result:
                    logger.security(
                        "Session elevation failed - session not found or inactive",
                        session_id=session_id,
                    )
                    return False
                ...
                if (
                    session_result["is_elevated"]
                    and session_result["elevation_expires_at"]
                ):
                ...
                if not success:
                    logger.security(
                        "Session elevation failed - invalid password",
                        session_id=session_id,
                        user_id=user_id,
                    )
                    return False
    ...
    async def check_permission(
        self, user_id: str, permission: str, require_elevation: bool = False
    ) -> bool:
        """
            ...
            if require_elevation:
                ...
                async with self.db_manager.get_session() as db_session:
                    ...
                    if not elevated_session_result:
                        logger.security(
                            "Permission denied - elevation required",
                            user_id=user_id,
    ...
    def get_user_permissions(self, user_id: str) -> Set[str]:
        ...
        try:
            ...
            logger.error(
                f"Error getting user permissions for {user_id}: {e}", user_id=user_id
            )
            return set()
    ...
    def update_user_permissions(self, user_id: str, permissions: Set[str]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error updating user permissions for {user_id}: {e}", user_id=user_id
            )
            return False
    ...
    async def get_security_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.db_manager.get_session() as db_session:
                ...
                active_sessions = (
                    active_sessions_result["count"] if active_sessions_result else 0
                )
                ...
                elevated_sessions = (
                    elevated_sessions_result["count"] if elevated_sessions_result else 0
                )
                ...
                mfa_verified_sessions = (
                    mfa_verified_sessions_result["count"]
                    if mfa_verified_sessions_result
                    else 0
                )
                ...
                high_risk_sessions = (
                    high_risk_sessions_result["count"]
                    if high_risk_sessions_result
                    else 0
                )
                ...
                known_devices = (
                    known_devices_result["count"] if known_devices_result else 0
                )
                ...
                trusted_devices = (
                    trusted_devices_result["count"] if trusted_devices_result else 0
                )
                ...
                active_mfa_challenges = (
                    ...
                    if active_mfa_challenges_result
                    else 0
                )
            ...
            return {
                **security_status,
                ...
                "blocked_ips": len(
                    [bf for bf in self.brute_force_tracking.values() if bf.is_blocked]
                ),
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            async with self.db_manager.get_session() as db_session:
                ...
                initial_sessions = (
                    initial_sessions_result["count"] if initial_sessions_result else 0
                )
                ...
                initial_challenges = (
                    initial_challenges_result["count"]
                    if initial_challenges_result
                    else 0
                )
                ...
                if old_devices_result:
                    ...
                    # Delete old devices
                    await db_session.execute(
                        "DELETE FROM devices WHERE last_seen < ?", (old_devices_cutoff,)
                    )
            ...
            old_tracking = [
                ip
                for ip, protection in self.brute_force_tracking.items()
            ...
            async with self.db_manager.get_session() as db_session:
                ...
                final_sessions = (
                    final_sessions_result["count"] if final_sessions_result else 0
                )
                ...
                final_challenges = (
                    final_challenges_result["count"] if final_challenges_result else 0
                )
            ...
            logger.info(
                "Enhanced UnifiedAuthManager cleanup completed",
