...
class DatabaseSetupWizard:
    ...
    def set_connection_details(self, details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if config.db_type == DatabaseType.SQLITE:
                config.file_path = details.get("file_path", "data/netlink.db")
            else:
                config.host = details.get("host", "localhost")
                config.port = details.get("port", config.port)
                config.database = details.get("database", "netlink")
    ...
    async def _test_sqlite_connection(self, config: DatabaseConnection) -> Dict[str, Any]:
        ...
        try:
            ...
            db_path = Path(config.file_path or "data/netlink.db")
    ...
    async def initialize_schema(self, options: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            from netlink.app.models.guild import Guild
    ...
    async def _create_sample_data(self, engine) -> bool:
        ...
        try:
            ...
            with Session(engine) as session:
                ...
                sample_guild = Guild(
                    name="General",
                    description="Default server for NetLink",
                ...
                welcome_message = Message(
                    content="Welcome to NetLink! This is a sample message.",
                    author_id=1,
                    guild_id=1
                )
    ...
    def _get_next_steps(self) -> List[str]:
        ...
        if self.progress.current_step == SetupStep.COMPLETE:
            return [
                "Restart the NetLink application to use the new database",
