...
class EndpointDiscovery:
        ...
        for path, method in common_patterns:
            ...
            if await self._test_endpoint_exists(path, method):
                endpoint = EndpointInfo(
                    ...
                    requires_auth=(
                        True if "admin" in path or path.endswith("/me") else False
                    ),
                )
                endpoints.append(endpoint)
    ...
    async def _test_endpoint_exists(self, path: str, method: str) -> bool:
        ...
        try:
            url = urljoin(
                self.base_url, path.replace("{id}", "1")
...
class ComprehensiveEndpointTester:
    ...
    def __init__(self, base_url: str):
        ...
        self.test_data_templates = {
            ...
            "user_login": {"username": "test_user", "password": "TestPassword123!"},
            "message_creation": {
    ...
    async def _setup_authentication(self):
        ...
        try:
            ...
            async with self.test_session.post(
                login_url, json=self.test_user_credentials
            ) as response:
                if response.status == 200:
                    ...
                    logger.warning(
                        "Authentication setup failed, some tests may be skipped"
                    )
    ...
    async def _test_endpoint_functionality(self, endpoint: EndpointInfo) -> TestResult:
        ...
        try:
            ...
            if endpoint.method == "GET":
                ...
                assert self.test_session is not None
                async with self.test_session.post(
                    url, json=test_data, headers=headers
                ) as response:
                    response_data = await self._safe_json_response(response)
                    test_result.response_status = response.status
                    test_result.response_data = response_data
                ...
                assert self.test_session is not None
                async with self.test_session.put(
                    url, json=test_data, headers=headers
                ) as response:
                    response_data = await self._safe_json_response(response)
                    test_result.response_status = response.status
            ...
            if test_result.response_status and 200 <= test_result.response_status < 300:
                ...
                test_result.response_status == 401
                and endpoint.requires_auth
                and not self.auth_token
            ):
                test_result.finish(
                    TestStatus.SKIPPED, "Authentication required but not available"
                )
            else:
                test_result.finish(
                    TestStatus.FAILED,
                    f"Unexpected status code: {test_result.response_status}",
                )
    ...
    async def _test_sql_injection(self, endpoint: EndpointInfo) -> TestResult:
        ...
        try:
            ...
            if test_result.response_status == 400:
                ...
                test_result.security_issues.append(
                    "Potential SQL injection vulnerability"
                )
                test_result.finish(
                    TestStatus.FAILED, "SQL injection not properly blocked"
                )
    ...
    async def _test_xss_vulnerability(self, endpoint: EndpointInfo) -> TestResult:
        ...
        try:
            ...
            if endpoint.method == "POST":
                ...
                test_result.finish(
                    TestStatus.SKIPPED, "XSS test not applicable for this method"
                )
    ...
    async def _test_authentication_bypass(self, endpoint: EndpointInfo) -> TestResult:
        ...
        try:
            if not endpoint.requires_auth:
                test_result.finish(
                    TestStatus.SKIPPED, "Endpoint does not require authentication"
                )
                return test_result
            ...
            if test_result.response_status == 401:
                test_result.finish(
                    TestStatus.PASSED, "Authentication properly enforced"
                )
            else:
                test_result.security_issues.append(
                ...
                test_result.finish(
                    TestStatus.FAILED, "Authentication not properly enforced"
                )
    ...
    async def _test_endpoint_performance(self, endpoint: EndpointInfo) -> TestResult:
        ...
        try:
            ...
            if avg_response_time < 100:  # < 100ms is excellent
                test_result.finish(
                    TestStatus.PASSED,
                    f"Excellent performance: {avg_response_time:.2f}ms avg",
                )
            elif avg_response_time < 500:  # < 500ms is acceptable
                test_result.finish(
                    TestStatus.PASSED,
                    f"Good performance: {avg_response_time:.2f}ms avg",
                )
            elif avg_response_time < 1000:  # < 1s is concerning
                test_result.performance_issues.append(
                    f"Slow response time: {avg_response_time:.2f}ms"
                )
                test_result.finish(
                    TestStatus.FAILED,
                    f"Slow performance: {avg_response_time:.2f}ms avg",
                )
            else:  # > 1s is poor
                test_result.performance_issues.append(
                    f"Very slow response time: {avg_response_time:.2f}ms"
                )
                test_result.finish(
                    TestStatus.FAILED,
                    f"Poor performance: {avg_response_time:.2f}ms avg",
                )
    ...
    async def _test_input_validation(self, endpoint: EndpointInfo) -> TestResult:
        ...
        try:
            if endpoint.method in ["GET", "DELETE"]:
                test_result.finish(
                    TestStatus.SKIPPED, "No input validation needed for this method"
                )
                return test_result
            ...
            if validation_passed:
                test_result.finish(
                    TestStatus.PASSED, "Input validation working correctly"
                )
            else:
                test_result.finish(
                    TestStatus.FAILED, "Input validation not working properly"
                )
    ...
    def _generate_test_report(
        ...
        for test_type in TestType:
            ...
            results_by_type[test_type.value] = {
                ...
                "skipped": sum(
                    1 for r in type_results if r.status == TestStatus.SKIPPED
                ),
            }
        ...
        performance_results = [
            ...
            if r.test_type == TestType.PERFORMANCE and r.response_time_ms > 0
        ]
        avg_response_time = (
            sum(r.response_time_ms for r in performance_results)
