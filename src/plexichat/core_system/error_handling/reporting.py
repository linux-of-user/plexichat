...
class ConsoleReportingBackend(ReportingBackend):
    ...
    async def report(self, context: ErrorContext) -> bool:
        ...
        try:
            ...
            logger.info(f"{color}[ERROR] {context.severity.value if context.severity else 'UNKNOWN'}{reset_color}")
            logger.info(f"ID: {context.error_id}")
            logger.info(f"Message: {context.exception or context.message}")
            if context.component:
                logger.info(f"Component: {context.component}")
            if context.operation:
                logger.info(f"Operation: {context.operation}")
            logger.info("-" * 50)
