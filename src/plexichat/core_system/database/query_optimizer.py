...
class SQLQueryAnalyzer:
    ...
    def __init__(self):
        self.query_patterns = {
            ...
            'like_wildcard': re.compile(r'\bLIKE\s+[\'"][%_].*[%_][\'"]', re.IGNORECASE),
        ...
        self.anti_patterns = {
            'function_in_where': re.compile(r'\bWHERE\s+\w+\([^)]*\)\s*[=<>]', re.IGNORECASE),
    ...
    def analyze_query(self, query: str) -> QueryAnalysis:
        ...
        analysis = QueryAnalysis(
            ...
            has_wildcards=bool(self.query_patterns['like_wildcard'].search(query_clean)),
            ...
            uses_select_star=bool(self.query_patterns['select_star'].search(query_clean)),
        ...
        analysis.optimization_suggestions = self._generate_suggestions(analysis, query_clean)
        
        return analysis
    
    def optimize_query(self, query: str, optimization_level: OptimizationLevel = OptimizationLevel.INTERMEDIATE) -> QueryOptimizationResult:
        ...
        if optimization_level in [OptimizationLevel.BASIC, OptimizationLevel.INTERMEDIATE, OptimizationLevel.ADVANCED, OptimizationLevel.AGGRESSIVE]:
            
            # 1. Replace SELECT * with specific columns (if we can determine them)
            if analysis.uses_select_star and optimization_level != OptimizationLevel.BASIC:
                warnings.append("Consider replacing SELECT * with specific column names for better performance")
            ...
            if analysis.query_type == QueryType.SELECT and not analysis.has_limit and optimization_level in [OptimizationLevel.ADVANCED, OptimizationLevel.AGGRESSIVE]:
    ...
    def _extract_tables(self, query: str) -> List[str]:
        ...
        for pattern in [from_pattern, join_pattern, update_pattern, insert_pattern, delete_pattern]:
    ...
    def _generate_suggestions(self, analysis: QueryAnalysis, query: str) -> List[str]:
        ...
        if analysis.has_wildcards and self.anti_patterns['leading_wildcard'].search(query):
            suggestions.append("Avoid leading wildcards in LIKE clauses; consider full-text search or range queries")
        
        if analysis.has_subqueries and self.query_patterns['in_subquery'].search(query):
            suggestions.append("Consider replacing IN subqueries with EXISTS or JOINs for better performance")
        
        if not analysis.has_limit and analysis.query_type == QueryType.SELECT:
            suggestions.append("Add LIMIT clause to restrict the number of rows returned")
        
        if self.anti_patterns['function_in_where'].search(query):
            suggestions.append("Avoid using functions in WHERE clauses; consider computed columns or indexes")
        
        if self.anti_patterns['or_conditions'].search(query):
            suggestions.append("Consider rewriting OR conditions as separate queries with UNION for better index usage")
        
        if analysis.complexity_score > 5.0:
            suggestions.append("Query complexity is high; consider breaking into smaller queries or using stored procedures")
    ...
    def _optimize_like_clauses(self, query: str) -> Tuple[str, List[str]]:
        ...
        leading_wildcard_pattern = re.compile(r'\b(\w+)\s+LIKE\s+[\'"]%([^%_]*)[\'"]', re.IGNORECASE)
        ...
        for column, value in matches:
            if len(value) > 2:  # Only optimize if we have meaningful content
                ...
                optimizations.append(f"Suggested optimization for LIKE clause: {suggestion}")
    ...
    def _optimize_in_subqueries(self, query: str) -> Tuple[str, List[str]]:
        ...
        in_subquery_pattern = re.compile(r'\b(\w+)\s+IN\s*\(\s*SELECT\s+(\w+)\s+FROM\s+(\w+)([^)]*)\)', re.IGNORECASE)
    ...
    def _suggest_indexes(self, analysis: QueryAnalysis, query: str) -> List[IndexSuggestion]:
        ...
        join_pattern = re.compile(r'\bON\s+(\w+)\.(\w+)\s*=\s*(\w+)\.(\w+)', re.IGNORECASE)
        ...
        for table in analysis.tables_accessed:
            table_where_columns = [col for col in where_columns if col in analysis.columns_accessed]
            if table_where_columns:
                suggestions.append(IndexSuggestion(
                    table=table,
                    columns=table_where_columns[:3],  # Limit to 3 columns for composite index
                    index_type="btree",
                    priority=1,
...
class NoSQLQueryOptimizer:
    ...
    def analyze_access_pattern(self, collection: str, query: Dict[str, Any]) -> Dict[str, Any]:
...
class QueryPerformanceMonitor:
    ...
    def record_query_execution(self, query: str, execution_time_ms: float, 
                             rows_returned: int = 0, rows_examined: int = 0):
        ...
        if execution_time_ms > self.slow_query_threshold:
            ...
            self.slow_queries = [q for q in self.slow_queries if q["timestamp"] > cutoff]
    
    def get_performance_report(self) -> Dict[str, Any]:
        """Generate performance report."""
        total_queries = sum(stats["count"] for stats in self.query_stats.values())
        avg_response_time = sum(stats["avg_time"] for stats in self.query_stats.values()) / len(self.query_stats) if self.query_stats else 0
    ...
    def _calculate_trends(self) -> Dict[str, Any]:
        ...
        recent_queries = [q for q in self.slow_queries 
                         if q["timestamp"] > datetime.now(timezone.utc) - timedelta(hours=1)]
        
        return {
            "slow_queries_last_hour": len(recent_queries),
            "trend": "improving" if len(recent_queries) < len(self.slow_queries) / 24 else "degrading"
        }
