...
class CallSession(SQLModel, table=True):
    ...
    session_keys: Dict[str, str] = Field(default={}, sa_column=Column(JSON))  # Per-participant keys
...
class CallInvitation(SQLModel, table=True):
    ...
    # Invitation status
    status: str = Field(default="pending", max_length=50, index=True)  # pending, accepted, declined, expired
...
class CallAnalytics(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        Index('idx_call_analytics_quality', 'audio_quality_score', 'video_quality_score'),
    )
