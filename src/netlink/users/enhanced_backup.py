...
from sqlalchemy import Text, DateTime, Index, LargeBinary
...
class EnhancedBackup(SQLModel, table=True):
    ...
    security_level: SecurityLevel = Field(default=SecurityLevel.CONFIDENTIAL, index=True)
    ...
    redundancy_factor: int = Field(default=5, ge=3)  # Government standard: 5 copies minimum
...
class EnhancedBackupShard(SQLModel, table=True):
    ...
    checksum_blake2b: str = Field(max_length=128)  # Additional checksum for verification
...
class ShardDistribution(SQLModel, table=True):
    ...
    storage_type: str = Field(max_length=50)  # 'user_storage', 'backup_node', 'cloud', 'local'
    
    # Distribution details
    distributed_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), index=True)
    last_verified_at: Optional[datetime] = Field(sa_column=Column(DateTime))
    ...
    # Indexes
    __table_args__ = (
        Index('idx_distribution_shard_storage', 'shard_id', 'storage_type', 'is_active'),
...
class BackupNode(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        Index('idx_backup_node_capacity', 'total_capacity_bytes', 'used_capacity_bytes'),
    )
...
class BackupRecoveryLog(SQLModel, table=True):
    ...
    backup_id: int = Field(foreign_key="enhanced_backups.id", index=True)
    recovery_type: str = Field(max_length=50, index=True)  # 'full', 'partial', 'emergency'
    ...
    # Status
    status: str = Field(max_length=50, index=True)  # 'started', 'in_progress', 'completed', 'failed'
