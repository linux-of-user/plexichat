...
class AIAnalyticsEngine:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("CREATE INDEX IF NOT EXISTS idx_usage_model ON usage_metrics(model_id)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_performance_timestamp ON performance_metrics(timestamp)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_cost_timestamp ON cost_metrics(timestamp)")
    ...
    def record_usage(self, metric: UsageMetric):
        ...
        with self._lock:
            ...
            self.cost_buffer.append(CostMetric(
    ...
    def get_usage_analytics(self, 
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in results:
                    user_id, model_id, provider, capability, request_count, total_tokens, total_cost, avg_latency, successful_requests = row
                    ...
                    analytics["by_provider"][provider]["total_tokens"] += total_tokens or 0
                    analytics["by_provider"][provider]["total_cost"] += total_cost or 0.0
                
                # Calculate averages
                if total_requests > 0:
                    analytics["summary"]["avg_latency"] = total_latency / total_requests
                    analytics["summary"]["success_rate"] = total_successful / total_requests
        ...
        try:
            ...
            for rule in self.alert_rules.values():
                ...
                if (rule.last_triggered and 
                    current_time - rule.last_triggered < timedelta(minutes=rule.window_minutes)):
                    continue
    ...
    def _evaluate_alert_condition(self, rule: AlertRule) -> bool:
        ...
        try:
            ...
            recent_performance = [m for m in self.performance_buffer if m.timestamp >= window_start]
