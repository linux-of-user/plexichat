...
class ImmutableShardManager:
    ...
    async def create_shards(
        ...
        """Create immutable shards from backup data."""
        ...
        for i, chunk in enumerate(chunks):
            shard_id = f"{backup_id}_shard_{i:06d}_{secrets.token_hex(8)}"
            
            # Calculate multiple hashes for government-level verification
            hashes = self._calculate_multiple_hashes(chunk)
            ...
            await self._save_shard_data(shard, chunk)
        ...
        logger.info(f"Created {len(shards)} immutable shards for backup {backup_id}")
        return shards
