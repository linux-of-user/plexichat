#!/usr/bin/env python3
"""
NetLink Database Setup CLI Tool
...
try:
    from src.netlink.core.database_setup_wizard import database_wizard, DatabaseType
    from src.netlink.core.external_database import external_db_manager, DatabaseProvider, DatabaseEngine
except ImportError as e:
    print(f"❌ Failed to import NetLink modules: {e}")
    print("Make sure you're running this from the NetLink project directory")
    sys.exit(1)

def print_banner():
    """Print the NetLink database setup banner."""
    print("=" * 60)
    print("🗄️  NetLink Database Setup Tool")
    print("=" * 60)
    print()
...
async def interactive_setup():
    ...
    print_banner()
    print("Welcome to the NetLink Database Setup Wizard!")
    ...
    if selected_type == "sqlite":
        file_path = input("Database file path (default: data/netlink.db): ").strip()
        if not file_path:
        ...
        database = input("Database name (default: netlink): ").strip() or "netlink"
    ...
    print("-" * 30)
    print("Your NetLink database is now configured and ready to use.")
    print("You can start the NetLink server with: python -m src.netlink.app.main")
    print()
...
async def quick_setup(db_type: str, **kwargs):
    ...
    if db_type == "sqlite":
        details = {"file_path": kwargs.get("file_path", "data/netlink.db")}
    else:
        details = {
            "host": kwargs.get("host", "localhost"),
            "port": kwargs.get("port", 5432 if db_type == "postgresql" else 3306),
            "database": kwargs.get("database", "netlink")
        }
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="NetLink Database Setup Tool")
