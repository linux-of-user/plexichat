...
class EnhancedSplitScreen:
    ...
    def _update_header(self):
        ...
        status_text = (
            f"[bold blue]NetLink Enhanced Console[/bold blue] | "
            f"Uptime: {uptime_str} | "
    ...
    def _start_fallback_mode(self):
        ...
        print("=" * 80)
        print("NetLink Enhanced Console - Fallback Mode")
        print("=" * 80)
    ...
    def _fallback_loop(self):
        ...
        while self.active:
            if self.log_buffer:
                ...
                print(f"NetLink Console - Logs: {self.stats['total_logs']} | Errors: {self.stats['errors']}")
                print("-" * 80)
                for entry in recent_logs:
    ...
    def export_logs(self, filename: str = None) -> str:
        ...
        if not filename:
            filename = f"netlink_logs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"

        try:
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(f"NetLink Console Logs Export\n")
