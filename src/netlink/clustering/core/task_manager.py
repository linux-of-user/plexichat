"""
Advanced Task Manager for NetLink Clustering System
...
class AdvancedTaskManager:
    """
    Advanced Task Manager for NetLink Clustering
    ...
    async def _load_tasks_from_db(self):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT * FROM tasks WHERE status NOT IN ('completed', 'failed', 'cancelled')") as cursor:
    ...
    async def _find_suitable_node(self, task: ClusterTask, available_nodes: List[Any]) -> Optional[Any]:
    ...
    async def _update_metrics(self):
        ...
        failed_tasks = sum(1 for task in self.tasks.values() if task.status in [TaskStatus.FAILED, TaskStatus.TIMEOUT])
        ...
        average_execution_time = sum(completed_task_times) / len(completed_task_times) if completed_task_times else 0.0
