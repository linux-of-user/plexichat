...
class OAuthProvider:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # Configuration
        self.issuer = self.config.get("issuer", "https://netlink.local")
    ...
    def _generate_client_id(self) -> str:
        ...
        return f"netlink_{secrets.token_urlsafe(16)}"
    ...
    async def _create_id_token(self, client_id: str, user_id: str, scopes: Set[str]) -> str:
        ...
        if "email" in scopes:
            payload["email"] = f"user_{user_id}@netlink.local"
    ...
    async def get_userinfo(self, access_token: str) -> Dict[str, Any]:
        ...
        if "email" in token_obj.scopes:
            userinfo.update({
                "email": f"user_{token_obj.user_id}@netlink.local",
                "email_verified": True
            })
