...
class LoggingManager:
    """Comprehensive logging manager for NetLink."""
    ...
    def _setup_file_handlers(self, logger: logging.Logger, log_dir: Path):
        ...
        error_handler = CompressingRotatingFileHandler(
            filename=log_dir / "netlink_errors.log",
            maxBytes=self._parse_size(self.config.get("logging.max_file_size", "10MB")),
            backupCount=self.config.get("logging.backup_count", 5),
            encoding="utf-8"
        )
    ...
    def _setup_structured_handler(self, logger: logging.Logger, log_dir: Path):
        ...
        structured_handler = CompressingRotatingFileHandler(
            filename=log_dir / "netlink_structured.jsonl",
            maxBytes=self._parse_size(self.config.get("logging.max_file_size", "10MB")),
            backupCount=self.config.get("logging.backup_count", 5),
            encoding="utf-8"
        )
    ...
    def _send_alert(self, entry: LogEntry):
        ...
        alert_logger = logging.getLogger("netlink.alerts")
...
def get_logger(name: str) -> logging.Logger:
    """Get a logger instance with NetLink configuration."""
