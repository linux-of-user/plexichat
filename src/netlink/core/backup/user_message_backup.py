...
class UniversalBackupManager:
    ...
    async def backup_user_data(
        ...
        if not await self.should_backup_user_data(user_id, data_type):
            logger.debug(f"Skipping backup for user {user_id}, data type {data_type.value} (opted out)")
            return None
        ...
        await self._record_backup_history(user_id, data_type, backup_id, len(serialized_data))
        
        logger.info(f"Backed up {data_type.value} data for user {user_id}: {len(serialized_data)} bytes")
        return backup_id
    ...
    async def _record_backup_history(
        ...
        retention_days = preferences.retention_days if preferences else self.config["default_retention_days"]
    ...
    async def get_user_backup_status(self, user_id: int) -> Dict[str, Any]:
        ...
        async with aiosqlite.connect(self.user_backup_db_path) as db:
            async with db.execute(
                "SELECT COUNT(*) FROM user_backup_history WHERE user_id = ?",
                (user_id,)
            ) as cursor:
