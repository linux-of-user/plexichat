...
from netlink.app.security.rate_limiter import ComprehensiveRateLimiter, RateLimitType, RateLimitAction
from netlink.app.security.permissions import PermissionManager, Permission, PermissionScope, Role
...
class SecurityCLI:
    ...
    async def list_rate_limit_rules(self) -> None:
        ...
        for rule in self.rate_limiter.rules.values():
            ...
            if rule.whitelist_ips:
                print(f"   Whitelisted IPs: {', '.join(rule.whitelist_ips)}")
            if rule.blacklist_ips:
                print(f"   Blacklisted IPs: {', '.join(rule.blacklist_ips)}")
            if rule.user_roles:
                print(f"   User Roles: {', '.join(rule.user_roles)}")
            if rule.endpoints:
                print(f"   Endpoints: {', '.join(rule.endpoints)}")
    ...
    async def create_rate_limit_rule(self, args: List[str]) -> None:
        ...
        from netlink.app.security.rate_limiter import RateLimitRule
        rule = RateLimitRule(
            name=name,
    ...
    async def show_role(self, role_name: str) -> None:
        ...
        print(f"Default Role: {'Yes' if role.is_default else 'No'}")
    ...
    async def check_permission(self, user_id: str, permission: str, scope: str = "global", scope_id: Optional[str] = None) -> None:
        ...
        if check_result.roles_checked:
            print(f"   Roles Checked: {', '.join(check_result.roles_checked)}")
    ...
    async def show_user_permissions(self, user_id: str) -> None:
        ...
        print(f"Created: {user_perms.created_at.strftime('%Y-%m-%d %H:%M:%S')}")
...
async def handle_security_command(args: List[str]) -> None:
    ...
    if not args:
        ...
        print("  delete-rule <name>            - Delete rate limiting rule")
        print("  status <ip> [user_id]         - Show rate limit status")
        print("  banned                        - Show banned entities")
        print("  unban-ip <ip>                 - Unban IP address")
        print("  unban-user <user_id>          - Unban user")
        print("")
        print("Permissions:")
        print("  list-roles                    - List all roles")
        print("  show-role <name>              - Show role details")
        print("  create-role <args>            - Create new role")
        print("  delete-role <name>            - Delete role")
        print("  assign-role <user> <role>     - Assign role to user")
        print("  revoke-role <user> <role>     - Revoke role from user")
        print("  check-perm <user> <perm>      - Check user permission")
        print("  show-user <user_id>           - Show user permissions")
        return
