"""
Background task service for NetLink.
...
from netlink.app.models.moderation import ModerationLog, UserModerationStatus, ModerationStatus
        ...
        while self.running:
            try:
                async with get_session() as session:
                    ...
                    for log in expired_logs:
                        ...
                        if log.target_user_id:
                            ...
                            if user_status:
                                ...
                                if (user_status.is_muted and 
                                    user_status.mute_expires_at and 
                                    user_status.mute_expires_at <= now):
                                    user_status.is_muted = False
                                    user_status.mute_expires_at = None
                                    user_status.mute_reason = None
                                
                                if (user_status.is_banned and 
                                    user_status.ban_expires_at and 
                                    user_status.ban_expires_at <= now):
                                    user_status.is_banned = False
                                    user_status.ban_expires_at = None
                                    user_status.ban_reason = None
                                
                                if (user_status.is_timed_out and 
                                    user_status.timeout_expires_at and 
                                    user_status.timeout_expires_at <= now):
                    ...
                    if expired_logs:
                        session.commit()
                        logger.info(f"Cleaned up {len(expired_logs)} expired moderation actions")
            ...
            try:
                async with get_session() as session:
                    ...
                    statement = select(ModerationLog).where(
                        ...
                        (ModerationLog.status.in_([ModerationStatus.EXPIRED, ModerationStatus.REVOKED]))
                    )
    ...
    async def cleanup_user_data(self, user_id: int) -> Dict[str, int]:
        ...
        try:
            async with get_session() as session:
                ...
                statement = select(ModerationLog).where(
                    (ModerationLog.target_user_id == user_id) | (ModerationLog.moderator_id == user_id)
                )
                ...
                logger.info(f"Cleaned up data for deleted user {user_id}: {cleanup_stats}")
