...
class LogCLI:
    ...
    def create_parser(self) -> argparse.ArgumentParser:
        ...
        parser = argparse.ArgumentParser(
            description="NetLink Advanced Log Management CLI",
            formatter_class=argparse.RawDescriptionHelpFormatter,
            epilog="""
Examples:
  netlink logs list                           # List all log files
  netlink logs view netlink.log               # View log file
  netlink logs view netlink.log -l ERROR      # View only ERROR entries
  netlink logs search "database error"        # Search across all logs
  netlink logs tail netlink.log               # Follow log file
  netlink logs stats netlink.log              # Show log statistics
  netlink logs archive --days 7               # Archive logs older than 7 days
  netlink logs export netlink.log -f json     # Export logs as JSON
  netlink logs clean --days 30                # Clean old archives
            """
        )
    ...
    def cmd_list(self, args):
        ...
        print(f"{'Filename':<30} {'Size':<10} {'Entries':<8} {'Modified':<20} {'Type':<8}")
        print("-" * 80)
        ...
        for log_file in log_files:
            ...
            print(f"{color}{log_file.filename:<30}{self.colors['RESET']} "
    ...
    def cmd_view(self, args):
        ...
        try:
            ...
            print(f"Showing {len(entries)} of {total_lines} total lines\n")
            ...
            for filename, entry in results:
                if filename != current_file:
                    if current_file is not None:
                        print()
                    print(f"{self.colors['BOLD']}{self.colors['CYAN']}=== {filename} ==={self.colors['RESET']}")
    ...
    def cmd_tail(self, args):
        ...
        try:
            ...
            print("(Real-time following would be implemented here)")
    ...
    def print_log_entry(self, entry: LogEntry, use_color: bool = True, highlight_term: str = None):
        ...
        print(f"{timestamp_color}{timestamp}{reset} "
    ...
    def print_header(self, text: str):
        ...
        print(f"{self.colors['BOLD']}{self.colors['GREEN']}{text}{self.colors['RESET']}")
