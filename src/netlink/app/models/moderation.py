"""
Moderation system models for NetLink.
...
class ModeratorRole(SQLModel, table=True):
    ...
    max_punishment_severity: ModerationSeverity = Field(default=ModerationSeverity.MEDIUM)
...
class MessageModerationQueue(SQLModel, table=True):
    ...
    # Detection details
    flagged_by: Optional[str] = Field(max_length=50)  # 'auto', 'user_report', 'moderator'
    ...
    review_decision: Optional[str] = Field(max_length=50)  # 'approved', 'removed', 'edited'
    review_notes: Optional[str] = Field(sa_column=Column(Text))
    
    # Timestamps
    flagged_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), index=True)
...
class AutoModerationRule(SQLModel, table=True):
    ...
    # Rule configuration
    rule_type: str = Field(max_length=50, index=True)  # 'keyword', 'spam', 'link', 'mention'
    ...
    time_window_minutes: Optional[int] = Field(default=None)  # Time window for counting violations
