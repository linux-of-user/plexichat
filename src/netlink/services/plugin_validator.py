"""
Plugin Validation System for NetLink Marketplace
...
class PluginValidator:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.required_metadata = [
            "name", "version", "description", "author", "netlink_version"
        ]
        
        # Compatibility checks
        self.min_netlink_version = "3.0.0"
    ...
    async def _validate_compatibility(self, result: ValidationResult, plugin_path: Path, metadata: Optional[Dict[str, Any]]):
        """Validate NetLink compatibility."""
        if not self.config["enable_compatibility_check"]:
            return
        
        if metadata:
            # Check NetLink version compatibility
            required_version = metadata.get("netlink_version", "")
            if required_version and required_version < self.min_netlink_version:
                result.issues.append(ValidationIssue(
                    category=ValidationCategory.COMPATIBILITY,
                    severity=ValidationSeverity.WARNING,
                    code="OLD_NETLINK_VERSION",
                    message=f"Plugin requires old NetLink version: {required_version}",
                    suggestion=f"Update to support NetLink {self.min_netlink_version}+"
                ))
